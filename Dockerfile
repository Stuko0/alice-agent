...
# Do not write .pyc files at runtime: /opt/alice is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/alice/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when alice
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# alice process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main alice process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Alice WebUI' catalog). The image has moved to s6-overlay /init\
...
#
# ui-tui/packages/alice-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/alice-ink/ ui-tui/packages/alice-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as alice-ink above).
...
# lock on the @alice/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/alice/.venv at first
...
# gives the non-root alice user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link alice-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/alice are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the alice user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/alice/bin && \
    cp /opt/alice/docker/alice-exec-shim.sh /opt/alice/bin/alice && \
...
# container always returns nothing — meaning `alice dump` reports
...
# Static services declared at build time: main-alice + dashboard.
...
# `exec alice`. Wired in as cont-init.d/01- so it
...
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/alice/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-alice-setup && \
    chmod +x /etc/cont-init.d/01-alice-setup
...
# bin globally so `docker exec <container> alice ...` and any
# subprocess that doesn't activate the venv first still find alice.
#
# /opt/alice/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/alice/bin:/opt/alice/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. alice
# subcommand vs. no-args), drops to the alice user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/alice/docker/main-wrapper.sh" ]
CMD [ ]
