...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_alice_home() under the active profile.
...
#   - cli.py, alice_cli/*      -> inline on startup (no event loop)
...
def _compute_tool_definitions(
    ...
    # This ensures that even if a composite toolset (like alice-cli)
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from alice_cli.config import load_config as _load
        cfg = _load() or {}
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from alice_cli.plugins import has_hook, invoke_hook
