import { buildAliceWebSocketUrl } from "@alice/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/alice/). The Python backend
// injects ``window.__ALICE_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ALICE_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ALICE_BASE_PATH = readBasePath();
const BASE = ALICE_BASE_PATH;
...
const SESSION_HEADER = "X-Alice-Session-Token";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``alice update``, ``alice gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ALICE_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ALICE_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("alice.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("alice.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__ALICE_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("alice.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__ALICE_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Alice dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``alice_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ALICE_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Alice-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``alice_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__ALICE_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateAlice: () =>
    fetchJSON<ActionResponse>("/api/alice/update", { method: "POST" }),
  checkAliceUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/alice/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** alice-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``alice update``. */
  can_update_alice?: boolean;
  config_path: string;
