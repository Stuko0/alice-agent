...
export default function SystemPage() {
  ...
  // spawned `alice import` runs non-interactively (stdin is /dev/null), so
  ...
  const applyUpdate = async () => {
    ...
    try {
      ...
      setActiveAction(resp.name ?? "alice-update");
      ...
      <ConfirmDialog
        ...
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'alice update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'alice update' (${updateInfo?.update_command ?? "alice update"}) and restart the gateway when it finishes.`
        }
          ...
          <CardContent className="flex flex-col gap-3 py-4">
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">alice portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="flex flex-col gap-4 py-4">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
              ...
              <span className="ml-auto">
                New credentials:{" "}
                <span className="font-mono">alice memory setup</span>
              </span>
            </div>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <ConfirmDialog
              ...
              title="Restore full Alice backup?"
              description={`This will overwrite your current Alice configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Alice team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
