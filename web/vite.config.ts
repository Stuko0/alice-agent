  ...
  return {
    ...
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[alice] Could not find session token in ${BACKEND} — ` +
              `is \`alice dashboard\` running? /api calls will 401.`,
          );
          return;
        }
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@alice/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `alice dashboard` must serve these; Vite has no
