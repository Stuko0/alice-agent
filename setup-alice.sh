...
# 5. Symlinks the 'alice' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}🌹 Alice Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/alice-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink alice into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up alice command..."

ALICE_BIN="$SCRIPT_DIR/venv/bin/alice"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ALICE_BIN" "$COMMAND_LINK_DIR/alice"
echo -e "${GREEN}✓${NC} Symlinked alice → $COMMAND_LINK_DISPLAY_DIR/alice"
...
# ============================================================================
# Seed bundled skills into ~/.alice/skills/
# ============================================================================

ALICE_SKILLS_DIR="${ALICE_HOME:-$HOME/.alice}/skills"
mkdir -p "$ALICE_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.alice/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ALICE_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  alice status        # Check configuration"
if is_termux; then
    echo "  alice gateway       # Run gateway in foreground"
else
    echo "  alice gateway install # Install gateway service (messaging + cron)"
fi
echo "  alice cron list     # View scheduled jobs"
echo "  alice doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m alice_cli.main setup
fi
