#!/usr/bin/env python3
"""Live test harness for Alice Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any alice imports.
ORIGINAL_HOME = os.environ.get("ALICE_HOME")
ORIGINAL_AUTH = Path.home() / ".alice" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.alice/ for one test, copying minimal credentials.
...
def reset_module_state():
    """Drop cached modules so the new ALICE_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "alice_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["ALICE_HOME"] = str(home)
...
def main():
    ...
    # Restore original ALICE_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ALICE_HOME"] = ORIGINAL_HOME
    else:
