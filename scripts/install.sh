...
# Python-driven tool session (e.g. Alice terminal tool). A pre-set PYTHONPATH
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Alice.app)"
            ...
            echo "                   Installs optional deps + runs alice setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `alice update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Alice-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Alice Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install alice command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `alice` command
...
resolve_install_layout() {
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        ...
        INSTALL_DIR="/usr/local/lib/alice-agent"
        ...
        # /usr/local/bin/alice wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
...
# Alice's own global installs pass an explicit --prefix and are unaffected.
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/alice-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/alice-uv-installer.$$.sh")"
...
# string clears that floor; anything below it is replaced with the Alice-
...
check_node() {
    ...
    # Repair pre-existing Alice-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Alice-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.alice/node/..."
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Alice install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Alice Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `alice update` path
                # (#4735).
                ...
                local stash_name
                stash_name="alice-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``alice update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Alice behaves unexpectedly."
                    else
...
setup_path() {
    ...
    # Create a user-facing shim for the alice command.
...
EOF
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_success "alice command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v alice' \
                >/dev/null 2>&1; then
        ...
        log_info "alice not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Alice Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Alice Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Alice Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so alice works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    if [ ! -f "$ALICE_HOME/.env" ]; then
        ...
        log_info "~/.alice/.env already exists, keeping it"
    fi
    ...
    if [ ! -f "$ALICE_HOME/config.yaml" ]; then
        ...
        log_info "~/.alice/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in alice_cli/default_soul.py — the
...
SOUL_EOF
    ...
    log_success "Configuration directory ready: ~/.alice/"

    # Seed bundled skills into ~/.alice/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `alice update` both honor this marker, so the
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Alice Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Alice will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Alice Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (alice --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `alice update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'alice setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m alice_cli.main setup < /dev/tty
    else
        python -m alice_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Alice to send/receive messages."
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'alice gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ALICE_CMD gateway install 2>/dev/null; then
                ...
                log_warn "Systemd install failed. You can start manually: alice gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
...
print_success() {
    ...
    echo ""
    echo -e "   ${GREEN}alice${NC}              Start chatting"
    echo -e "   ${GREEN}alice setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}alice config${NC}       View/edit configuration"
    echo -e "   ${GREEN}alice config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}alice gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}alice update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'alice' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'alice' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'alice' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Alice for pip install"
...
# extract a tree MISSING the electron binary, so the `electron`->`Alice` rename
...
# _purge_electron_build_cache() used by `alice desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Alice-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `alice desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Alice" ]; then
            app="$desktop_dir/release/linux-unpacked/Alice"
        elif [ -x "$desktop_dir/release/linux-unpacked/alice" ]; then
            app="$desktop_dir/release/linux-unpacked/alice"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Alice.app" \
            "$desktop_dir/release/mac/Alice.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Alice is damaged and can't be opened").
    ...
    # checkout stays clean for the next `alice update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
