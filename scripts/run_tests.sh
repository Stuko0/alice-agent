#!/usr/bin/env bash
# Canonical test runner for alice-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.alice/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.alice/alice-agent/venv"; do
...
if [ -f "$HOME/.alice/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.alice"
...
exec env -i \
  ...
  ${ALICE_RUN_SLOW_PET_TESTS:+ALICE_RUN_SLOW_PET_TESTS="$ALICE_RUN_SLOW_PET_TESTS"} \
