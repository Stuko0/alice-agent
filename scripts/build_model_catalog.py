#!/usr/bin/env python3
"""Build the Alice Model Catalog — a centralized JSON manifest of curated models.
...
Alice CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Alice release.
...
# Ensure ALICE_HOME is set for imports that touch it at module level.
os.environ.setdefault("ALICE_HOME", os.path.join(os.path.expanduser("~"), ".alice"))

from alice_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS  # noqa: E402
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "alice-agent repo",
            "docs": "https://alice-agent.nousresearch.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
