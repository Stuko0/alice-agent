...
VERSION_FILE = REPO_ROOT / "alice_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows alice-shim resolution + prefer --update on recovery; #52378)
    ...
    "274096618+alice-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "alice@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyaalice.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (ALICE_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (ALICE_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: alice-eval community link)
    ...
    "aliceagent26@gmail.com": "aliceagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+aliceagent26@users.noreply.github.com": "aliceagent26",
    ...
    "vigo@alice": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Alice runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level ALICE_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Alice
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"alice-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "alice@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://10.1.200.116:3000/arquant-admin/NewAlice",
    ...
    lines.append(f"# Alice Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Alice Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Alice Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Alice Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Alice Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Alice Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Alice Agent v{new_version} ({calver_date})' "
