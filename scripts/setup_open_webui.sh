...
# Bootstrap Open WebUI against Alice Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.alice/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-alice.sh
...
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Alice Agent WebUI}"
...
install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.alice.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.alice</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.alice"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.alice"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-alice.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Alice Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-alice.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.alice/logs/openwebui.log
StandardError=append:%h/.alice/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-alice.service
}
