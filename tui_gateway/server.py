...
# appends every unhandled exception to ~/.alice/logs/tui_gateway_crash.log
...
class _SlashWorker:
    """Persistent AliceCLI subprocess for slash commands."""
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from alice_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from alice_cli.plugins import invoke_hook
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `alice --tui` keeps real _stdio.
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `alice --tui`, so it must NOT count as dead here (doing
...
def _max_live_sessions() -> int:
    try:
        from alice_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its ALICE_HOME
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s ALICE_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_alice_home``).
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `alice` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # ALICE_HOME so config/skills/model resolve to it, and hand the
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from alice_cli.runtime_provider import canonical_custom_identity
...
def _apply_managed(cfg: dict) -> dict:
    ...
    alice_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def resolve_skin() -> dict:
    try:
        from alice_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from alice_cli.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    # Coding posture (base Alice): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `alice --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from alice_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(
    ...
    *,
    ...
    custom_provs = None
    try:
        from alice_cli.config import get_compatible_custom_providers, load_config
    ...
    if agent:
        try:
            from alice_cli.context_switch_guard import merge_preflight_compression_warning
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from alice_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from alice_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from alice_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from alice_cli.config import load_config as _load_full_cfg
...
def _load_fallback_model():
    ...
    stays in parity with ``AliceCLI.__init__`` and ``gateway/run.py``:
...
def _resolve_runtime_with_fallback(
    ...
    ``alice_cli/cli_agent_setup_mixin.py``: when the primary provider
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses alice_cli.mcp_startup; TUI stdio keeps
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind ALICE_HOME. None/own profile → launch (unchanged).
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds ALICE_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``alice --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``alice gateway`` process, whose
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Alice has to support both the legacy 8-row petdex atlas and the current
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from alice_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from alice_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from alice_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from alice_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from alice_cli.pets import set_pet_scale
...
def _pet_gen_root():
    ...
    root = get_alice_home() / "cache" / "pet-gen"
...
# (agent/billing_view.py + alice_cli/nous_billing.py) — same as /credits.
...
def _(rid, params: dict) -> dict:
    ...
    """
    from alice_cli.nous_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    ...
    """
    from alice_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from alice_cli.nous_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Alice profile home
    # (~/.alice/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $ALICE_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_alice_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn ALICE_HOME override for a resumed remote profile
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from alice_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from alice_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: alice_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from alice_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".alice" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.alice/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.alice/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Alice Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Alice/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Alice Desktop. If it serves Alice/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from alice_cli.model_switch import parse_model_flags
...
def _projects_payload(conn) -> dict:
    from alice_cli import projects_db as pdb
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under ALICE_HOME (~/.alice by default) — config/sessions/skills,
...
def _discover_repos_payload(db, *, conn=None, backfill: bool = True) -> list[dict]:
    ...
    repos even with zero alice sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (alice home subtree + bare
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from alice_cli.models import list_available_providers, normalize_provider
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from alice_cli.runtime_provider import resolve_runtime_provider
        from alice_cli.auth import has_usable_secret
        from alice_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Alice provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.alice/.env`` into the gateway process via
    ``alice_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `alice` is interactive — use `/alice chat -q …` or run `alice` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`alice setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`alice gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`alice sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`alice config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m alice_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "alice_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from alice_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from alice_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from alice_cli.config import is_managed, save_env_value
        from alice_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `alice model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.alice/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from alice_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from alice_cli.config import remove_env_value
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from alice_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from alice_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from alice_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from alice_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from alice_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``alice journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from alice_cli.skills_hub import do_install
        ...
        if action == "browse":
            from alice_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from alice_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``alice plugins`` / the dashboard, so the three surfaces
