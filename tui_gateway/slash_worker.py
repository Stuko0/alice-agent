"""Persistent slash-command worker — one AliceCLI per TUI session.
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``ALICE_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("ALICE_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("ALICE_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _run(cli: AliceCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ALICE_SESSION_KEY"] = args.session_key
    os.environ["ALICE_INTERACTIVE"] = "1"

    # Start before the (hundreds-of-ms) AliceCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = AliceCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
