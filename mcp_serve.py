"""
Alice MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Alice-specific extra)

Usage:
    alice mcp serve
    alice mcp serve --verbose
...
logger = logging.getLogger("alice.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ALICE_HOME."""
    try:
        from alice_constants import get_alice_home
        return get_alice_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ALICE_HOME", Path.home() / ".alice")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from alice_constants import get_alice_home
        directory_file = get_alice_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ALICE_HOME", Path.home() / ".alice")
...
class EventBridge:
    ...
    This is the Alice equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from alice_constants import get_alice_home
            db_file = get_alice_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ALICE_HOME", Path.home() / ".alice")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Alice MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "alice",
        instructions=(
            "Alice Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Alice MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
