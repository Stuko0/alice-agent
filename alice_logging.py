"""Centralized logging setup for Alice Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.alice/logs/`` (profile-aware via ``get_alice_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``alice_cli.web_server``,
    ``alice_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Alice (TUI, gateway, ``hy_memory`` server, MCP
...
from alice_constants import get_config_path, get_alice_home
...
# Used by _ComponentFilter and exposed for ``alice logs --component``.
COMPONENT_PREFIXES = {
    ...
    "cron": ("cron",),
    "gui": (
        "alice_cli.web_server",
        "alice_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Alice logging subsystem.
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the alice group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from alice_cli.config import is_managed
