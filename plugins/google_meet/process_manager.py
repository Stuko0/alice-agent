...
session-scoped state file under ``$ALICE_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ALICE_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_alice_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this alice install, leave it first —
    ...
    env = os.environ.copy()
    env["ALICE_MEET_URL"] = url
    env["ALICE_MEET_OUT_DIR"] = str(out)
    env["ALICE_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ALICE_MEET_HEADED"] = "1"
    if auth_state:
        env["ALICE_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ALICE_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ALICE_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ALICE_MEET_MODE"] = mode
    if realtime_model:
        env["ALICE_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ALICE_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ALICE_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ALICE_MEET_REALTIME_KEY"] = realtime_api_key
