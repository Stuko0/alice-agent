...
alice meet install                 # pip deps + Chromium (transcribe only)
alice meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
alice meet auth                    # optional; skips guest-lobby wait
alice meet setup                   # preflight checks
```

`alice meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
pip install playwright websockets && python -m playwright install chromium
alice plugins enable google_meet
alice meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
alice meet node approve my-mac ws://<mac-ip>:18789 <token>
alice meet node ping my-mac                   # confirm reachable
```

Run `alice meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Alice Agent bot is in this call taking notes."
...
Local:
```
$ALICE_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$ALICE_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `alice meet node approve`.
