...
Let the Alice Agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where alice runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `alice meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `alice meet` CLI |
...
| `cli.py` | `alice meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ALICE_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `alice meet node {run,list,approve,remove,status,ping}` |
...
alice meet install                                      # pip + Chromium
alice meet setup                                        # preflight
alice meet auth                                         # optional
alice meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
alice meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.alice/.env
alice meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
alice meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
alice meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.alice/.env
alice meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, alice will **not** switch your system audio input automatically — the
...
alice plugins enable google_meet
alice meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
alice meet node approve my-mac ws://<mac-ip>:18789 <token>
alice meet node ping my-mac
