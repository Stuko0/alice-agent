...
``{title, url, description, position}`` rows every other Alice web
provider produces.
...
prefers Alice-managed xAI Grok OAuth (via ``alice auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.alice/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``alice native`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `alice auth` to sign in with "
