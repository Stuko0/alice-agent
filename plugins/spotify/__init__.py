...
``_check_spotify_available()`` — when the user has not run ``alice auth
spotify``, the tools remain registered (so they appear in ``alice
...
The Spotify auth flow (``alice auth spotify``), CLI plumbing, and docs
