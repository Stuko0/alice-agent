...
A standards-compliant OpenID Connect Relying Party for the ``alice dashboard``
...
:class:`~alice_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``alice_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~alice_cli.dashboard_auth.Session`.
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Alice does not call
