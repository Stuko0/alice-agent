"""langfuse — Alice plugin for Langfuse observability.

Traces Alice conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Alice plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``alice plugins enable
observability/langfuse`` or ``alice native → Langfuse Observability``). At
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Alice plugin system —
...
def _trace_key(
    ...
    *,
    ...
    Older Alice paths only expose ``task_id``/``session_id``. Newer paths
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Alice branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Alice also has a turn-scoped
    ...
    # messages is a list only for legacy Alice branches that fired
    # pre_llm_call with API messages directly. Current Alice fires
...
def register(ctx) -> None:
    ...
    # Alice versions.  pre_api_request / post_api_request fire per API
