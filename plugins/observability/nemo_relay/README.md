...
Optional Alice observability plugin that maps Alice observer hooks to
...
not replace Alice Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Alice emit NeMo Relay lifecycle events for
the work Alice already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Alice Agent can:

- Preserve Alice execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
alice plugins enable observability/nemo_relay
```

The `ALICE_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `ALICE_HOME` that the
agent run will use:

```bash
env ALICE_HOME=/tmp/alice-nemo-relay-test \
  alice plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`ALICE_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`ALICE_HOME` is the Alice profile/config home used by both
`alice plugins enable ...` and the later `alice chat ...` run. If unset,
Alice uses the user's default home, usually `~/.alice`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export ALICE_HOME=/tmp/alice-nemo-relay-test
alice plugins enable observability/nemo_relay
alice chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `alice` command you run is built from the
...
uv run alice plugins enable observability/nemo_relay
uv run alice chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/alice_agent-*.whl
...
The plugin can configure exporters directly from `ALICE_NEMO_RELAY_*`
...
```bash
export ALICE_NEMO_RELAY_ATOF_ENABLED=1
export ALICE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export ALICE_NEMO_RELAY_ATIF_ENABLED=1
export ALICE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `ALICE_NEMO_RELAY_ATOF_FILENAME`
- `ALICE_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `ALICE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `ALICE_NEMO_RELAY_ATIF_AGENT_NAME`
- `ALICE_NEMO_RELAY_ATIF_AGENT_VERSION`
- `ALICE_NEMO_RELAY_ATIF_MODEL_NAME`
- `ALICE_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Alice at it:

```bash
export ALICE_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `ALICE_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`ALICE_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`ALICE_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Alice does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Alice keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Alice routes LLM and tool
...
For the full generic Alice middleware contract, see
...
export ALICE_HOME=/tmp/alice-nemo-relay-docs/alice-home
mkdir -p "$ALICE_HOME"

cat > "$ALICE_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Alice session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export ALICE_NEMO_RELAY_ATOF_ENABLED=1
export ALICE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/alice-nemo-relay-docs/subagent/atof
export ALICE_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export ALICE_NEMO_RELAY_ATOF_MODE=overwrite
export ALICE_NEMO_RELAY_ATIF_ENABLED=1
export ALICE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/alice-nemo-relay-docs/subagent/atif
export ALICE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export ALICE_NEMO_RELAY_ATIF_AGENT_NAME='Alice Agent E2E'
export ALICE_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export ALICE_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"alice.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Alice Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Alice dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/alice-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/alice-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/alice-nemo-relay-docs/workdir/beta.txt
cd /tmp/alice-nemo-relay-docs/workdir

export ALICE_NEMO_RELAY_ATOF_ENABLED=1
export ALICE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/alice-nemo-relay-docs/parallel/atof
export ALICE_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export ALICE_NEMO_RELAY_ATOF_MODE=overwrite
export ALICE_NEMO_RELAY_ATIF_ENABLED=1
export ALICE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/alice-nemo-relay-docs/parallel/atif
export ALICE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export ALICE_NEMO_RELAY_ATIF_AGENT_NAME='Alice Agent E2E'
export ALICE_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Alice sessions map to `agent` scopes.
- Alice API request hooks map to `llm` scope start/end events.
- Alice tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Alice execution middleware to hand
...
```bash
export ALICE_NEMO_RELAY_PLUGINS_TOML=/tmp/alice-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Alice routes execution
...
Alice provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Alice provider adapter next_call(...)

Alice tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Alice tool dispatcher next_call(...)
```
...
middleware for a local Alice run. This path requires a NeMo Relay runtime that
...
```bash
export ALICE_HOME=/tmp/alice-middleware-test/alice-home
mkdir -p "$ALICE_HOME" /tmp/alice-middleware-test/nemo-relay
...
cat > /tmp/alice-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/alice-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/alice-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Alice Middleware E2E"
agent_version = "local"
...
export ALICE_NEMO_RELAY_PLUGINS_TOML=/tmp/alice-middleware-test/nemo-relay/plugins.toml
