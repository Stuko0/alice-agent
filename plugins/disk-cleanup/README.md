...
Auto-tracks and cleans up ephemeral files created during Alice Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ALICE_HOME` and `/tmp/alice-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ALICE_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ALICE_HOME` or `/tmp/alice-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ALICE_HOME/disk-cleanup/` is itself excluded
- `$ALICE_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
