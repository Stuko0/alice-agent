"""disk_cleanup — ephemeral file cleanup for Alice Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ALICE_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ALICE_HOME and /tmp/alice-*
Never touches: ~/.alice/logs/ or any system directory.
"""
...
try:
    ...
    def get_alice_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ALICE_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".alice").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ALICE_HOME/logs/``."""
    return get_alice_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ALICE_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ALICE_HOME or ``/tmp/alice-*``.
    ...
    try:
        ...
        pass
    # Allow /tmp/alice-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("alice-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "alice-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $ALICE_HOME that must NEVER be deleted by quick(),
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ALICE_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ALICE_HOME, but never recurse into known
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under ALICE_HOME (e.g. /tmp/alice-*) — fall through.
        pass
