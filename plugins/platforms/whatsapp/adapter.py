...
def _is_allowed_bridge_path(url: str) -> bool:
    ...
    known Alice media cache directory.
...
def check_whatsapp_requirements() -> bool:
    ...
    """
    # Prefer Alice-managed Node/npm so Windows installs are not broken by a
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `alice gateway`.",
                retryable=False,
            )
            return False
        ...
        # clear "run alice whatsapp" message instead of the watcher
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Run `alice whatsapp` to pair, or remove WHATSAPP_ENABLED from "
                "your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — run `alice whatsapp` to pair.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `alice update` bumps the Baileys pin).  The stamp file records
            # the package.json hash of the last successful install.
            bridge_dir = bridge_path.parent
            _pkg_json = bridge_dir / "package.json"
            _dep_stamp = bridge_dir / "node_modules" / ".alice-pkg-hash"
            ...
            if not _deps_fresh:
                ...
                # Alice-managed portable Node before falling back to PATH.
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `alice update`, so without this check it
            ...
            # hardcodes ~/.alice/{image,audio,document}_cache, which diverges
            # under ALICE_HOME overrides, profiles, and the new cache/ layout.
            from gateway.platforms.base import (
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    print(f"[{self.name}]   If session expired, re-pair: alice whatsapp")
...
# dict in alice_cli/gateway.py, and the _send_whatsapp dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_whatsapp in alice_cli/gateway.py and the
...
def _is_connected(config) -> bool:
    ...
    would make WhatsApp always show as "configured" in ``alice setup`` even
    ...
    if config is not None and getattr(config, "enabled", False) and extra:
        ...
        return True
    # Read via alice_cli.gateway.get_env_value (not os.getenv) so setup-status
