...
# ``model_tools`` import time, so ``alice status``, ``alice chat``, etc.
...
#     fresh "Alice is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a alice MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ALICE_HOME}/google_chat_thread_counts.json so
        ...
        # they don't sit in the chat forever as "Alice is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ALICE_HOME", str(_Path.home() / ".alice"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ALICE_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a alice MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Alice is thinking…' marker message.
        ...
        "Alice is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Alice is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Alice is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Alice is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Alice is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``alice setup``.

    The setup wizard at ``alice_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.alice/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. alice-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.alice/.env")
    print_info("Restart the gateway: alice gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``alice cron`` running as a
    separate process from ``alice gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Alice plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'alice-agent[google_chat]'",
        ...
        # the "Alice is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Alice is thinking…' marker message "
