"""
LINE Messaging API platform adapter for Alice Agent.
...
to Alice Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ALICE_HOME``. PR #8398.
        """
        from aiohttp import web
...
def is_connected(config) -> bool:
    """Surface in ``alice status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``alice status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``alice setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.alice/.env`` via ``alice_cli.config``.
    """
    print()
    ...
    try:
        from alice_cli.config import get_env_var, set_env_var
    except ImportError:
        print("alice_cli.config not available; set LINE_* vars manually in ~/.alice/.env")
        return
