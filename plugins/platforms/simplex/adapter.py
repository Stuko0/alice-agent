"""SimpleX Chat platform adapter (Alice plugin).
...
This adapter ships as a Alice platform plugin under
``plugins/platforms/simplex/``. The Alice plugin loader scans the
...
Optional environment variables:
    ...
    ALICE_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``alice setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Alice — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ALICE_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``alice setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.alice/.env`` via
    ``alice_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "alice_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.alice/.env"
        )
        return
