...
In single-bot mode (what Alice currently supports), open_id works as a
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_alice_home() / "feishu_seen_message_ids.json"
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10,
                    thread_name_prefix="alice-feishu-sdk",
                )
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Alice gateway is already using this Feishu app_id"
    ...
    def _write_update_prompt_response(answer: str) -> None:
        response_path = get_alice_home() / ".update_response"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Alice does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Alice webhook mode")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto Alice' SessionSource fields.
...
# Called by `alice gateway setup` via _setup_feishu() in alice_cli/gateway.py.
# =============================================================================
...
# dict in alice_cli/gateway.py, and the _send_feishu dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    """
    if not FEISHU_AVAILABLE:
        return {"error": "Feishu dependencies not installed. Run: pip install 'alice-agent[feishu]'"}
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_feishu in alice_cli/gateway.py and the static
    _PLATFORMS["feishu"] dict. CLI helpers are lazy-imported.
    """
    from alice_cli.config import get_env_value, save_env_value
    from alice_cli.setup import prompt_choice
    ...
    if access_idx == 0:
        ...
        print_info("Unknown users can request access; approve with `alice pairing approve`.")
    elif access_idx == 1:
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'alice-agent[feishu]'",
