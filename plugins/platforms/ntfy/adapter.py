"""ntfy platform adapter (Alice plugin).
...
``plugins/platforms/ntfy/``. The Alice plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "alice-in"              # subscribe topic (incoming)
          publish_topic: "alice-out"     # optional — defaults to topic
...
_ECHO_TAG = "alice-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``alice cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Alice dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `alice gateway status` without
