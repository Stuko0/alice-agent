"""
Email platform adapter for the Alice gateway.

Allows users to interact with Alice by sending emails.
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `alice config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `alice gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Alice Agent")
        ...
        msg_id = f"<alice-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
...
# alice_cli/gateway.py, and the _send_email dispatch in
