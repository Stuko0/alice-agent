"""
Photon Spectrum (iMessage) platform adapter for Alice Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Alice dep
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `alice photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `alice setup` / status.
        return False
    return True
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Alice defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Alice-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Alice checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `alice photon setup`)"
            )
...
async def _standalone_send(
    ...
    *,
    ...
    headers = {"X-Alice-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Alice plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `alice photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: alice photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `alice gateway setup` alongside every other
    ...
    # Register CLI subcommands — `alice photon ...`
