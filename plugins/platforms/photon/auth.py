...
Credential storage mirrors every other Alice channel:

    * runtime SDK creds  -> ``~/.alice/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.alice/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a alice dependency
...
# registers Alice as its own client_id.
...
# Default name of the project Alice provisions for the operator.
DEFAULT_PROJECT_NAME = "Alice Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of alice-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.alice/auth.json`` honouring the active Alice profile."""
    try:
        from alice_constants import get_alice_home
        return Path(get_alice_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.alice")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.alice/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.alice/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.alice/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: alice_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `alice photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `alice photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `alice photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `alice photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `alice photon setup`)"
