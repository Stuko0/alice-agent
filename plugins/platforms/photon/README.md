...
This plugin connects Alice Agent to iMessage (and other Spectrum
...
TypeScript-only, Alice runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Alice-Sidecar-Token`.
...
There is no separate `login` command; like every other Alice channel,
...
Run `alice photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.alice/.env` (the same place every other
...
Management metadata lives in `~/.alice/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `alice photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `alice photon status`, a DM and a group roundtrip,
