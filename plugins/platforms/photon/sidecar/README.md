...
Small Node helper that bridges Alice Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Alice is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Alice-Sidecar-Token`)
...
The Alice plugin's `alice photon setup` command runs `npm install`
here automatically.
