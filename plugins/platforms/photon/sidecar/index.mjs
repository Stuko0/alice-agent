// Alice Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Alice): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Alice -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Alice-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `alice photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Alice can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Alice'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Alice allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Alice, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-alice-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Alice supplied them so a known-good
