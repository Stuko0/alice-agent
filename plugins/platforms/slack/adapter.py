...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `alice slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_alice_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Alice does not currently need to route them into the agent loop.
            ...
            # /model work directly without /alice prefix). A single regex
            ...
            # manifest (see `alice slack manifest`). In Socket Mode, Slack
            ...
            from alice_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/alice$")

            @self._app.command(_slash_pattern)
            async def handle_alice_command(ack, command):
            ...
            for _action_id in (
                "alice_approve_once",
                "alice_approve_session",
                "alice_approve_always",
                "alice_deny",
            ):
            ...
            for _action_id in (
                "alice_confirm_once",
                "alice_confirm_always",
                "alice_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Alice handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Alice session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "alice_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "alice_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "alice_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "alice_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "alice_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "alice_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "alice_confirm_once": "once",
            "alice_confirm_always": "always",
            "alice_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "alice_approve_once": "once",
            "alice_approve_session": "session",
            "alice_approve_always": "always",
            "alice_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/alice <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/alice
        ...
        if slash_name in {"alice", ""}:
            # Legacy /alice <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from alice_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/alice   ``).
        ...
        # "Running /cmd…" ack shown by handle_alice_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/alice <question>" must produce public replies so
...
# ``alice_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``alice_cli/setup.py::_setup_slack``.
    """
    from pathlib import Path
    from alice_cli.config import get_env_value, save_env_value
    from alice_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under ALICE_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from alice_cli.slack_cli import _build_full_manifest
            from alice_constants import get_alice_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Alice",
                bot_description="Your Alice Agent on Slack",
            )
            target = Path(get_alice_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `alice slack manifest --write` anytime to refresh after "
                "Alice adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `alice update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://alice-agent.stuko.dev/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``alice_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'alice-agent[slack]'",
        # Interactive setup wizard — replaces alice_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in alice_cli/gateway.py.
