"""alice-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $ALICE_HOME/config.yaml (profile-scoped):
  plugins:
    alice-memory-store:
      db_path: $ALICE_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    ...
    try:
        ...
        return cfg_get(all_config, "plugins", "alice-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, lydia_home):
        ...
        try:
            ...
            existing["plugins"]["alice-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_alice_home()}/memory_store.db"
        return [
