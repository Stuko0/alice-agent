...
- Supermemory API key from [app.supermemory.ai/integrations?connect=alice](http://app.supermemory.ai/integrations?connect=alice)
...
Config file: `$ALICE_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `alice` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `alice-{identity}` → `alice-coder`). |
...
All Supermemory API calls send `x-sm-source: alice`, and document writes stamp
`metadata.sm_source: alice`. This is a **functional routing key, not telemetry**:
it groups Alice-written memories into a dedicated "Alice" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Alice profile:

```json
{
  "container_tag": "alice-{identity}"
}
```

For a profile named `coder`, this resolves to `alice-coder`. The default profile resolves to `alice-default`. Without `{identity}`, all profiles share the same container.
