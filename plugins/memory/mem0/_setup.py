...
def _setup_oss(alice_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "alice-user")
...
_PGVECTOR_CONTAINER = "alice-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "alice"
...
def _setup_oss_interactive(alice_home: str, config: dict) -> None:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'alice-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "alice-user")

    agent_id = input("  Agent ID [alice]: ").strip()
