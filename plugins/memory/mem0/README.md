...
```bash
alice memory setup    # select "mem0"
```
...
Behavioral settings live in `$ALICE_HOME/mem0.json` (set them via `alice memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.alice/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `alice-user` | User identifier on Mem0 |
...
```bash
alice memory setup mem0 --mode oss \
...
```bash
alice memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ALICE_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.alice/mem0_qdrant"}}
  }
}
```
...
```bash
alice memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ALICE_HOME/mem0.json`).
