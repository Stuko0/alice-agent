...
Resolution order for config file:
  1. $ALICE_HOME/honcho.json  (instance-local, enables isolated Alice instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Alice profile."""
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $ALICE_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.alice/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_alice_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Alice system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.alice
    ...
    def from_global_config(
        ...
        Resolution: $ALICE_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Alice profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.alice block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Alice session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Alice session title (from /title command; non-per-session)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'alice honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        # the "go run alice honcho setup" hint they used to.
        try:
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `alice honcho setup` to configure)."
            )
