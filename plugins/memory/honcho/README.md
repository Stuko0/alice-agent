...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/alice>
...
```bash
alice memory setup honcho   # configure Honcho directly (works on a fresh install)
alice memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
alice config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.alice/.env
```

> `alice honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `alice memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ALICE_HOME/honcho.json` | Profile-local (isolated Alice instances) |
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `alice honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Alice session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`alice-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`alice`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-alice-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening alice in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Alice session ID (timestamp + hex). Every `alice` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Alice profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `alice-agent`) |
...
|---------|-------------|
| `alice memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `alice honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `alice memory setup`) |
| `alice honcho status` | Show resolved config for active profile |
| `alice honcho enable` / `disable` | Toggle Honcho for active profile |
| `alice honcho mode <mode>` | Change recall or observation mode |
| `alice honcho peer --user <name>` | Update user peer name |
| `alice honcho peer --ai <name>` | Update AI peer name |
| `alice honcho tokens --context <N>` | Set context token budget |
| `alice honcho tokens --dialectic <N>` | Set dialectic max chars |
| `alice honcho map <name>` | Map current directory to a session name |
| `alice honcho sync` | Create host blocks for all Alice profiles |
