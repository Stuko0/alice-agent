...
``alice://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Alice OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "alice-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``alice-desktop`` / ``alice-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Alice.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local ALICE_HOME override can't reach.
