...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: alice)
...
Or via $ALICE_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Alice can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ALICE_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_alice_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "alice": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "alice"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Alice config."""
    profile = config.get("profile", "alice")
    return str(profile or "alice")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Alice profile name (from agent_identity)
      {workspace} — Alice workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``alice-{user}`` with no user becomes ``alice``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Alice Agent and the User"
    ...
    def post_setup(self, lydia_home: str, config: dict) -> None:
        ...
        from alice_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "alice")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "alice"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: alice-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Alice Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "alice"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "alice"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "alice", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "alice")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Alice Agent and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Alice as a non-root user, or switch "
                    "to cloud / local_external mode via 'alice memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Alice get sluggish. (issue #13125)
                try:
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Alice created/used that client on the shared async loop,
