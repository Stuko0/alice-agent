# Alice Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: alice-achievements plugin
...
### `GET /api/plugins/alice-achievements/achievements`
...
### `POST /api/plugins/alice-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/alice-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/alice-achievements/overview`
...
- Persist plugin data under:
  - `~/.alice/plugins/alice-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.alice/plugins/alice-achievements/scan_checkpoint.json`
...
- `~/.alice/plugins/alice-achievements/`
