#!/usr/bin/env python3
"""
Alice Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Alice Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills alice-agent-dev,github-auth
    python cli.py --list-tools             # List available tools and exit
"""

# IMPORTANT: alice_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See alice_bootstrap.py for full rationale.
try:
    import alice_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when alice_bootstrap isn't registered in the venv
    # yet — happens during partial ``alice update`` where git-reset landed
...
os.environ["ALICE_QUIET"] = "1"  # Our own modules
...
from alice_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from alice_cli.cli_commands_mixin import CLICommandsMixin
from alice_cli.fallback_config import get_fallback_chain
...
try:
    from alice_cli.pt_input_extras import (
...
from alice_cli.banner import _format_context_length, format_banner_version_label
...
from alice_cli.browser_connect import (
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.alice/.
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("ALICE_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from alice_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.alice/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)

    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If ALICE_IGNORE_USER_CONFIG=1 is set (via ``alice chat --ignore-user-config``),
    the user config at ``~/.alice/config.yaml`` is skipped entirely and only the
    ...
    ignore_user_config = os.environ.get("ALICE_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Alice, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Alice - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see alice_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from alice_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and AliceCLI.__init__ checks "default" first.
                    if (
    ...
    from alice_cli.config import _expand_env_vars
    ...
    # alice_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `alice config`/`doctor`/guards (which use load_config) honor it. The
    ...
    # Normalize config key: the new config system (alice_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && alice` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _ALICE_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_ALICE_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["ALICE_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.alice/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from alice_logging import setup_logging
...
try:
    from alice_cli.config import print_config_warnings
...
try:
    from alice_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``alice --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from alice_cli.mcp_startup import wait_for_mcp_discovery
...
from alice_cli.banner import build_welcome_banner
from alice_cli.commands import SlashCommandAutoSuggest, SlashCommandCompleter
...
from alice_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("ALICE_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("ALICE_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _notify_session_finalize(
    *,
    ...
    try:
        from alice_cli.plugins import invoke_hook as _invoke_hook
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.alice/alice-agent`` clone is updated only by ``alice update``,
...
def _setup_worktree(
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run alice -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"alice-{short_id}"
    branch_name = f"alice/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`alice_cli.config.load_config` (the authoritative loader that
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`alice_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``alice/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("alice-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``alice/alice-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``alice -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("alice/alice-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. ALICE_LIGHT / ALICE_TUI_LIGHT env (true/false) — explicit override
#   2. ALICE_TUI_THEME=light|dark — explicit theme
#   3. ALICE_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Alice assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("ALICE_LIGHT", "ALICE_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("ALICE_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("ALICE_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from alice_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_alice_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._alice_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Alice output it's common to display cron schedules verbatim.
    #
...
from alice_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_alice_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_alice_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._alice_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_alice_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._alice_bp_start = now
        ...
        _vt100_mod._alice_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``AliceCLI._busy_command()``.
        """
        yield self


# ASCII Art - ALICE-AGENT logo (full width, single line - requires ~95 char terminal)
...
[#7f6c96]███████╗   ██║   ██████╔╝██║██║  ██║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
[#7f6c96]╚══════╝   ╚═╝   ╚═════╝ ╚═╝╚═╝  ╚═╝      ╚═╝  ╚═╝ ╚═════╝ ╚══════╝╚═╝  ╚═══╝   ╚═╝[/]"""
# ASCII Art - Alice Gothic L (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "🌹 Alice - AI Agent Framework"
        tiny_line = "🌹 Alice"
    else:
        agent_name = (
            _skin.get_branding("agent_name", "Alice Agent")
            if _skin
            else "Alice Agent"
        )
    ...
    if os.environ.get("ALICE_FAST_STARTUP_BANNER") == "1":
        from alice_cli import __release_date__ as _release_date
        from alice_cli import __version__ as _version

        version_line = f"Alice Agent v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from alice_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.alice/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.alice/config.yaml on first use)
...
class AliceCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Alice Agent.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: ALICE_MAX_TOKENS)
        _env_mt = os.environ.get("ALICE_MAX_TOKENS")
        if _env_mt:
            try:
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from alice_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("ALICE_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("ALICE_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("ALICE_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `alice chat --ignore-rules` in alice_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("ALICE_IGNORE_RULES") == "1"
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `alice update` to rebuild the venv) to restore persistence."
                )
            except Exception:
        ...
        # it's shared across all Alice processes for this ALICE_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.alice/checkpoints/.  Opt-in via
        ...
        self._history_file = _alice_home_webhook / ".alice_history"
    ...
    def _claim_active_session(
        ...
        if self._active_session_lease is not None:
            return True
        try:
            from alice_cli.active_sessions import try_acquire_active_session
    ...
    def _recover_terminal_after_interrupt(self) -> None:
        ...
        """
        try:
            from alice_cli.curses_ui import flush_stdin
    ...
    def _pet_resolve_config(self) -> None:
        ...
        switch made via ``/pet`` or ``alice pets`` without a restart, mirroring
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from alice_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"🌹 {self.model if getattr(self, 'model', None) else 'Alice'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from alice_cli.model_normalize import (
        ...
        if self._model_is_default:
            ...
            try:
                from alice_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                ...
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "🌹 Alice")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - AliceCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``alice doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``alice doctor`` so the banner stays
        small.
        """
        try:
            from alice_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Alice needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Alice LLM (not agentic)
        from alice_cli.model_switch import is___PROT_NOUS_ALICE___non_agentic

        model_name = getattr(self, "model", "") or ""
        if is___PROT_NOUS_ALICE___non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Stuko __PROT_ALICE_MODEL__ & 4 models are NOT agentic and are not "
                "designed for use with Alice Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.alice/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from alice_cli.clipboard import save_clipboard_image

        img_dir = get_alice_home() / "images"
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'alice setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from alice_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from alice_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Alice!{_RST}")
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from alice_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(
        ...
        from alice_cli.main import _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            print(f"\n  [Alice #{visible_index}]{_ts_suffix(msg)}")
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``alice sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `alice sessions list` (gemini-cli#27770 port).
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("ALICE_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.alice/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``alice --resume <id>``
        ...
        saved_dir = get_alice_home() / "sessions" / "saved"
        try:
        ...
        try:
            ...
            if self.session_id:
                print(
                    f"       Resume the live session with: alice --resume {self.session_id}"
                )
        except Exception as e:
    ...
    def _run_curses_picker(
        ...
        from alice_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        if not getattr(result, "success", False):
            return True
        try:
            from alice_cli.model_cost_guard import expensive_model_warning
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Alice session."),
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        if self.agent is not None:
            try:
                from alice_cli.context_switch_guard import (
        ...
        try:
            from alice_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `alice model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from alice_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from alice_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        # clobber disk config.
        from alice_cli.inventory import build_models_payload, load_picker_context
    ...
    def _should_handle_model_command_inline(
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from alice_cli.commands import resolve_command
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_alice_home()}/config.yaml")
            print()
            ...
            print(
                f"    2. Or configure settings in {display_alice_home()}/config.yaml"
            )
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in alice_cli/commands.py instead of touching every dispatch site.
        from alice_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from alice_cli.tips import get_random_tip
            ...
            if len(parts) > 1:
                ...
                if raw_title:
                    if self._session_db:
                        ...
                        from alice_state import format_session_db_unavailable
            ...
            from alice_cli.main import _print_version_info
            ...
            from alice_cli.config import reload_env
            ...
            try:
                # Discover from disk (bundled + user), matching `alice plugins
                ...
                from alice_cli.plugins_cmd import (
                ...
                # catalog lives behind `alice plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: alice plugins install owner/repo")
                    print(
                        f"  Or drop a plugin directory into {display_alice_home()}/plugins/"
                    )
                    if bundled_count:
                        print(
                            f"  ({bundled_count} bundled plugins available — see: alice plugins list)"
                        )
                else:
                    ...
                    loaded: dict = {}
                    try:
                        from alice_cli.plugins import get_plugin_manager
                    ...
                    if bundled_count:
                        print(
                            f"  (+{bundled_count} bundled — see: alice plugins list)"
                        )
                    print("  Enable/disable: alice plugins enable/disable <name>")
            except Exception as e:
            ...
            from alice_cli.moa_config import (
                moa_usage,
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from alice_cli.commands import COMMANDS
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        try:
            from alice_cli.goals import gather_background_processes as _gather_bg
    ...
    def _toggle_verbose(self):
        ...
        from alice_cli.colors import Colors as _Colors
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``ALICE_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``ALICE_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from alice_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by alice_logging not
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors ALICE_DEV_CREDITS_FIXTURE
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            ...
            print("  Run `alice portal` to log in, then /credits.")
            return
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `alice portal` to log in, then /billing.")
            return
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        from alice_cli.nous_billing import (
            BillingError,
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from alice_cli.nous_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `alice portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
        ...
        if choice != "yes":
            ...
            return
        try:
            from alice_cli.auth import step_up_nous_billing_scope
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from alice_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.alice/skills/ and queue a note for the
        next user turn.
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(
                os.path.join(tempfile.gettempdir(), "alice_voice"), exist_ok=True
            )
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard ALICE_HOME, so no -p needed.
            try:
            ...
            print(f"  alice --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f'  alice -c "{session_title}"{profile_flag}')
            print()
            ...
            try:
                from alice_cli.skin_engine import get_active_goodbye
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from alice_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding(
                "welcome",
                "Welcome to Alice Agent! Type your message or /help for commands.",
            )
        ...
        try:
            from alice_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            _redact_raw = os.getenv("ALICE_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(ALICE_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Alice migration (especially migrations done by
        ...
        try:
            ...
            if (
                ...
                try:
                    from alice_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in alice_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from alice_cli.model_switch import resolve_persist_behavior
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Alice binding. See issue #22379.
                """
                event.current_buffer.insert_text("\n")
        ...
        def handle_ctrl_z(event):
            ...
            agent_name = get_active_skin().get_branding("agent_name", "Alice Agent")
        ...
        # Dynamic prompt: shows Alice symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Alice needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(("class:clarify-title", "Alice needs your input"))
            lines.append((
                "class:clarify-border",
                " "
                + ("─" * max(0, box_width - len("Alice needs your input") - 3))
                + "╮\n",
            ))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = AliceCLI._compute_model_picker_viewport(
                selected,
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_alice_osd_patched", False):
                ...
                _pt_renderer._alice_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``ALICE_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(
                    ...
                    try:
                        _grace = float(os.getenv("ALICE_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the alice process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: alice setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, "_session_db") and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `alice sessions list`
...
def _run_kanban_goal_loop_q(cli: "AliceCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``ALICE_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("ALICE_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from alice_cli import kanban_db as _kb
    from alice_cli.goals import DEFAULT_MAX_TURNS as _DEF_TURNS
    from alice_cli.goals import run_kanban_goal_loop as _run_loop
...
def main(
    ...
    """
    Alice Agent CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting Alice Gateway (messaging platforms)...")
    ...
    # Default to alice-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Alice): with no explicit --toolsets, collapse
        ...
        if missing_skills:
            ...
            if loaded_skills:
                logger.warning(
                    ...
                    "Continuing with: %s. "
                    "List available skills with `alice skills list`.",
                    missing_display,
                    ", ".join(loaded_skills),
                )
            else:
    ...
    # mode registers its own inside AliceCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  ALICE_SIGTERM_GRACE overrides the 1.5 s
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``alice chat -q "work kanban task <id>"``;
            ...
            _kanban_task_id = os.environ.get("ALICE_KANBAN_TASK", "").strip()
            if _kanban_task_id:
                try:
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Alice" box, no tool-gen
                        ...
                        if os.environ.get("ALICE_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("ALICE_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from alice_cli.kanban_db import (
                ...
                # Single-query mode (`alice chat -q "…"`): skip the welcome
