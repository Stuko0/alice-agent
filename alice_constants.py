"""Shared constants for Alice Agent.
...
def _get_platform_default_alice_home() -> Path:
    ...
    new installs.  If ``~/.hermes`` exists with content but ``~/.alice`` does
...
def _migrate_legacy_home(legacy: Path, target: Path) -> None:
    """Automatically migrate the legacy ~/.hermes directory to ~/.alice.
    ...
    try:
        ...
        sys.stderr.write(
            f"[Alice] Migrated legacy home {legacy} → {target}\n"
        )
        sys.stderr.flush()
    except OSError as exc:
        sys.stderr.write(
            f"[Alice] WARNING: Could not migrate {legacy} → {target}: {exc}\n"
            f"[Alice] Please rename the directory manually.\n"
        )
        sys.stderr.flush()


def get_alice_home() -> Path:
    """Return the Alice home directory (default: platform-native path).

    Reads ALICE_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``ALICE_HOME`` is unset but an ``active_profile`` file indicates
    ...
    if not _profile_fallback_warned:
        ...
        if active and active != "default":
            ...
            msg = (
                f"[ALICE_HOME fallback] ALICE_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass ALICE_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
...
def get_default_alice_root() -> Path:
    ...
    (``~/.alice`` on POSIX, ``%LOCALAPPDATA%\\alice`` on native Windows).

    In Docker or custom deployments where ``ALICE_HOME`` points outside
    ``~/.alice`` (e.g. ``/opt/data``), returns ``ALICE_HOME`` directly
    — that IS the root.

    In profile mode where ``ALICE_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.alice/profiles/coder``) and Docker
    ...
    try:
        ...
        # ALICE_HOME is under ~/.alice (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — ALICE_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Alice is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_alice_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_alice_home() / "optional-mcps"
...
def get_alice_dir(new_subpath: str, old_name: str) -> Path:
    ...
    Args:
        new_subpath: Preferred path relative to ALICE_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to ALICE_HOME (e.g. ``"image_cache"``).
...
def iter_alice_node_dirs(home: Path | None = None) -> list[Path]:
    ...
    into ``%LOCALAPPDATA%\\alice\\node``. POSIX installs use
    ``$ALICE_HOME/node/bin``. Include both shapes on every platform so mixed
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%ALICE_HOME%\\node`` on Windows."""
    import re
    import tempfile
...
def heal_alice_managed_node() -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ALICE_HOME": str(get_alice_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Alice-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Alice-managed installs.

    This is for Alice-owned subprocesses that should not be broken by a bad,
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``alice update`` — leaving a **dangling symlink** that ``which`` still
...
def display_alice_home() -> str:
    """Return a user-friendly display string for the current ALICE_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.alice`` (or ``~/.alice`` for legacy installs)
        profile:  ``~/.alice/profiles/coder``
        custom:   ``/opt/alice-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    a path.  For code that needs a real ``Path``, use
    :func:`get_alice_home` instead.
    """
    home = get_alice_home()
    try:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Alice profile HOME.

    ``ALICE_HOME`` scopes Alice state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={ALICE_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{ALICE_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{ALICE_HOME}/home`` when it exists, preserving the
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under ALICE_HOME.

    Replaces the ``get_alice_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, alice_logging.py, alice_time.py, etc.).
    """
    return get_alice_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under ALICE_HOME."""
    return get_alice_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under ALICE_HOME."""
