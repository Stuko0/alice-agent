...
# Alice Agent ☤
<p align="center">
  <a href="https://alice-agent.stuko.dev/">Alice Agent</a> | <a href="https://alice-agent.stuko.dev/">Alice Desktop</a>
</p>
<p align="center">
...
El instalador se encarga de todo: uv, Python 3.11, Node.js, ripgrep, ffmpeg, **y un Git Bash portátil** (MinGit, descomprimido en `%LOCALAPPDATA%\alice\git` — no requiere administrador, completamente aislado de cualquier instalación de Git del sistema). Alice usa este Git Bash incluido para ejecutar comandos de shell.
...
> **Android / Termux:** La ruta manual probada está documentada en la [guía de Termux](https://alice-agent.stuko.dev/docs/getting-started/termux). En Termux, Alice instala el extra `.[termux]` curado porque el extra completo `.[all]` actualmente incluye dependencias de voz incompatibles con Android.
>
> **Windows:** Windows nativo es totalmente compatible — el comando de PowerShell de arriba instala todo. Si prefieres usar WSL2, el comando de Linux también funciona allí. La instalación nativa de Windows se encuentra en `%LOCALAPPDATA%\alice`; WSL2 instala en `~/.alice` como en Linux.

Después de la instalación:

```bash
source ~/.bashrc    # recargar shell (o: source ~/.zshrc)
alice              # ¡empieza a chatear!
```
...
```bash
alice              # CLI interactiva — inicia una conversación
alice model        # Elige tu proveedor y modelo LLM
alice native        # Configura qué herramientas están habilitadas
alice config set   # Establece valores de configuración individuales
alice gateway      # Inicia el gateway de mensajería (Telegram, Discord, etc.)
alice setup        # Ejecuta el asistente de configuración completo
alice claw migrate # Migra desde OpenClaw (si vienes de OpenClaw)
alice update       # Actualiza a la última versión
alice doctor       # Diagnostica cualquier problema
```

📖 **[Documentación completa →](https://alice-agent.stuko.dev/docs/)**
...
Alice tiene dos puntos de entrada: inicia la interfaz de terminal con `alice`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `alice`                                      | Ejecuta `alice gateway setup` + `alice gateway start`, luego envía un mensaje al bot |
...
Para las listas de comandos completas, consulta la [guía de CLI](https://alice-agent.stuko.dev/docs/user-guide/cli) y la [guía del Gateway de Mensajería](https://alice-agent.stuko.dev/docs/user-guide/messaging).

---

## Documentación

Toda la documentación está en **[alice-agent.stuko.dev/docs](https://alice-agent.stuko.dev/docs/)**:

| Sección                                                                                             | Contenido                                                    |
| --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| [Inicio rápido](https://alice-agent.stuko.dev/docs/getting-started/quickstart)              | Instalar → configurar → primera conversación en 2 minutos   |
| [Uso de CLI](https://alice-agent.stuko.dev/docs/user-guide/cli)                             | Comandos, atajos de teclado, personalidades, sesiones        |
| [Configuración](https://alice-agent.stuko.dev/docs/user-guide/configuration)               | Archivo de configuración, proveedores, modelos, todas las opciones |
| [Gateway de Mensajería](https://alice-agent.stuko.dev/docs/user-guide/messaging)           | Telegram, Discord, Slack, WhatsApp, Signal, Home Assistant   |
| [Seguridad](https://alice-agent.stuko.dev/docs/user-guide/security)                        | Aprobación de comandos, emparejamiento por DM, aislamiento en contenedor |
| [Herramientas y Toolsets](https://alice-agent.stuko.dev/docs/user-guide/features/tools)   | Más de 40 herramientas, sistema de toolsets, backends de terminal |
| [Sistema de Habilidades](https://alice-agent.stuko.dev/docs/user-guide/features/skills)   | Memoria procedimental, Skills Hub, creación de habilidades   |
| [Memoria](https://alice-agent.stuko.dev/docs/user-guide/features/memory)                   | Memoria persistente, perfiles de usuario, mejores prácticas  |
| [Integración MCP](https://alice-agent.stuko.dev/docs/user-guide/features/mcp)              | Conecta cualquier servidor MCP para capacidades extendidas   |
| [Programación Cron](https://alice-agent.stuko.dev/docs/user-guide/features/cron)           | Tareas programadas con entrega a plataforma                  |
| [Archivos de Contexto](https://alice-agent.stuko.dev/docs/user-guide/features/context-files) | Contexto de proyecto que da forma a cada conversación      |
| [Arquitectura](https://alice-agent.stuko.dev/docs/developer-guide/architecture)            | Estructura del proyecto, bucle del agente, clases principales |
| [Contribuir](https://alice-agent.stuko.dev/docs/developer-guide/contributing)              | Configuración de desarrollo, proceso de PR, estilo de código |
| [Referencia de CLI](https://alice-agent.stuko.dev/docs/reference/cli-commands)             | Todos los comandos y flags                                   |
| [Variables de Entorno](https://alice-agent.stuko.dev/docs/reference/environment-variables) | Referencia completa de variables de entorno                  |

---

## Migración desde OpenClaw

Si vienes de OpenClaw, Alice puede importar automáticamente tu configuración, memorias, habilidades y claves API.

**Durante la configuración inicial:** El asistente de configuración (`alice setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
alice claw migrate              # Migración interactiva (preset completo)
alice claw migrate --dry-run    # Vista previa de qué se migraría
alice claw migrate --preset user-data   # Migrar sin secretos
alice claw migrate --overwrite  # Sobreescribir conflictos existentes
```
...
- **Habilidades** — habilidades creadas por el usuario → `~/.alice/skills/openclaw-imports/`
...
Consulta `alice claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
