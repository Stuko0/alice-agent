# nix/nixosModules.nix — NixOS module for alice-agent
#
# Two modes:
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.alice-agent;
    effectivePackage =
    ...
    alice-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-alice-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "alice-config-attrs";
      description = "Alice YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "alice-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "alice-documents" { } (
      ''
        mkdir -p $out
    ...
    # alice user + sudo on first boot (writable layer persists), then
    ...
    containerEntrypoint = pkgs.writeShellScript "alice-container-entrypoint" ''
      ...
      if [ ! -f /var/lib/alice-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/alice-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/alice ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/alice
    ...
    # Default: /var/lib/alice/workspace → /data/workspace.
  ...
  in {
    options.services.alice-agent = with lib; {
      enable = mkEnableOption "Alice Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = alice-agent;
        description = "The alice-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Alice config (attrset). Deep-merged across module
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `alice gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the alice user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the alice plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Alice discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in alice-agent's
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.alice symlink to the service
            stateDir. These users are automatically added to the alice group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.alice-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.alice-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the alice user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.alice-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed alice
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure alice is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.alice        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alice/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alice/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alice/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alice/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alice/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."alice-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.alice
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.alice ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.alice ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.alice -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.alice/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.alice/$_subdir"
            chmod 2770 "${cfg.stateDir}/.alice/$_subdir"
            find "${cfg.stateDir}/.alice/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # alice-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.alice/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.alice/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alice/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.alice/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.alice/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alice/.managed
          chmod 0644 ${cfg.stateDir}/.alice/.managed
    ...
    ALICE_CONTAINER_MODE_EOF
          ...
          '' else ''
            rm -f ${cfg.stateDir}/.alice/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.alice";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.alice" ]; then
                  rm -f "${symlinkPath}"
                  echo "alice-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.alice -> stateDir/.alice for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.alice";
                target = "${cfg.stateDir}/.alice";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "alice-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.alice/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.alice/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.alice/auth.json
              fi
            ''}
          ''}
    ...
    ALICE_NIX_ENV_EOF
        ...
        find ${cfg.stateDir}/.alice/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.alice/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alice/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.alice-agent = {
          description = "Alice Agent Gateway";
          ...
          serviceConfig = {
            ...
            # so interactive users in the alice group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.alice-agent = {
          description = "Alice Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/alice gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
