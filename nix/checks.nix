...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "alice-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "alice-build-package" { } ''
          echo "PASS: package built at ${alice-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "alice-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "alice-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${alice-agent}/bin/alice || (echo "FAIL: alice binary missing"; exit 1)
          test -x ${alice-agent}/bin/alice-agent || (echo "FAIL: alice-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${alice-agent}/bin/alice version 2>&1 | grep -qi "alice" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "alice-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in alice alice-agent alice-acp; do
            test -x ${alice-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "alice-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking alice --help ==="
          ${alice-agent}/bin/alice --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${alice-agent}/bin/alice --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "alice-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${alice-agent}/share/alice-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${alice-agent}/share/alice-agent/skills -name "SKILL.md" | wc -l)
        ...
        bundled-plugins = pkgs.runCommand "alice-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${alice-agent}/share/alice-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${alice-agent}/share/alice-agent/plugins/platforms/irc/plugin.yaml || \
        ...
        bundled-locales = pkgs.runCommand "alice-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${alice-agent}/share/alice-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${alice-agent}/share/alice-agent/locales -name "*.yaml" | wc -l)
        ...
        bundled-tui = pkgs.runCommand "alice-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${alice-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${alice-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
        ...
        in pkgs.runCommand "alice-extra-python-packages" { } ''
          ...
          if grep -q "PYTHONPATH" ${alice-agent}/bin/alice; then
        ...
        in pkgs.runCommand "alice-config-roundtrip" {
