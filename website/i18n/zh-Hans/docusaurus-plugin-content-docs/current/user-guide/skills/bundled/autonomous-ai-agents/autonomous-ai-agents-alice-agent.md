---
title: "Alice Agent — 配置、扩展或贡献 Alice Agent"
sidebar_label: "Alice Agent"
description: "配置、扩展或贡献 Alice Agent"
---
...
|---|---|
| 来源 | 内置（默认安装） |
| 路径 | `skills/autonomous-ai-agents/alice-agent` |
| 版本 | `2.1.0` |
| 作者 | Alice Agent + Teknium |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `alice`, `setup`, `configuration`, `multi-agent`, `spawning`, `cli`, `gateway`, `development` |
...
:::info
以下是 Alice 在触发此 skill 时加载的完整 skill 定义。这是 agent 在 skill 激活时看到的指令内容。
:::
...
- **通过 skill 自我提升** — Alice 通过将可复用流程保存为 skill 来从经验中学习。当它解决复杂问题、发现工作流或被纠正时，可以将该知识持久化为 skill 文档，加载到未来的会话中。skill 随时间积累，使 agent 在你的特定任务和环境中表现越来越好。
...
- **Profiles（配置文件）** — 运行多个独立的 Alice 实例，各自拥有隔离的配置、会话、skill 和记忆。
- **可扩展** — 插件、MCP 服务器、自定义工具、webhook 触发器、cron 调度以及完整的 Python 生态系统。

人们将 Alice 用于软件开发、研究、系统管理、数据分析、内容创作、家庭自动化，以及任何受益于具有持久上下文和完整系统访问权限的 AI agent 的场景。
...
alice auth                 交互式凭据管理器
alice auth add PROVIDER    添加 OAuth 或 API key 凭据（例如 nous、openai-codex、qwen-oauth）
alice auth list            列出已存储的凭据
alice auth remove PROVIDER 移除已存储的凭据
...
~/.alice/sessions/         Session transcripts
...
Profiles 使用 `~/.alice/profiles/<name>/`，布局相同。

### 配置节

使用 `alice config edit` 或 `alice config set section.key value` 编辑。
...
支持 20+ 个提供商。通过 `alice model` 或 `alice setup` 设置。
...
通过 `alice native`（交互式）或 `alice native enable/disable NAME` 启用/禁用。
...
常见的"为什么 Alice 对我的输出/工具调用/命令做了 X？"开关——以及更改它们的确切命令。其中大多数需要新会话（聊天中的 `/reset`，或启动新的 `alice` 调用），因为它们在启动时只读取一次。

### 工具输出中的密钥脱敏

密钥脱敏**默认关闭** — 工具输出（终端 stdout、`read_file`、网页内容、子 agent 摘要等）不经修改直接传递。如果用户希望 Alice 在 API key、token 和密钥进入对话上下文和日志之前自动屏蔽它们：
...
```bash
alice config set privacy.redact_pii true    # 启用
alice config set privacy.redact_pii false   # 禁用（默认）
```

### 命令审批提示

默认情况下（`approvals.mode: manual`），Alice 在运行被标记为破坏性的 shell 命令（`rm -rf`、`git reset --hard` 等）之前会提示用户。模式如下：
...
```bash
alice config set approvals.mode smart       # 推荐的折中方案
alice config set approvals.mode off         # 绕过一切（不推荐）
```
...
某些 shell hook 集成在触发前需要明确加入允许列表。通过 `~/.alice/shell-hooks-allowlist.json` 管理——在 hook 首次尝试运行时以交互方式提示。

### 禁用 web/browser/image-gen 工具

要完全阻止模型访问网络或媒体工具，打开 `alice native` 并按平台切换。在下次会话（`/reset`）后生效。参见上方的工具与 Skill 部分。
...
将额外的 Alice 进程作为完全独立的子进程运行——拥有独立的会话、工具和环境。

### 何时使用此方式 vs delegate_task

| | `delegate_task` | 生成 `alice` 进程 |
|-|-----------------|--------------------------|
...
Alice 使用 prompt_toolkit，需要真实终端。使用 tmux 进行交互式生成：
...
- **fire-and-forget 使用 `alice chat -q`** — 无需 PTY
...
持久化调度器——`cron/jobs.py` + `cron/scheduler.py`。通过 `cronjob` 工具、`alice cron` CLI（`list`、`add`、`edit`、`pause`、`resume`、`run`、`remove`）或 `/cron` 斜杠命令驱动。
...
- **CLI：** `alice curator <verb>` — `status`、`run`、`pause`、`resume`、`pin`、`unpin`、`archive`、`restore`、`prune`、`backup`、`rollback`。
...
Alice 在 Windows 上原生运行（PowerShell、cmd、Windows Terminal、git-bash mintty、VS Code 集成终端）。大多数功能开箱即用，但 Win32 和 POSIX 之间有一些差异曾给我们带来麻烦——遇到新问题时请在此记录，以免下一个人（或下一个会话）重新踩坑。
...
**首次运行时 HTTP 400 "No models provided"。** `config.yaml` 保存时带有 UTF-8 BOM（Windows 应用写入时常见）。重新保存为不带 BOM 的 UTF-8。`alice config edit` 写入时不带 BOM；手动在记事本中编辑是常见原因。

### `execute_code` / 沙箱

**WinError 10106**（"无法加载或初始化请求的服务提供商"）来自沙箱子进程——它无法创建 `AF_INET` socket，因此回退的 loopback-TCP RPC 在 `connect()` 之前失败。根本原因通常**不是**损坏的 Winsock LSP；而是 Alice 自身的环境清理器从子进程环境中删除了 `SYSTEMROOT` / `WINDIR` / `COMSPEC`。Python 的 `socket` 模块需要 `SYSTEMROOT` 来定位 `mswsock.dll`。通过 `tools/code_execution_tool.py` 中的 `_WINDOWS_ESSENTIAL_ENV_VARS` 允许列表修复。如果仍然遇到此问题，在 `execute_code` 块内 echo `os.environ` 以确认 `SYSTEMROOT` 已设置。完整诊断方案见 `references/execute-code-sandbox-env-windows.md`。

### 测试/贡献

**`scripts/run_tests.sh` 在 Windows 上无法直接使用** — 它查找 POSIX venv 布局（`.venv/bin/activate`）。Alice 安装的 venv 位于 `venv/Scripts/`，也没有 pip 或 pytest（为减小安装体积而精简）。解决方案：将 `pytest + pytest-xdist + pyyaml` 安装到系统 Python 3.11 用户站点，然后设置 `PYTHONPATH` 直接调用 pytest：
...
**正斜杠几乎在所有地方都有效。** `C:/Users/...` 被每个 Alice 工具和大多数 Windows API 接受。在代码和日志中优先使用正斜杠——避免在 bash 中转义反斜杠。
...
### 工具不可用
1. `alice native` — 检查 toolset 是否为你的平台启用
2. 某些工具需要环境变量（检查 `.env`）
3. 启用工具后执行 `/reset`

### 模型/提供商问题
1. `alice doctor` — 检查配置和依赖
2. `alice auth` — 重新认证 OAuth 提供商（或 `alice auth add <provider>`）
3. 检查 `.env` 中是否有正确的 API key
4. **Copilot 403**：`gh auth login` 的 token **不适用于** Copilot API。必须通过 `alice model` → GitHub Copilot 使用 Copilot 专用 OAuth 设备码流程。
...
### Skill 未显示
1. `alice skills list` — 验证已安装
2. `alice skills config` — 检查平台启用状态
3. 显式加载：`/skill name` 或 `alice -s name`
...
- **Gateway 崩溃循环**：重置失败状态：`systemctl --user reset-failed alice-gateway`
...
配置：`~/.alice/config.yaml`（设置）、`~/.alice/.env`（API key）。
...
所有处理器必须返回 JSON 字符串。路径使用 `get_alice_home()`，永远不要硬编码 `~/.alice`。

### 添加斜杠命令

1. 在 `alice_cli/commands.py` 的 `COMMAND_REGISTRY` 中添加 `CommandDef`
...
- POSIX 文件模式（0o600 等）→ `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")`（参见 `tests/alice_cli/test_auth_toctou_file_modes.py`）
- `signal.SIGALRM` → 仅 Unix（参见 `tests/conftest.py::_enforce_test_timeout`）
...
- 所有路径使用 `alice_constants` 中的 `get_alice_home()`（profile 安全）
