...
sidebar_position: 7
title: "Docker"
description: "在 Docker 中运行 Alice Agent 以及将 Docker 用作终端后端"
---

# Alice Agent — Docker

Docker 与 Alice Agent 的交集有两种截然不同的方式：

1. **在 Docker 中运行 Alice** — agent 本身在容器内运行（本页的主要内容）
2. **Docker 作为终端后端** — agent 在宿主机上运行，但将每条命令在单个持久化 Docker 沙箱容器中执行，该容器在工具调用、`/new` 和子 agent 之间保持存活，直至 Alice 进程结束（参见 [配置 → Docker 后端](./configuration.md#docker-backend)）

本页介绍选项 1。容器将所有用户数据（配置、API 密钥、会话、技能、记忆）存储在从宿主机挂载于 `/opt/data` 的单个目录中。镜像本身是无状态的，可通过拉取新版本进行升级而不会丢失任何配置。

## 快速开始

如果这是你第一次运行 Alice Agent，请在宿主机上创建一个数据目录，并以交互方式启动容器以运行设置向导：
...
这将进入设置向导，向导会提示你输入 API 密钥并将其写入 `~/.alice/.env`。你只需执行一次。强烈建议此时为 gateway 配置一个聊天系统。
...
`/opt/data` 卷是所有 Alice 状态的唯一数据来源。它映射到宿主机的 `~/.alice/` 目录，包含：

| 路径 | 内容 |
|------|----------|
| `.env` | API 密钥和机密 |
| `config.yaml` | 所有 Alice 配置 |
| `SOUL.md` | Agent 个性/身份 |
| `sessions/` | 对话历史 |
| `memories/` | 持久化记忆存储 |
| `skills/` | 已安装的技能 |
| `home/` | Alice 工具子进程（`git`、`ssh`、`gh`、`npm` 及 skill CLI）的 per-profile HOME |
| `cron/` | 定时任务定义 |
| `hooks/` | 事件 hook |
| `logs/` | 运行时日志 |
| `skins/` | 自定义 CLI 皮肤 |
...
在托管/发布的 Docker 镜像中，`/opt/alice` 是安装好的应用树。它由 root 拥有，并且对运行时的 `alice` 用户只读，因此 agent 回合、gateway 会话、dashboard 操作以及普通的 `docker exec alice alice ...` 命令都不能原地修改核心源码、打包的 `.venv`、`node_modules` 或 TUI bundle。
...
:::warning
切勿同时对同一数据目录运行两个 Alice **gateway** 容器——会话文件和记忆存储不支持并发写入。
:::

## 多 profile 支持

Alice 支持[多个 profile](../reference/profile-commands.md)——独立的 `~/.alice/` 子目录，让你可以从单个安装运行独立的 agent（不同的 SOUL、skills、memory、sessions、credentials）。**在官方 Docker 镜像内，s6 监管树把每个 profile 当作一等受监管服务**，因此推荐部署方式是：**一个容器承载多个 profile**。

每个通过 `alice profile create <name>` 创建的 profile 都会获得：

- 一个专用的 s6 服务槽位 `/run/service/gateway-<name>/`，运行时动态注册，无需重建镜像。
- 崩溃后的自动重启，由 `s6-supervise` 管理退避。
- 每个 profile 独立的轮转日志：`${ALICE_HOME}/logs/gateways/<name>/current`。
...
- Python 3 及所有 Alice 依赖（`uv pip install -e ".[all]"`）
...
1. 以 root 身份运行 `/etc/cont-init.d/01-alice-setup`（即 `docker/stage2-hook.sh`）：可选的 UID/GID 重映射、修复卷所有权、首次启动时初始化 `.env` / `config.yaml` / `SOUL.md`、同步内置技能。
2. 运行 `/etc/cont-init.d/02-reconcile-profiles`（即 `alice_cli.container_boot`）：遍历 `$ALICE_HOME/profiles/<name>/`，在 `/run/service/gateway-<profile>/` 下重建各 profile 的 gateway s6 服务槽，并仅自动启动上次记录状态为 `running` 的 profile（参见 [Per-profile gateway 监管](#per-profile-gateway-supervision)）。
3. 启动静态的 `main-alice` 和 `dashboard` s6-rc 服务。
4. 将容器的 CMD 作为主程序 exec（`/opt/alice/docker/main-wrapper.sh`），根据用户传给 `docker run` 的参数进行路由：
   - 无参数 → `alice`（默认）
   - 第一个参数是 PATH 上的可执行文件（如 `sleep`、`bash`）→ 直接 exec
   - 其他情况 → `alice <args>`（子命令透传）
   主程序退出时容器退出，并使用其退出码。
...
在容器内，每个通过 `alice profile create <name>` 创建的 profile 都会自动在 `/run/service/gateway-<name>/` 注册一个受 s6 监管的 gateway 服务。你在宿主机上运行的生命周期命令在此同样适用：

```sh
alice profile create coder            # 注册 gateway-coder s6 槽
alice -p coder gateway start          # s6-svc -u  → 受监管的 gateway
alice -p coder gateway stop           # s6-svc -d  → 服务停止
alice -p coder gateway restart        # s6-svc -t  → 向 supervisor 发送 SIGTERM
alice profile delete coder            # 拆除 s6 槽
```
...
在容器内执行 `alice status` 会显示 `Manager: s6 (container supervisor)`。使用 `/command/s6-svstat /run/service/gateway-<name>` 查看原始 supervisor 状态（注意 `/command/` 仅在监管树进程的 PATH 中；从 `docker exec` 调用时请传入绝对路径）。
...
当使用 Docker 作为执行环境时（不是上述方法，而是 agent 在 Docker 沙箱内运行命令——参见 [配置 → Docker 后端](./configuration.md#docker-backend)），Alice 为所有工具调用复用单个长期运行的容器，并自动将技能目录（`~/.alice/skills/`）和技能声明的所有凭据文件以只读卷的形式绑定挂载到该容器中。技能脚本、模板和引用在沙箱内无需手动配置即可使用，由于容器在 Alice 进程的整个生命周期内持续存在，你安装的任何依赖或写入的文件都会在下次工具调用时保留。
...
对于发布到 npm 或 PyPI 的任何工具，指示 Alice 通过 `npx`（npm）或 `uvx`（Python）运行，并将该命令记入其持久记忆。如果工具需要配置文件或凭据，指示其将这些文件放在 `/opt/data` 下（如 `/opt/data/<tool>/config.yaml`）。
...
对于 npm 或 PyPI 之外的工具——`apt` 包、预构建二进制文件、镜像中未包含的语言运行时——指示 Alice 如何安装（如 `apt-get update && apt-get install -y <package>`），并告知它记住该安装命令。工具在容器剩余生命周期内持续可用，Alice 在容器重启后下次需要该工具时会重新运行安装命令。

这种方式适合安装快速且偶尔使用的工具。对于频繁使用的工具，建议采用下一种方式。

### 持久安装——构建派生镜像

当工具必须在每次容器启动时立即可用且无需重新安装延迟时，构建一个继承自 `nousresearch/alice-agent` 并在层中安装该工具的新镜像：
...
入口点脚本和 `/opt/data` 语义原样继承，本页其余内容仍然适用。拉取更新的上游 `nousresearch/alice-agent` 时记得重新构建镜像。

### 复杂工具或多服务栈——运行 sidecar 容器

对于自带服务（数据库、Web 服务器、队列、无头浏览器集群）或过于庞大而不适合放在 Alice 容器内的工具，将其作为独立容器运行在共享 Docker 网络上。Alice 通过容器名称访问 sidecar，与访问本地推理服务器的方式相同（参见 [连接本地推理服务器](#connecting-to-local-inference-servers-vllm-ollama-etc)）。
...
在 Alice 容器内，sidecar 可通过 `http://my-tool:<port>` 访问（或其提供的任何协议）。这种模式使每个服务的生命周期、资源限制和升级节奏保持独立，避免因单个工具的依赖而使 Alice 镜像臃肿。
...
在 Docker 中运行 Alice 且推理服务器（vLLM、Ollama、text-generation-inference 等）也在宿主机或另一个容器中运行时，网络配置需要额外注意。
...
然后在 `~/.alice/config.yaml` 中，使用**容器名称**作为主机名：
...
:::tip 关键点
- 使用**容器名称**（`vllm`）作为主机名——而非 `localhost` 或 `127.0.0.1`，它们指向 Alice 容器本身。
...
1. 两个容器是否在同一 Docker 网络上（`docker network inspect alice-net`）
...
```sh
docker logs --tail 50 alice          # 最近日志
docker run -it --rm nousresearch/alice-agent:latest version     # 验证版本
docker stats alice                    # 资源使用情况
```