...
`~/.alice/config.yaml`, `~/.alice/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/alice`:

```text
/etc/alice/
├── config.yaml     # managed config layer (wins over ~/.alice/config.yaml)
└── .env            # managed env layer (wins over ~/.alice/.env + shell)
```
...
|---|---|---|
| 1 | `/etc/alice/config.yaml` (managed) | `/etc/alice/.env` (managed) |
| 2 | `~/.alice/config.yaml` (user) | `~/.alice/.env` (user) |
...
```bash
alice config        # shows a header naming the managed source + the pinned keys
alice doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Alice refuses and names the source:

```bash
$ alice config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/alice/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `alice config set` / setup will not write
...
sudo tee /etc/alice/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/alice/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/alice
sudo chmod 0644 /etc/alice/config.yaml /etc/alice/.env
```

Changes take effect on the next Alice start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`alice doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Alice as `root`), managed scope is advisory.
