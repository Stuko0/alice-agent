...
sidebar_position: 7
title: "Docker"
description: "Running Alice Agent in Docker and using Docker as a terminal backend"
---

# Alice Agent — Docker

There are two distinct ways Docker intersects with Alice Agent:

1. **Running Alice IN Docker** — the agent itself runs inside a container (this page's primary focus)
2. **Docker as a terminal backend** — the agent runs on your host but executes every command inside a single, persistent Docker sandbox container that survives across tool calls, `/new`, and subagents for the life of the Alice process (see [Configuration → Docker Backend](./configuration.md#docker-backend))

This page covers option 1. The container stores all user data (config, API keys, sessions, skills, memories) in a single directory mounted from the host at `/opt/data`. The image itself is stateless and can be upgraded by pulling a new version without losing any configuration.

## Quick start

If this is your first time running Alice Agent, create a data directory on the host and start the container interactively to run the setup wizard:
...
like `-v ~/.alice:/opt/data`, `-e KEY=value`, and pasted API keys / tokens.
...
docker run -it --rm \
  -v ~/.alice:/opt/data \
  nousresearch/alice-agent setup
```

This drops you into the setup wizard, which will prompt you for your API keys and write them to `~/.alice/.env`. You only need to do this once. It is highly recommended to set up a chat system for the gateway to work with at this point.

:::tip
Inside the container, run `alice setup --portal` once — the refresh token persists in the mounted `~/.alice` volume. See [Nous Portal](/integrations/nous-portal).
:::
...
docker run -it --rm \
  -v ~/.alice:/opt/data \
  nousresearch/alice-agent
```

Or if you have already opened a terminal in your running container (via Docker Desktop for instance), just run:

```sh
/opt/alice/.venv/bin/alice
```

## Persistent volumes

The `/opt/data` volume is the single source of truth for all Alice state. It maps to your host's `~/.alice/` directory and contains:

| Path | Contents |
|------|----------|
| `.env` | API keys and secrets |
| `config.yaml` | All Alice configuration |
| `SOUL.md` | Agent personality/identity |
| `sessions/` | Conversation history |
| `memories/` | Persistent memory store |
| `skills/` | Installed skills |
| `home/` | Per-profile HOME for Alice tool subprocesses (`git`, `ssh`, `gh`, `npm`, and skill CLIs) |
...
In hosted and published Docker images, `/opt/alice` is the installed application tree. It is root-owned and read-only to the runtime `alice` user, so agent turns, gateway sessions, dashboard actions, and normal `docker exec alice alice ...` commands cannot edit the core source, bundled `.venv`, `node_modules`, or TUI bundle in place.

All mutable Alice state belongs under `/opt/data`: config, `.env`, profiles, skills, memories, sessions, logs, dashboard uploads, plugins, and other user-managed files. The image also disables runtime `.pyc` writes and Alice lazy dependency installs into `/opt/alice`; optional platform dependencies needed by the published image should be baked into the image or installed through a new image build.
...
:::warning
Never run two Alice **gateway** containers against the same data directory simultaneously — session files and memory stores are not designed for concurrent write access.
:::

## Multi-profile support

Alice supports [multiple profiles](../reference/profile-commands.md) — separate `~/.alice/` subdirectories that let you run independent agents (different SOUL, skills, memory, sessions, credentials) from a single installation. **Inside the official Docker image, the s6 supervision tree treats each profile as a first-class supervised service**, so the recommended deployment is **one container hosting all profiles**.

Each profile created with `alice profile create <name>` gets:
...
docker exec alice alice profile create work
...
docker exec alice alice -p work gateway restart
```
...
| Profile creation | `docker exec ... alice profile create <name>` (seconds) | New `docker run` invocation + port allocation + bind-mount config |
...
| Backup | One `~/.alice` directory | N directories to coordinate |
...
services:
  alice-work:
    image: nousresearch/alice-agent:latest
    container_name: alice-work
    restart: unless-stopped
    command: gateway run
    ports:
      - "8642:8642"
  ...
  alice-personal:
    image: nousresearch/alice-agent:latest
    container_name: alice-personal
    restart: unless-stopped
    command: gateway run
    ports:
      - "8643:8642"
    volumes:
      - ~/.alice-personal:/opt/data
```

The warning from [Persistent volumes](#persistent-volumes) still applies: never point two containers at the same `~/.alice` directory simultaneously. The s6 supervisor inside each container manages its own profile set; cross-container sharing of a data volume corrupts session files and memory stores.
...
- The boot reconciler's audit line shape is `<iso-timestamp> profile=<name> prior_state=<state> action=<registered|started>`, so a quick `grep profile=coder ~/.alice/logs/container-boot.log` reveals when a given profile was last restored and whether s6 auto-started it.
...
Start with `docker compose up -d` and view logs with `docker compose logs -f`. The supervised gateway's stdout is also tee'd to `${ALICE_HOME}/logs/gateways/<profile>/current` on the volume — see [Where the logs go](#where-the-logs-go) for the full routing map.

## Optional: Linux desktop audio bridge

Voice mode in Docker needs two separate things to work: Alice must be allowed to probe audio devices inside the container, and the container must be able to reach your host audio server. The setup below covers the host audio plumbing for Linux desktops that expose a PulseAudio-compatible socket, including many PipeWire setups.

:::caution
This is a Linux desktop workaround, not a general Docker Desktop feature. It is useful when you already have host audio working and want CLI voice mode inside the Alice container. If Alice still reports `Running inside Docker container -- no audio devices`, use a build that includes Docker audio probing support for `PULSE_SERVER` / `PIPEWIRE_REMOTE`.
:::
...
FROM nousresearch/alice-agent:latest
...
```sh
docker exec alice /opt/alice/.venv/bin/python -c "import sounddevice as sd; print(sd.query_devices())"
```

## Resource limits

The Alice container needs moderate resources. Recommended minimums:
...
- Python 3.13 with dependencies synced from the lockfile via `uv sync --frozen --no-install-project` for the baked extras (`all`, `messaging`, Anthropic/Bedrock/Azure identity, Hindsight, Matrix), followed by a no-dependency editable install of Alice itself.
...
4. Exec's the container's CMD as the main program (`/opt/alice/docker/main-wrapper.sh`), which routes the arguments the user passed to `docker run`:
   - no args → `alice` (the default)
   - first arg is an executable on PATH (e.g. `sleep`, `bash`) → exec it directly
   - anything else → `alice <args>` (subcommand passthrough)
...
`docker exec alice <cmd>` defaults to running as root inside the container, but the image ships a thin shim at `/opt/alice/bin/alice` (earliest on PATH) that detects root callers and transparently re-execs through `s6-setuidgid alice`. So `docker exec alice login`, `docker exec alice profile create …`, `docker exec alice setup`, etc. all write files owned by UID 10000 — i.e. readable by the supervised gateway — with no extra `--user` flag needed. Non-root callers (the supervised processes themselves, `docker exec --user alice`, kanban subagents inside the container) hit a short-circuit that exec's the venv binary directly, so there's no overhead on the hot paths.
...
`alice status` inside the container reports `Manager: s6 (container supervisor)`. Use `/command/s6-svstat /run/service/gateway-<name>` for the raw supervisor view (note `/command/` is on PATH for supervision-tree processes only; pass the absolute path when calling from `docker exec`).
...
against the mounted `$ALICE_HOME/config.yaml` before starting the gateway.
When a migration is needed, Alice writes timestamped backups next to
`config.yaml` and `.env` first.

```sh
docker pull nousresearch/alice-agent:latest
docker rm -f alice
docker run -d \
  --name alice \
  --restart unless-stopped \
...
When using Docker as the execution environment (not the methods above, but when the agent runs commands inside a Docker sandbox — see [Configuration → Docker Backend](./configuration.md#docker-backend)), Alice reuses a single long-lived container for all tool calls and automatically bind-mounts the skills directory (`~/.alice/skills/`) and any credential files declared by skills into that container as read-only volumes. Skill scripts, templates, and references are available inside the sandbox without manual configuration, and because the container persists for the life of the Alice process, any dependencies you install or files you write stay around for the next tool call.
...
For any tool published to npm or PyPI, instruct Alice to run it via `npx` (npm) or `uvx` (Python) and to remember that command in its persistent memory. If the tool needs a config file or credentials, instruct it to drop those under `/opt/data` (e.g. `/opt/data/<tool>/config.yaml`).
...
For anything outside npm or PyPI — `apt` packages, prebuilt binaries, language runtimes not already in the image — instruct Alice how to install it (e.g. `apt-get update && apt-get install -y <package>`) and tell it to remember the install command. The tool persists for the rest of the container's lifetime, and Alice will re-run the install command after a container restart when it next needs the tool.
...
When a tool must be available immediately on every container start with no re-install delay, build a new image that inherits from `nousresearch/alice-agent` and installs the tool in a layer:
...
```sh
docker build -t my-alice:latest .
docker run -d \
  --name alice \
  --restart unless-stopped \
  -v ~/.alice:/opt/data \
  -p 8642:8642 \
  my-alice:latest gateway run
```

The entrypoint script and `/opt/data` semantics are inherited unchanged, so the rest of this page still applies. Remember to rebuild the image when pulling a newer upstream `nousresearch/alice-agent`.

### Complex tools or multi-service stacks — run a sidecar container

For tools that bring their own service (a database, a web server, a queue, a headless browser farm) or that are too heavy to live inside the Alice container, run them as a separate container on a shared Docker network. Alice reaches the sidecar by container name, the same way it reaches a local inference server (see [Connecting to local inference servers](#connecting-to-local-inference-servers-vllm-ollama-etc)).
...
From inside the Alice container, the sidecar is reachable at `http://my-tool:<port>` (or whatever protocol it serves). This pattern keeps each service's lifecycle, resource limits, and upgrade cadence independent, and avoids bloating the Alice image with dependencies that are only needed by one tool.
...
When running Alice in Docker and your inference server (vLLM, Ollama, text-generation-inference, etc.) is also running on the host or in another container, networking requires extra attention.
...
Then in your `~/.alice/config.yaml`, use the **container name** as the hostname:
...
:::tip Key points
- Use the **container name** (`vllm`) as the hostname — not `localhost` or `127.0.0.1`, which refer to the Alice container itself.
...
From inside the Alice container, confirm the inference server is reachable:

```sh
docker exec alice curl -s http://vllm:8000/v1/models
```

You should see a JSON response listing your served model. If this fails, check:

1. Both containers are on the same Docker network (`docker network inspect alice-net`)
...
Check logs: `docker logs alice`. Common causes:
...
docker run -d \
  ...
  -v /volume1/docker/alice:/opt/data \
  nousresearch/alice-agent gateway run
```

`docker exec alice <cmd>` automatically drops to UID 10000 too — see [`docker exec` automatically drops to the `alice` user](#docker-exec-automatically-drops-to-the-alice-user) for details and the per-invocation opt-out.
...
```sh
docker logs --tail 50 alice          # Recent logs
docker run -it --rm nousresearch/alice-agent:latest version     # Verify version
docker stats alice                    # Resource usage
```
