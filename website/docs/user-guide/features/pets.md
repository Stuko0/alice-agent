...
Alice can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ALICE_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Alice renders the sprite at full fidelity when your
...
alice pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `alice pets list [query] [--limit N]` |
| List installed pets | `alice pets list --installed` |
| Install a pet | `alice pets install <slug> [--select] [--force]` |
...
| Preview/animate | `alice pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `alice pets off` |
| Remove an installed pet | `alice pets remove <slug>` |
| Diagnose setup | `alice pets doctor` |

`alice pets show` flags:
...
Beyond installing pre-made pets from the gallery, Alice can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `alice pets` → the generate flow.
...
always-on-top desktop window. Out there it stays visible while Alice is
...
Run `alice pets doctor` — it reports:
