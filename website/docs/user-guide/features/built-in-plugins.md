...
description: "Plugins shipped with Alice Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Alice ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.alice/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Alice Plugin](/guides/build-a-alice-plugin) to write your own.
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `alice memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `alice plugins list` and the interactive `alice plugins` UI), but none load until you explicitly enable them:

```bash
alice plugins enable disk-cleanup
```

Or via `~/.alice/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Alice. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
alice plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `alice plugins enable <name>`.
...
| `alice-achievements` | dashboard tab | Steam-style collectible badges generated from your real Alice session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `alice memory` and `alice plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ALICE_HOME` or `/tmp/alice-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ALICE_HOME | always | Never |
...
**State** — everything lives at `$ALICE_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `ALICE_HOME` or `/tmp/alice-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `alice plugins enable disk-cleanup` (or check the box in `alice plugins`).

**Disabling again:** `alice plugins disable disk-cleanup`.
...
**Enabling:** `alice plugins enable security-guidance` (or check the box in `alice plugins`).

**Disabling again:** `alice plugins disable security-guidance`.
...
Traces Alice turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Alice' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `alice logs`.
...
```bash
alice native          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Alice and the next turn ships a trace.
...
alice chat -q "hello"              # check the Langfuse UI for a "Alice turn" trace
```
...
**Disabling:** `alice plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.alice/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
alice plugins enable google_meet
...
**Disabling:** `alice plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.alice/cache/google_meet/` until you remove them.

### alice-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Alice session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Alice feature changes.

**How it works:**

- Scans your entire `~/.alice/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$ALICE_HOME/plugins/alice-achievements/state.json`
...
| Secret | Hidden until Alice detects the first related signal in your history |

**API** — routes mount under `/api/plugins/alice-achievements/`:
...
**State files** — live under `$ALICE_HOME/plugins/alice-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Alice updates. |
...
**Enabling:** Nothing to enable — `alice-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `alice dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/alice-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.alice/plugins/alice-achievements/` that ships no dashboard. The plugin's state files under `$ALICE_HOME/plugins/alice-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Alice plugin — see [Build a Alice Plugin](/guides/build-a-alice-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.alice/plugins/<name>/`
- Manifest source is reported as `bundled` in `alice plugins list`
