...
If the file is empty, whitespace-only, or cannot be read, Alice falls back to a built-in default identity ("You are Alice Agent, an intelligent AI assistant created by Stuko..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
