...
Alice Agent can drive your desktop — clicking, typing, scrolling,
...
```
alice computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `alice computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `alice native`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Alice app). `alice computer-use doctor` will tell you which permission is missing. |
...
```
alice -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.alice/config.yaml`.

## `alice computer-use doctor` — your first triage stop

`alice computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ alice computer-use doctor
...
agent is acting here." Each Alice run declares its own cua-driver
**session id** (something like `alice-3a7b9c14d2e8`); the cursor's
...
Alice intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Alice-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Alice skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Alice
...
Alice applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.alice/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Alice applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Alice Agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Alice Agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Alice warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Alice from inside the RDP / console
...
```
ALICE_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
ALICE_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Alice disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Alice sets
...
When it's on, `alice computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Alice at a binary you built from source instead
of the published release. Alice resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`ALICE_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Alice testing (see notes).
...
### Option B — point Alice straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`ALICE_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
ALICE_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
ALICE_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Alice is using your build

- `alice computer-use status` prints the resolved binary path and
  version.
- `alice computer-use doctor` confirms the binary is reachable and
...
- **Alice spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Alice
  change needed either way.
- **Local builds skip the version check.** Alice warns when the
...
**First action when anything's off: run `alice computer-use doctor`.**
...
Run `alice computer-use install` to fetch the cua-driver binary, or
run `alice native` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `alice computer-use doctor` will
...
- **Alice-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Alice `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Alice (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Alice among them)
