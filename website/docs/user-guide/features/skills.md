...
All skills live in **`~/.alice/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Alice at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `alice update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.alice` profile):

```bash
curl -fsSL https://alice-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
alice profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
alice skills opt-out            # stop future seeding — nothing on disk is touched
alice skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
alice skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `alice update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `alice skills opt-in`) to re-enable.

:::note Safe by default
`alice skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Alice installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Alice to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.alice/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
alice chat --toolsets skills -q "What skills do you have?"
alice chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Alice-tool framing, no invented
commands).
...
/home/user/.alice/cache/chart-q4-2025.png
...
When a missing value is encountered, Alice asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `alice setup` or `~/.alice/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `alice config migrate` prompts for unconfigured settings, and `alice config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.alice/skills/                  # Single source of truth
...
If you maintain skills outside of Alice — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Alice to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.alice/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.alice/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Alice process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Alice ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.alice/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.alice/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `alice bundles list`.
...
alice bundles create research

# Overwrite an existing bundle
alice bundles create backend-dev --skill ... --force

# Delete a bundle
alice bundles delete backend-dev

# Re-scan ~/.alice/skill-bundles/ and report changes
alice bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.alice/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.alice/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.alice/pending/skills/` and are
...
```bash
alice skills browse                              # Browse all hub skills (official first)
alice skills browse --source official            # Browse only official optional skills
alice skills search kubernetes                   # Search all sources
alice skills search react --source skills-sh     # Search the skills.sh directory
alice skills search https://mintlify.com/docs --source well-known
alice skills inspect openai/skills/k8s           # Preview before installing
alice skills install openai/skills/k8s           # Install with security scan
alice skills install official/security/1password
alice skills install skills-sh/vercel-labs/json-render/json-render-react --force
alice skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
alice skills list --source hub                   # List hub-installed skills
alice skills check                               # Check installed hub skills for upstream updates
alice skills update                              # Reinstall hub skills with upstream changes when needed
alice skills audit                               # Re-scan all hub skills for security
alice skills uninstall k8s                       # Remove a hub skill
alice skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
alice skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
alice skills publish skills/my-skill --to github --repo owner/repo
alice skills snapshot export setup.json          # Export skill config
alice skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Alice. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `alice skills search <query> --source skills-sh`. Alice resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Alice currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Alice repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Alice can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
alice skills search https://mintlify.com/docs --source well-known
alice skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
alice skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Alice can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
alice skills install openai/skills/k8s
alice skills tap add myorg/skills-repo
```
...
[Skills Hub](https://alice-agent.nousresearch.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Alice-side changes required.
...
- Alice source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Alice supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Alice source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Alice can search and convert agent entries from LobeHub's public catalog into installable Alice skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Alice source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Alice integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Alice' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Alice source id: `browse-sh`
- Trust level: `community`

```bash
alice skills search airbnb --source browse-sh
alice skills inspect browse-sh/airbnb.com/search-listings-ddgioa
alice skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Alice fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
```bash
alice skills install https://sharethis.chat/SKILL.md
alice skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
alice skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `alice skills update` re-fetches from the same URL automatically when you want to refresh.
...
`alice skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Alice | Always trusted |
...
```bash
alice skills check          # Report which installed hub skills changed upstream
alice skills update         # Reinstall only the skills with updates available
alice skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Alice users add with `alice skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.alice.tags`, `version`, `author`, `platforms`, `metadata.alice.config`).
...
Alice discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Alice user can subscribe and install:

```bash
alice skills tap add my-org/alice-skills
alice skills search deploy
alice skills install my-org/alice-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.alice/.hub/taps.json`:
...
The `alice skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `alice skills tap list` shows the effective path per tap.
...
```bash
alice skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.alice/.hub/taps.json` (created on demand).

## Bundled skill updates (`alice skills reset`)

Alice ships with a set of bundled skills in `skills/` inside the repo. On install and on every `alice update`, a sync pass copies those into `~/.alice/skills/` and records a manifest at `~/.alice/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Alice recomputes the hash of your local copy and compares it to the origin hash:
...
alice skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `ALICE_HOME`, so `alice -p coder skills reset <name>` only affects that profile.
:::
