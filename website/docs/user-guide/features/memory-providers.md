...
Alice Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
alice memory setup      # interactive picker + configuration
alice memory status     # check what's active
alice memory off        # disable external provider
```

You can also select the active memory provider via `alice plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.alice/config.yaml`:
...
When a memory provider is active, Alice automatically:
...
**Setup Wizard:**
```bash
alice memory setup        # select "honcho" — runs the Honcho-specific post-setup
```
...
:::tip Migrating from `alice honcho`
If you previously used `alice honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Alice profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Alice profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Alice profile. Host key `alice` → default; `alice.<profile>` for others. |
...
```bash
alice profile create coder --clone
```

`--clone` creates a `alice.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Alice profile, creates host blocks for any profile without one, inherits settings from the default `alice` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `alice memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
alice memory setup    # select "openviking"
# Or manually:
alice config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.alice/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.alice/.env
```
...
`OPENVIKING_AGENT` is Alice' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
alice memory setup    # select "mem0" → "Platform"
# Or manually:
alice config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.alice/.env
```

**Setup (OSS):**
```bash
alice memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
alice memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
alice memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Config:** `$ALICE_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.alice/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `alice-user` | User identifier |
| `agent_id` | `alice` | Agent identifier |
...
**Switching modes:** Re-run `alice memory setup mem0 --mode <platform|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
alice memory setup    # select "hindsight"
# Or manually:
alice config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.alice/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p alice ui start`

**Config:** `$ALICE_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `alice` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Alice Agent and the User` | Context label for retained memories |
...
**Setup:**
```bash
alice memory setup    # select "holographic"
# Or manually:
alice config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.alice-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ALICE_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
alice memory setup    # select "retaindb"
# Or manually:
alice config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.alice/.env
```
...
# Then configure Alice
alice memory setup    # select "byterover"
# Or manually:
alice config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$ALICE_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=alice) |
...
**Setup:**
```bash
alice memory setup    # select "supermemory"
# Or manually:
alice config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.alice/.env
```

**Config:** `$ALICE_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `alice` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `alice-{identity}` → `alice-coder`) to isolate memories per Alice profile
...
| **Requires** | `pip install alice-memori` + `alice-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install alice-memori
alice-memori install
alice config set memory.provider memori
alice memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `alice-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$ALICE_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ALICE_HOME/` so each profile has its own credentials
