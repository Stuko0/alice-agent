...
Alice has three hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.alice/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.alice/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Each hook is a directory under `~/.alice/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".alice" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.alice/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.alice/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".alice" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.alice/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.alice/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/alice-events"
...
A popular pattern from the community: drop a Markdown checklist at `~/.alice/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. Alice does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.alice/BOOT.md` with natural-language startup instructions.
...
Create `~/.alice/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `alice cron list` and check if any scheduled jobs failed overnight.
...
**`~/.alice/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.alice/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.alice/hooks/boot-md/handler.py`**

```python
"""Run ~/.alice/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".alice" / "BOOT.md"
...
```bash
alice logs --follow --level INFO | grep boot-md
```
...
Delete `~/.alice/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
An earlier version of Alice shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.

### How It Works

1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.alice/hooks/`
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is Alice's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
See the **[Build a Plugin guide](/guides/build-a-alice-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
...
Declare shell-script hooks in your `cli-config.yaml` and Alice will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`alice_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.alice/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.alice/agent-hooks/` (by convention) | `~/.alice/plugins/<name>/` | `~/.alice/hooks/<name>/` |
...
Each time the event fires, Alice spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // Alice-canonical
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.alice/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.alice/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.alice/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.alice/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.alice/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.alice/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Alice event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.alice/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.alice/agent-hooks/log-orchestration.sh
log=~/.alice/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Alice sees it, then persists the decision to `~/.alice/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `alice --accept-hooks chat`)
2. `ALICE_ACCEPT_HOOKS=1` environment variable
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `alice hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

#### Manual allowlisting

Manual allowlisting is useful for non-TTY or service-account deployments where an operator cannot answer the first-use prompt interactively. The allowlist file is `~/.alice/shell-hooks-allowlist.json`, and the expected format is an `approvals` array. Each approval records the hook `event` and the exact `command` string:
...
```json
{
  "approvals": [
    {
      "event": "post_llm_call",
      "command": "/home/alice/.alice/hooks/my-hook.py"
    }
  ]
}
```

The command string must match the configured hook command exactly. A path-keyed object with a `sha256` field is not the expected format and will not approve the hook. Verify manual entries with `alice hooks list`.

### The `alice hooks` CLI

| Command | What it does |
|---------|--------------|
| `alice hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `alice hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `alice hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `alice hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.alice/agent-hooks/` so the path is easy to audit.
- Re-run `alice hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
