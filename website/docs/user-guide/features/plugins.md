...
Project-local plugins under `./.alice/plugins/` are disabled by default. Enable them only for trusted repositories by setting `LYDIA_ENABLE_PROJECT_PLUGINS=true` before starting Alice.
...
| Project | `.alice/plugins/` | Project-specific plugins (requires `LYDIA_ENABLE_PROJECT_PLUGINS=true`) |
