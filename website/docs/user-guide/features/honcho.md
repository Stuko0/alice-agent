...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Alice's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Alice instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
alice memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `alice` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Alice syncs them back at session init.
...
The `alice honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `alice memory setup honcho` (or run `alice memory setup` and pick it from the list); the `alice honcho` subcommand then appears on the next invocation.

```bash
alice memory setup honcho    # Configure Honcho directly (works before activation)
alice honcho status          # Connection status, config, and key settings
alice honcho setup           # Redirects to `alice memory setup` (post-activation alias)
alice honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
alice honcho peer            # Show or update peer names + dialectic reasoning level
alice honcho mode            # Show or set recall mode (hybrid/context/tools)
alice honcho tokens          # Show or set token budget for context and dialectic
alice honcho identity        # Seed or show the AI peer's Honcho identity
alice honcho sync            # Sync Honcho config to all existing profiles
alice honcho peers           # Show peer identities across all profiles
alice honcho sessions        # List known Honcho session mappings
alice honcho map             # Map current directory to a Honcho session name
alice honcho enable          # Enable Honcho for the active profile
alice honcho disable         # Disable Honcho for the active profile
alice honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `alice honcho`

If you previously used the standalone `alice honcho setup`:
...
No re-login or re-setup needed. Run `alice memory setup` and select "honcho" — the wizard detects your existing config.
