...
chat completions — use your Alice-managed provider subscription as their
...
| Use case | "Use Alice as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Alice stores
the refresh token in `~/.alice/auth.json` — the same place all Alice
...
Model:      Alice-4-70B    # or Alice-4.3-36B, Alice-4-405B
```
...
interface in `alice_cli/proxy/adapters/`.
...
If you see `not logged in`, run `alice portal`. If you see
...
supports; check `stuko.dev/models`.
...
`alice_cli/proxy/adapters/<provider>.py` and registering it in
