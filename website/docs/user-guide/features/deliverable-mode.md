...
When Alice Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.alice/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.alice/config.yaml` (switchable per session
via `/personality`).
...
If you use Alice' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.alice/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Alice Agent's deliverable
