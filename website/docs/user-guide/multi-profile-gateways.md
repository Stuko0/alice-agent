...
If you only run one Alice Agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Alice Agents that should all
...
(`ai.alice.gateway-<name>.plist`) or systemd user service
(`alice-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
alice profile create coder
alice profile create personal-bot
alice profile create research
...
```bash
alice config set gateway.multiplex_profiles true
alice gateway restart
```

Equivalently, in the default profile's `~/.alice/config.yaml`:
...
You do **not** run `alice gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `alice gateway start` / `run` is a
...
home). `alice status` reports the multiplexer and the profiles it serves;
`alice status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/alice-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: alice-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    alice gateway "$action"
  else
    alice -p "$profile" gateway "$action"
  fi
}
...
```bash
alice-gateways start      # start every configured profile
alice-gateways stop       # stop every configured profile
alice-gateways restart    # restart all
alice-gateways status     # status across all
alice-gateways list       # delegates to `alice gateway list`
```

:::tip
The `default` profile is targeted with `alice gateway <action>` (no `-p`),
not `alice -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `alice -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.alice.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/alice-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.alice.gateway.plist` /
`alice-gateway.service`.
...
```bash
# Default profile
tail -f ~/.alice/logs/gateway.log
tail -f ~/.alice/logs/gateway.error.log

# Named profile
tail -f ~/.alice/profiles/<name>/logs/gateway.log
tail -f ~/.alice/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.alice/logs/gateway.log ~/.alice/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
alice logs -f                  # follow default profile
alice -p coder logs -f         # follow one profile
alice logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
alice profile list             # profiles + model + gateway state
alice-gateways status          # full status across every profile
launchctl list | grep alice    # macOS — PIDs and labels
systemctl --user list-units 'alice-gateway-*'   # Linux — units
```
...
The default profile uses `~/.alice/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
alice config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.alice/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=alice --why="gateways running" \
  sleep infinity &
...
`alice-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.alice/.env ~/.alice/profiles/*/.env
```

## Updating the code

`alice update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `alice gateway start` after a previous `alice gateway stop`. The
...
```bash
ps -ef | grep "alice_cli.*-p <profile>"
cat ~/.alice/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.alice.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.alice.gateway-<profile>.plist

# Linux
systemctl --user restart alice-gateway-<profile>.service
```

### Health check

```bash
alice doctor                  # default profile
alice -p <profile> doctor     # one profile
```
