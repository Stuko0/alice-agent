...
4. Store the token in `${ALICE_HOME:-~/.alice}/.env` (or via `alice setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Alice. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
- **`AIRTABLE_API_KEY` flows from `${ALICE_HOME:-~/.alice}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
