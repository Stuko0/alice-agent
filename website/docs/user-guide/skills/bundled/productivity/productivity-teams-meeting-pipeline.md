...
description: "Operate the Teams meeting summary pipeline via Alice CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions"
---
...
Everything operator-facing is a `alice teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${ALICE_HOME:-~/.alice}/.env`:
...
```bash
alice teams-pipeline validate              # config snapshot — run first after any change
alice teams-pipeline token-health          # Graph token status
alice teams-pipeline token-health --force-refresh   # force a fresh token acquisition
alice teams-pipeline list                  # recent meeting jobs
alice teams-pipeline list --status failed  # only failed jobs
alice teams-pipeline show <job-id>         # full detail of one job
alice teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
alice teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
alice teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
alice teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
alice teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

alice teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
alice teams-pipeline delete-subscription <sub-id>
alice teams-pipeline maintain-subscriptions            # renew near-expiry ones
alice teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `alice teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `alice cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
