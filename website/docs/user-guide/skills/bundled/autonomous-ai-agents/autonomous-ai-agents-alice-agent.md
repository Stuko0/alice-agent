---
title: "Alice Agent — Configure, extend, or contribute to Alice Agent"
sidebar_label: "Alice Agent"
...
| Path | `skills/autonomous-ai-agents/alice-agent` |
| Version | `2.1.0` |
...
:::info
The following is the complete skill definition that Alice loads when this skill is triggered. This is what the agent sees as instructions when the skill is active.
:::
...
alice auth add PROVIDER    Add OAuth or API-key credential (e.g. nous, openai-codex, qwen-oauth)
alice auth list            List stored credentials
alice auth remove PROVIDER Remove a stored credential
...
```
alice auth add             Interactive credential wizard
...
alice honcho setup/status  Honcho memory integration (requires honcho plugin)
...
without BOM. `alice config edit` writes without BOM; manual edits in
...
Alice's own env scrubber dropping `SYSTEMROOT` / `WINDIR` / `COMSPEC`
...
POSIX venv layouts (`.venv/bin/activate`). The Alice-installed venv at
...
If Alice reports a smaller context window than your local model supports
(e.g., 128k when llama-server has `-c 262144`):

**Check if `model.context_length` is explicitly set.** Alice uses a
...
All handlers must return JSON strings. Use `get_alice_home()` for paths, never hardcode `~/.alice`.
...
**Windows contributors:** `scripts/run_tests.sh` currently looks for POSIX venvs (`.venv/bin/activate` / `venv/bin/activate`) and will error out on Windows where the layout is `venv/Scripts/activate` + `python.exe`. The Alice-installed venv at `venv/Scripts/` also has no `pip` or `pytest` — it's stripped for end-user install size. Workaround: install pytest + pytest-xdist + pyyaml into a system Python 3.11 user site (`/c/Program Files/Python311/python -m pip install --user pytest pytest-xdist pyyaml`), then run tests directly:
