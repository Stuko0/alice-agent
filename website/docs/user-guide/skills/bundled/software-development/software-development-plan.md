...
| Version | `2.0.0` |
| Author | Alice Agent (writing-craft adapted from obra/superpowers) |
| License | MIT |
...
- Your deliverable is a markdown plan saved inside the active workspace under `.alice/plans/`.
...
- `.alice/plans/YYYY-MM-DD_HHMMSS-<slug>.md`

Treat that as relative to the active working directory / backend workspace. Alice file tools are backend-aware, so using this relative path keeps the plan with the workspace on local, docker, ssh, modal, and daytona backends.

If the runtime provides a specific target path, use that exact path.
If not, create a sensible timestamped filename yourself under `.alice/plans/`.
...
> **For Alice:** Use subagent-driven-development skill to implement this plan task-by-task.
...
Use Alice native to understand the project:
