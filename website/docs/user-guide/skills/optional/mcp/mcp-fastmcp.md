...
| | |
|---|---|
| Source | Optional — install with `alice skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Alice or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Alice. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.alice/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.alice/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Alice integration, either:

- configure the server in `~/.alice/config.yaml` using the `native-mcp` skill, or
...
### Alice cannot see the deployed server

The server-building part may be correct while the Alice config is not. Load the `native-mcp` skill and configure the server in `~/.alice/config.yaml`, then restart Alice.
