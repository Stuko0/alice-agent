...
| | |
|---|---|
| Source | Optional — install with `alice skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Alice Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Alice terminal calls (desktop app flow only)
...
### Service Account (recommended for Alice)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ALICE_HOME:-~/.alice}/.env` (the skill will prompt for this on first load).
...
## Alice Execution Pattern (desktop app flow)

Alice terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/alice-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/alice-op.sock"
