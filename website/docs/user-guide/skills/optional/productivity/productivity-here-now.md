...
| | |
|---|---|
| Source | Optional — install with `alice skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ALICE_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ALICE_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ALICE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client alice
```
...
```bash
PUBLISH="${ALICE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client alice
```
...
```bash
DRIVE="${ALICE_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `alice`)    |
