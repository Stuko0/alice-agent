...
| | |
|---|---|
| Source | Optional — install with `alice skills install official/productivity/siyuan` |
...
3. Store it in `${ALICE_HOME:-~/.alice}/.env`:
   ```
...
```yaml
# In ~/.alice/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
