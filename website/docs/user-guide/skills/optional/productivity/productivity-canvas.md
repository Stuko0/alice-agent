...
| | |
|---|---|
| Source | Optional — install with `alice skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Alice Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${ALICE_HOME:-~/.alice}/.env`:
...
```bash
CANVAS="python $ALICE_HOME/skills/productivity/canvas/scripts/canvas_api.py"
