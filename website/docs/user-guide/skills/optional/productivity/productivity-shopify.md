...
| | |
|---|---|
| Source | Optional — install with `alice skills install official/productivity/shopify` |
...
4. Save to `${ALICE_HOME:-~/.alice}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Alice","productType":"Apparel","tags":["test"]}}'
```
