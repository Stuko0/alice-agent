...
| | |
|---|---|
| Source | Optional — install with `alice skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Alice Agent |
| License | MIT |
...
import upstream classes into Alice itself.
...
```bash
mkdir -p ~/.alice/cache/darwinian-evolver && cd ~/.alice/cache/darwinian-evolver
...
```bash
cd ~/.alice/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.alice/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.alice/skills/research/darwinian-evolver
DE_DIR=~/.alice/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Alice core.
   Custom driver scripts under `~/.alice/skills/...` are user-side and fine.
