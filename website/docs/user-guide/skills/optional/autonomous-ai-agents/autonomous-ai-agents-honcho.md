...
description: "Configure and use Honcho memory with Alice -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Alice -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `alice skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Alice

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Alice profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/alice#running-honcho-locally-with-alice

### Verify

```bash
alice honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Alice profile's AI peer
...
Honcho models conversations as interactions between **peers**. Alice creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Alice instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Alice run |
| `global` | Single session across all directories |

Manual override: `alice honcho map my-project-name`
...
Each Alice profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block alice.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `alice.coder` host block in `honcho.json`
...
```bash
alice honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Alice when Honcho memory is active.
...
Config file: `$ALICE_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `alice honcho setup`. Ensure `memory.provider: honcho` is in `~/.alice/config.yaml`.

### Memory not persisting across sessions
Check `alice honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `alice profile create <name> --clone`. For existing profiles: `alice honcho sync`.
...
| Command | Description |
|---------|-------------|
| `alice honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `alice honcho status` | Show resolved config, connection test, peer info for active profile |
| `alice honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `alice honcho disable` | Disable Honcho for the active profile |
| `alice honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `alice honcho peers` | Show peer identities across all profiles |
| `alice honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `alice honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `alice honcho sessions` | List known directory-to-session-name mappings |
| `alice honcho map <name>` | Map current working directory to a Honcho session name |
| `alice honcho identity` | Seed AI peer identity or show both peer representations |
| `alice honcho sync` | Create host blocks for all Alice profiles that don't have one yet |
| `alice honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Alice + Honcho |
| `alice memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `alice memory status` | Show active memory provider and config |
| `alice memory off` | Disable external memory provider |
