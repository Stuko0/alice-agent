...
title: "Git Worktrees"
description: "Run multiple Alice Agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Alice Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Alice so each session has a clean, isolated working directory.

## Why Use Worktrees with Alice?

Alice treats the **current working directory** as the project root:

- CLI: the directory where you run `alice` or `alice chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.alice/config.yaml`
...
git worktree add ../repo-feature feature/alice-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/alice-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Alice there:

```bash
cd ../repo-feature

# Start Alice in the worktree
alice
```
...
git worktree add ../repo-experiment-a feature/alice-a
git worktree add ../repo-experiment-b feature/alice-b
```
...
- Works on its own branch (`feature/alice-a` vs `feature/alice-b`).
...
- Alice checkpoint data under `~/.alice/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Alice experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/alice-checkpoints-docs`, `feature/alice-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Alice from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `alice -w` (Automatic Worktree Mode)

Alice has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `alice/alice-<hash>`).
...
```bash
alice -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `alice -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Alice session its own clean checkout.
