...
sidebar_position: 3
title: "Desktop App"
description: "The native Alice desktop app — a polished experience for chatting with Alice, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Alice desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Alice Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `alice` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Alice has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`alice`) and **[TUI](./tui.md)** (`alice --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`alice dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Alice Desktop](../getting-started/installation.md).

If you already have Alice installed, simply run
...
- **The same conversation history** as every other Alice surface — sessions started here resume in the CLI/TUI and vice versa.
...
Talk to Alice and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `alice model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Alice management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md).
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#alice-cron).
- **Profiles** — switch between [Alice profiles](./profiles.md) (isolated config/skills/sessions).
...
- **Uninstall Chat GUI only** — removes the desktop app and its data; the Alice Agent, your config, and your chats stay. (Same as `alice uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `alice uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `alice uninstall --full`.)
...
You can do the same from the terminal — `alice uninstall --gui` for the GUI alone, or `alice uninstall` / `alice uninstall --full` for the agent too.

:::note
Running `alice uninstall --gui` from a **source checkout** (a `alice desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `alice desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::
...
By default the app starts and manages its own **local** backend. You can instead point it at a Alice backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.

:::info The remote backend is a running `alice serve` process
"Remote backend" means a **`alice serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `alice serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.alice/.env` (the secrets file, mode 0600):
...
alice serve --host 0.0.0.0 --port 9119
```

Keep that `alice serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.
...
Boot logs land in `ALICE_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
rm "$HOME/.alice/alice-agent/.alice-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.alice/alice-agent/venv"

# Reset a stuck macOS microphone prompt
tccutil reset Microphone com.nousresearch.alice
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.alice/alice-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
- [TUI](./tui.md) — the modern terminal UI used by `alice --tui` and the dashboard chat tab
