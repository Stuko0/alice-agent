...
Connect Alice to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Alice — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Alice runs it in a
...
alice photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Alice Agent` project on your account.
...
Runtime credentials are written to `~/.alice/.env`
...
(device token, dashboard project id) lives in `~/.alice/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Alice
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Alice replies with a pairing code. Approve it with:

```bash
alice pairing approve photon <CODE>
```

Use `alice pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.alice/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.alice/.env`):
...
By default Alice responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Alice` and
`@Alice Agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Alice will reply.
...
assigned iMessage line Alice uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run alice photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `alice photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `alice photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `alice photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Alice sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Alice wake words  | JSON list / comma / newline regex patterns for group mentions |
