...
Connect Alice to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Alice supports two WeCom integration modes:
...
> Run `alice gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Alice decrypts the message, queues it for the agent
...
(Use `alice gateway start` only after `alice gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Alice and the token the
...
in `~/.alice/.env` values around `=` will also break signature checks. After
fixing, restart `alice gateway run`.
...
Check `alice gateway run` logs for the bound host/port. If the adapter bound to
