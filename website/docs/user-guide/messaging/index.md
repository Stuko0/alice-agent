...
description: "Chat with Alice from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Alice from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Alice](/guides/use-voice-mode-with-alice).
...
```mermaid
flowchart TB
    subgraph Gateway["Alice Gateway"]
...
For group chats, hooks, and automation flows, Alice supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Alice keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Alice does not hide failures just because the text resembles a silence token.
...
```bash
alice gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
alice gateway              # Run in foreground
alice gateway setup        # Configure messaging platforms interactively
alice gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo alice gateway install --system   # Linux only: install a boot-time system service
alice gateway start        # Start the default service
alice gateway stop         # Stop the default service
alice gateway status       # Check default service status
alice gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Alice Agent to the latest version |
...
Configure per-platform overrides in `~/.alice/gateway.json`:
...
alice pairing approve telegram XKGH5N7P

# Other pairing commands:
alice pairing list          # View pending + approved users
alice pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Alice appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.alice/config.yaml`:
...
Off by default. When enabled, Alice prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.alice/config.yaml`:
...
```bash
alice gateway install               # Install as user service
alice gateway start                 # Start the service
alice gateway stop                  # Stop the service
alice gateway status                # Check status
journalctl --user -u alice-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo alice gateway install --system
sudo alice gateway start --system
sudo alice gateway status --system
journalctl -u alice-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Alice installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit alice-gateway` (or `sudo systemctl edit alice-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `alice update`. When `alice update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart alice-gateway` command (it never blocks on an interactive password prompt).
...
```bash
alice gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `alice update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo alice update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/alice-gateway`:

```
alice ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed alice-gateway*, /usr/bin/systemctl --no-ask-password start alice-gateway*, /usr/bin/systemctl --no-ask-password restart alice-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Alice will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Alice installations on the same machine (with different `ALICE_HOME` directories), each gets its own systemd service name. The default `~/.alice` uses `alice-gateway`; other installations use `alice-gateway-<hash>`. The `alice gateway` commands automatically target the correct service for your current `ALICE_HOME`.
:::

### macOS (launchd)

```bash
alice gateway install               # Install as launchd agent
alice gateway start                 # Start the service
alice gateway stop                  # Stop the service
alice gateway status                # Check status
tail -f ~/.alice/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.alice.gateway.plist`. It includes three environment variables:
...
- **ALICE_HOME** — scopes the gateway to your Alice installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `alice gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `ALICE_HOME` directory gets its own launchd label. The default `~/.alice` uses `ai.alice.gateway`; other installations use `ai.alice.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `alice-cli` | Full access |
| Telegram | `alice-telegram` | Full tools including terminal |
| Discord | `alice-discord` | Full tools including terminal |
| WhatsApp | `alice-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `alice-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `alice-slack` | Full tools including terminal |
| Google Chat | `alice-google_chat` | Full tools including terminal |
| Signal | `alice-signal` | Full tools including terminal |
| SMS | `alice-sms` | Full tools including terminal |
| Email | `alice-email` | Full tools including terminal |
| Home Assistant | `alice-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `alice-mattermost` | Full tools including terminal |
| Matrix | `alice-matrix` | Full tools including terminal |
| DingTalk | `alice-dingtalk` | Full tools including terminal |
| Feishu/Lark | `alice-feishu` | Full tools including terminal |
| WeCom | `alice-wecom` | Full tools including terminal |
| WeCom Callback | `alice-wecom-callback` | Full tools including terminal |
| Weixin | `alice-weixin` | Full tools including terminal |
| BlueBubbles | `alice-bluebubbles` | Full tools including terminal |
| QQBot | `alice-qqbot` | Full tools including terminal |
| Yuanbao | `alice-yuanbao` | Full tools including terminal |
| Microsoft Teams | `alice-teams` | Full tools including terminal |
| API Server | `alice-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `alice-webhook` | Full tools including terminal |
| Raft | `alice-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.alice/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
