...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Alice Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Alice to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.alice/.env`:
...
All standard Alice commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.alice/logs/gateway.log`
...
4. Consider enabling verbose logging: `ALICE_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Alice automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
ALICE_LOG_LEVEL=debug alice gateway
```
...
```bash
alice chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
