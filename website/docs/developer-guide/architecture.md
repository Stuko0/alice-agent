...
sidebar_position: 1
title: "Architecture"
description: "Alice Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Alice Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ alice_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # AliceCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── alice_state.py           # SQLite session/state database with FTS5
├── alice_constants.py       # ALICE_HOME, profile-aware paths
...
│
├── alice_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `alice` subcommands (large file)
...
│   ├── skills_config.py      # alice skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # alice native — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # alice gateway start/stop
│
...
```text
User input → AliceCLI.process_input()
...
Three discovery sources: `~/.alice/plugins/` (user), `.alice/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `alice plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-alice-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Alice as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`alice -p <name>`) gets its own ALICE_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
