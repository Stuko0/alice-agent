...
description: "How Alice stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `alice_cli/cron.py` | CLI `alice cron` subcommands |
...
Jobs are stored in `~/.alice/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `alice cron` commands are run or during active CLI sessions.
...
```python
# ~/.alice/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.alice/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `alice cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `alice cron` CLI provides direct job management:

```bash
alice cron list                    # Show all jobs
alice cron create                  # Interactive job creation (alias: add)
alice cron edit <job_id>           # Edit job configuration
alice cron pause <job_id>          # Pause a running job
alice cron resume <job_id>         # Resume a paused job
alice cron run <job_id>            # Trigger immediate execution
alice cron remove <job_id>         # Delete a job
```
