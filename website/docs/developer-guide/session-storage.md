...
Alice Agent uses a SQLite database (`~/.alice/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `alice_state.py`


## Architecture Overview

```
~/.alice/state.db (SQLite, WAL mode)
...
Multiple alice processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.alice/state.db
...
Default path: `~/.alice/state.db`

This is derived from `alice_constants.get_alice_home()` which resolves to
`~/.alice/` by default, or the value of `ALICE_HOME` environment variable.
