...
The ACP adapter wraps Alice' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
alice acp / alice-acp / python -m acp_adapter
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'alice-agent[acp]==<version>' alice-acp`, pointed at the `alice-agent` PyPI release.
...
- `allow_once` -> Alice `once`
- `allow_always` -> Alice `always`
- reject options -> Alice `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Alice native to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["alice-acp"])
...
Instead it reuses Alice' runtime resolver:

- `acp_adapter/auth.py`
- `alice_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Alice provider/credentials. It also always advertises a terminal setup auth method (`alice-setup`, args `--setup`) so first-run registry clients can open Alice' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.alice/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `alice-acp` toolset definition
- `alice_cli/main.py` — `alice acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `alice-acp` script
