...
sidebar_position: 3
title: 'Learning Path'
description: 'Choose your learning path through the Alice Agent documentation based on your experience level and goals.'
---

# Learning Path

Alice Agent can do a lot — CLI assistant, Telegram/Discord bot, task automation, RL training, and more. This page helps you figure out where to start and what to read based on your experience level and what you're trying to accomplish.

:::tip Start Here
If you haven't installed Alice Agent yet, begin with the [Installation guide](/getting-started/installation) and then run through the [Quickstart](/getting-started/quickstart). Everything below assumes you have a working installation.
:::

:::tip First-time provider setup
First-time users almost always want `alice setup --portal` — one OAuth covers a model plus the four Tool Gateway tools (search/image/TTS/browser). See [Nous Portal](/integrations/nous-portal).
:::
...
- **Just browsing?** Check the [Key Features](#key-features-at-a-glance) table for a quick overview of everything Alice Agent can do.
...
Use Alice Agent as an interactive terminal assistant for writing, reviewing, and running code.
...
:::tip
Pass files directly into your conversation with context files. Alice Agent can read, edit, and run code in your projects.
:::

### "I want a Telegram/Discord bot"

Deploy Alice Agent as a bot on your favorite messaging platform.
...
7. [Use Voice Mode with Alice](/guides/use-voice-mode-with-alice)
...
:::tip
Cron jobs let Alice Agent run tasks on a schedule — daily summaries, periodic checks, automated reports — without you being present.
:::

### "I want to build custom tools/skills"

Extend Alice Agent with your own tools and reusable skill packages.

1. [Plugins](/user-guide/features/plugins)
2. [Build a Alice Plugin](/guides/build-a-alice-plugin)
...
page is for built-in Alice core development, not the usual user/custom-tool path.
:::

### "I want to train models"

Use reinforcement learning to fine-tune model behavior with Alice Agent's RL training pipeline (powered by [Atropos](https://github.com/Stuko0/atropos)).

1. [Quickstart](/getting-started/quickstart)
2. [Configuration](/user-guide/configuration)
3. [Atropos RL Environments](https://github.com/Stuko0/atropos) (external)
4. [Provider Routing](/user-guide/features/provider-routing)
5. [Architecture](/developer-guide/architecture)

:::tip
RL training works best when you already understand the basics of how Alice Agent handles conversations and tool calls. Run through the Beginner path first if you're new.
:::

### "I want to use it as a Python library"

Integrate Alice Agent into your own Python applications programmatically.
...
| **Code Execution** | Run Python scripts that call Alice native programmatically | [Code Execution](/user-guide/features/code-execution) |
