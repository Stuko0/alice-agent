...
description: "Install and deploy Alice Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
Alice Agent ships a Nix flake & a NixOS module.
...
**For non-NixOS users**, this only changes the install step. Everything after (`alice setup`, `alice gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage alice the same way you manage any other NixOS service.
:::
...
After `nix profile install`, `alice`, `alice-agent`, and `alice-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `alice setup` walks you through provider selection, `alice gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.alice/`.

:::warning Messaging platforms (Discord, Telegram, Slack)
The default package includes ALL libraries alice-agent might need. if you want a smaller variant, check the other flake outputs. 
...
# /etc/nixos/flake.nix (or your system flake)
{
  ...
  outputs = { nixpkgs, alice-agent, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        alice-agent.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
# configuration.nix
{ config, ... }: {
  services.alice-agent = {
    enable = true;
    settings.model.default = "anthropic/claude-sonnet-4";
    environmentFiles = [ config.sops.secrets."alice-env".path ];
    addToSystemPackages = true;
  };
}
```

That's it. `nixos-rebuild switch` creates the `alice` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o alice /dev/stdin /var/lib/alice/env
```

```nix
services.alice-agent.environmentFiles = [ "/var/lib/alice/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `alice` CLI on your system PATH **and** sets `ALICE_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `alice` in your shell creates a separate `~/.alice/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `alice` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.
...
Users listed in `hostUsers` are automatically added to the `alice` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo alice chat` manually.
:::
...
systemctl status alice-agent

# Watch logs (Ctrl+C to stop)
journalctl -u alice-agent -f

# If addToSystemPackages is true, test the CLI
alice version
alice config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Alice defaults to OpenRouter.
:::
...
```nix
services.alice-agent.configFile = /etc/alice/config.yaml;
```

This bypasses `settings` entirely — no merge, no generation. The file is copied as-is to `$ALICE_HOME/config.yaml` on each activation.
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."alice-env".path ]` |
| Give the agent a personality | `${services.alice-agent.stateDir}/.alice/SOUL.md` | manage the file directly |
...
| Override the alice package | `package` | `inputs.alice-agent.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/alice"` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$ALICE_HOME/.env` at activation time (`nixos-rebuild switch`). Alice reads this file on every startup, so changes take effect with a `systemctl restart alice-agent` — no container recreation needed.
...
```nix
{
  sops = {
    ...
    secrets."alice-env" = { format = "yaml"; };
  };

  services.alice-agent.environmentFiles = [
    config.sops.secrets."alice-env".path
  ];
}
```
...
```yaml
# secrets/alice.yaml (encrypted with sops)
alice-env: |
...
```nix
{
  age.secrets.alice-env.file = ./secrets/alice-env.age;

  services.alice-agent.environmentFiles = [
    config.age.secrets.alice-env.path
  ];
}
```
...
{
  services.alice-agent = {
    authFile = config.sops.secrets."alice/auth.json".path;
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Alice looks for specific filenames by convention:

- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: Alice loads its primary `SOUL.md` from `$ALICE_HOME/SOUL.md`, which in the NixOS module is `${services.alice-agent.stateDir}/.alice/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.

```nix
{
  services.alice-agent.documents = {
...
```nix
{
  services.alice-agent.mcpServers = {
    filesystem = {
      command = "npx";
      args = [ "-y" "@modelcontextprotocol/server-filesystem" "/data/workspace" ];
    };
...
:::tip
Environment variables in `env` values are resolved from `$ALICE_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
```nix
{
  services.alice-agent.mcpServers.remote-api = {
...
Set `auth = "oauth"` for servers using OAuth 2.1. Alice implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.

```nix
{
  services.alice-agent.mcpServers.my-oauth-server = {
...
Tokens are stored in `$ALICE_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.

<details>
<summary><strong>Initial OAuth authorization on headless servers</strong></summary>

The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Alice prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u alice` (native):

```bash
# Container mode
docker exec -it alice-agent \
  alice mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth

# Native mode
sudo -u alice ALICE_HOME=/var/lib/alice/.alice \
  alice mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
alice mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.alice/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/alice/.alice/mcp-tokens/
# Ensure: chown alice:alice, chmod 0600
```
...
```nix
{
  services.alice-agent.mcpServers.analysis = {
    command = "npx";
...
When alice runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `alice setup` | Config is declarative — edit `settings` in your Nix config |
| `alice config edit` | Config is generated from `settings` |
| `alice config set <key> <value>` | Config is generated from `settings` |
| `alice gateway install` | The systemd service is managed by NixOS |
| `alice gateway uninstall` | The systemd service is managed by NixOS |
...
When container mode is enabled, alice runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-alice-agent-0.1.0  ──►  /nix/store/... (ro)
~/.alice -> /var/lib/alice/.alice       (symlink bridge, per hostUsers)
/var/lib/alice/                    ──►  /data/          (rw)
...
The Nix-built binary works inside the Ubuntu container because `/nix/store` is bind-mounted — it brings its own interpreter and all dependencies, so there's no reliance on the container's system libraries. The container entrypoint resolves through a `current-package` symlink: `/data/current-package/bin/alice gateway run --replace`. On `nixos-rebuild switch`, only the symlink is updated — the container keeps running.

### What Persists Across What

| Event | Container recreated? | `/data` (state) | `/home/alice` | Writable layer (`apt`/`pip`/`npm`) |
|---|---|---|---|---|
| `systemctl restart alice-agent` | No | Persists | Persists | Persists |
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the alice package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/alice` is preserved (these are bind mounts).

If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/alice-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current alice package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `alice plugins install` needed.

### Directory Plugins (`extraPlugins`)

For plugins that are just a source tree with `plugin.yaml` + `__init__.py` (e.g., [alice-lcm](https://github.com/stephenschoettler/alice-lcm)):

```nix
services.alice-agent.extraPlugins = [
...
Plugins are symlinked into `$ALICE_HOME/plugins/` at activation time. Alice discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.

### Entry-Point Plugins (`extraPythonPackages`)

For pip-packaged plugins that register via `[project.entry-points."alice_agent.plugins"]` (e.g., [rtk-alice](https://github.com/ogallotti/rtk-alice)):

```nix
services.alice-agent.extraPythonPackages = [
...
The package's `site-packages` is added to PYTHONPATH in the alice wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in alice-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.

```nix
# Enable Discord, Telegram, Slack
services.alice-agent.extraDependencyGroups = [ "messaging" ];
```
...
```nix
{
  ...
  outputs = { alice-agent, nixpkgs, ... }: {
    ...
    # Then:
    #   pkgs.alice-agent.override { extraPythonPackages = [...]; }
    #   pkgs.alice-agent.override { extraDependencyGroups = [ "hindsight" ]; }
  };
}
```
...
```nix
services.alice-agent.settings.plugins.enabled = [
  "alice-lcm"
  "rtk-rewrite"
];
```
...
:::note
A build-time collision check prevents plugin packages from shadowing core alice dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the alice-agent service |
| `package` | `package` | `alice-agent` | The alice-agent package to use |
| `user` | `str` | `"alice"` | System user |
| `group` | `str` | `"alice"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/alice"` | State directory (`ALICE_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `alice` CLI to system PATH and set `ALICE_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$ALICE_HOME/.env` at activation time |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `alice gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the alice user's per-user profile so terminal commands, skills, and cron jobs all see them |
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$ALICE_HOME/plugins/`. Each must contain `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.alice` symlink to the service stateDir and are auto-added to the `alice` group |
...
```
/var/lib/alice/                     # stateDir (owned by alice:alice, 0750)
├── .alice/                         # ALICE_HOME
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Alice binary + all Nix deps |
| `/data` | `/var/lib/alice` | `rw` | All state, config, workspace |
| `/home/alice` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |
...
cd /etc/nixos && nix flake update alice-agent
...
```bash
systemctl status alice-agent
docker ps -a --filter name=alice-agent
docker inspect alice-agent --format='{{.State.Status}}'
docker exec -it alice-agent bash
docker exec alice-agent readlink /data/current-package
docker exec alice-agent cat /data/.container-identity
```
...
```bash
sudo systemctl stop alice-agent
docker rm -f alice-agent
sudo rm /var/lib/alice/.container-identity
sudo systemctl start alice-agent
```
...
```bash
# Native mode
sudo -u alice cat /var/lib/alice/.alice/.env

# Container mode
docker exec alice-agent cat /data/.alice/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec alice-agent readlink /data/current-package)
```
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u alice-agent` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
| Container recreated unexpectedly | `extraVolumes`, `extraOptions`, or `image` changed | Expected — writable layer resets. Reinstall packages or use a custom image |
| `alice version` shows old version | Container not restarted | `systemctl restart alice-agent` |
| Permission denied on `/var/lib/alice` | State dir is `0750 alice:alice` | Use `docker exec` or `sudo -u alice` |
| `nix-collect-garbage` removed alice | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "alice-agent"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
| `unable to find user alice` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart alice-agent` |
