...
description: "Run Alice Agent directly on an Android phone with Termux"
---

# Alice on Android with Termux
...
Alice Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Alice from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `alice` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `alice` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/alice" "$PREFIX/bin/alice"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `alice` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.alice/.env`.
...
### `alice doctor` says ripgrep or Node is missing
