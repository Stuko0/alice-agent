...
This tutorial walks you through setting up a Telegram bot powered by Alice Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Alice Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.alice/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Alice itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Alice Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_alice_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Alice Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Alice Gateway...
...
# View live logs
journalctl --user -u alice-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `alice gateway install` to update the plist.
:::
...
```bash
alice gateway stop && alice gateway start
```
...
alice pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.alice/.env`:
...
Control how much detail the bot shows when using tools. In `~/.alice/config.yaml`:
...
Customize how the bot communicates by editing `~/.alice/SOUL.md`:

For a full guide, see [Use SOUL.md with Alice](/guides/use-soul-with-alice).
...
```bash
# From the CLI
alice cron list          # View all scheduled jobs
alice cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u alice-gateway` (Linux) or `~/.alice/logs/gateway.log` (macOS) |
| Cron job output | `~/.alice/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.alice/cron/jobs.json` |
| Pairing data | `~/.alice/pairing/` |
| Session history | `~/.alice/sessions/` |
