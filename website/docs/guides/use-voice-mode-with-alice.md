---
sidebar_position: 8
title: "Use Voice Mode with Alice"
description: "A practical guide to setting up and using Alice voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Alice
...
- you want Alice sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Alice.
...
## Step 1: make sure normal Alice works first
...
Alice supports both local and cloud speech stacks.
...
### If you use `alice setup`

If you choose NeuTTS in the setup wizard, Alice checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Alice transcribes and responds
...
- asking Alice to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Alice loop.

## Tuning CLI behavior

### Silence threshold

If Alice starts/stops too aggressively, tune:
...
Alice stays a normal chat bot, but can speak replies.
...
- you want Alice to function like a portable research or ops assistant
...
Alice joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Alice detects speech boundaries
- transcripts are posted in the associated text channel
- Alice responds in text and audio
...
1. get text Alice working
2. install `alice-agent[voice]`
