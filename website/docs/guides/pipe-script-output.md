...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `alice send`."
---

# Pipe Script Output to Messaging Platforms

`alice send` is a small, scriptable CLI that pushes a message to any
messaging platform Alice is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | alice send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `alice
...
alice send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
alice send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
alice send --list

# Filter by platform
alice send --list telegram
```
...
Any platform Alice ships adapters for works as a target:
...
`alice send` resolves the message body in this order:

1. **Positional argument** — `alice send --to telegram "hi"`
2. **`--file PATH`** — `alice send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | alice send --to telegram`

When stdin is a TTY (no pipe), Alice does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  alice send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `alice send` reuses your Alice config, the same script works on
any host where Alice is installed — no need to export bot tokens into
...
`alice send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  alice send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | alice send \
...
```bash
./train.py --epochs 200 && \
  alice send --to telegram "training done" || \
  alice send --to telegram "training failed (exit $?)"
```
...
alice send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(alice send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `alice send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `alice send` calls
the platform's REST endpoint directly using credentials from
`~/.alice/.env` and `~/.alice/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `alice gateway start` and retry.
...
# Machine-readable
alice send --list --json
```

The listing is built from `~/.alice/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`alice gateway
...
| Approach | Multi-platform | Reuses Alice creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `alice send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`alice send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `alice send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `alice send` shares with cron delivery.
