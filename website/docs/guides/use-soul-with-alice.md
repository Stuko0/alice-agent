---
sidebar_position: 7
title: "Use SOUL.md with Alice"
description: "How to use SOUL.md to shape Alice Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Alice

`SOUL.md` is the **primary identity** for your Alice instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Alice to feel like the same assistant every time you talk to it — or if you want to replace the Alice persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Alice should be
- what Alice should avoid stylistically
- how Alice should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Alice is and how Alice speaks
...
Alice now uses only the global SOUL file for the current instance:

```text
~/.alice/SOUL.md
```

If you run Alice with a custom home directory, it becomes:
...
Alice automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Alice does not overwrite it
- if the file exists but is empty, Alice adds nothing from it to the prompt

## How Alice uses it

When Alice starts a session, it reads `SOUL.md` from `ALICE_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Alice feels.
...
Alice already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Alice should sound.

# Avoid
What Alice should not do.

# Defaults
How Alice should behave when ambiguity appears.
```
...
Then restart Alice or start a new session.
...
### I edited SOUL.md but Alice still sounds the same

Check:
- you edited `~/.alice/SOUL.md` or `$ALICE_HOME/SOUL.md`
...
### Alice is ignoring parts of my SOUL.md
