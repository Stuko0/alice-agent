---
sidebar_position: 6
title: "Use MCP with Alice"
description: "A practical guide to connecting MCP servers to Alice Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Alice

This guide shows how to actually use MCP with Alice Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Alice tool
- you want Alice to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Alice to internal APIs, databases, or company systems without modifying Alice core

Do not use MCP when:
- a built-in Alice tool already solves the job well
...
- Alice remains the agent
- MCP servers contribute tools
- Alice discovers those tools at startup or reload time
...
If you installed Alice with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Alice banner/status should show MCP integration when configured
- ask Alice what tools it has available
...
## WSL2: bridge Alice in WSL to Windows Chrome

This is the practical setup when:

- Alice runs inside WSL2
...
In this setup, Alice does **not** connect to Chrome directly. Instead:

- Alice runs in WSL
- Alice starts a local stdio MCP server
...
```text
Alice (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Alice stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Alice core browser transport
...
```bash
alice mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
alice mcp test chrome-devtools-win
```

Then start a fresh Alice session or run:
...
Once loaded, Alice can use the MCP-prefixed browser tools directly. For example:
...
If Alice runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Alice from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Alice from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Alice:
...
2. Alice-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Alice will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Alice to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Alice to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Alice remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
alice mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
alice mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `alice mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Alice to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Alice to merge, publish, deploy, or spawn runtimes.
...
Now Alice can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Alice core.
...
Because Alice now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
