...
alice cron create "0 2 * * *" \
  "You are a project manager triaging the Stuko0/alice-agent GitHub repo.

1. Run: gh issue list --repo Stuko0/alice-agent --state open --json number,title,labels,author,createdAt --limit 30
...
alice cron create "0 9 * * 1" \
  "Scan the Stuko0/alice-agent repo for documentation drift.

1. Run: gh pr list --repo Stuko0/alice-agent --state merged --json number,title,files,mergedAt --limit 30
