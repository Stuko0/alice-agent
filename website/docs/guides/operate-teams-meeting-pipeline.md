...
```bash
alice teams-pipeline token-health
alice teams-pipeline token-health --force-refresh
```
...
```bash
alice teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
alice teams-pipeline maintain-subscriptions
alice teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Alice cron (recommended if you already run the Alice gateway)

Alice ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.alice/scripts/`. First create the script:

```bash
mkdir -p ~/.alice/scripts
cat > ~/.alice/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec alice teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.alice/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
alice cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/alice-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Alice Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=alice
EnvironmentFile=/etc/alice/env
ExecStart=/usr/local/bin/alice teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/alice-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Alice Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now alice-teams-pipeline-maintain.timer
systemctl list-timers alice-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/alice teams-pipeline maintain-subscriptions >> /var/log/alice/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.alice/.env` at the top of a wrapper script that crontab calls.
...
```bash
alice teams-pipeline subscriptions   # should show expirationDateTime advanced
alice teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
alice teams-pipeline list
alice teams-pipeline list --status failed
alice teams-pipeline show <job-id>
```

### Replay a stored job

```bash
alice teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
alice teams-pipeline fetch --meeting-id <meeting-id>
alice teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `alice teams-pipeline maintain-subscriptions --dry-run`
- inspect `alice teams-pipeline list --status failed`
...
- run `alice teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `alice teams-pipeline run`
...
- [ ] `alice teams-pipeline validate` returns an OK snapshot
- [ ] `alice teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Alice cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
