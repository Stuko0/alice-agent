...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Alice Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Alice instantly when PRs are opened or updated.
:::
...
- **Alice Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  alice gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.alice/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Alice can access GitHub. Start a chat:
...
Still in the chat, ask Alice to review a real PR:
...
A skill gives Alice consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.alice/skills/code-review
```

Create `~/.alice/skills/code-review/SKILL.md`:
...
Verify it loaded — start `alice` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Alice your team's standards:
...
2. Teach Alice your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
alice gateway status    # Is the gateway running?
alice cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-alice-plugin)** — wrap the review logic into a shareable plugin
