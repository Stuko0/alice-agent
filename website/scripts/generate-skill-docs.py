...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/alice-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `alice skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Alice Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Alice ships with a large built-in skill library copied into `~/.alice/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Alice also syncs bundled skills on `alice update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.alice/skills/` tree, it is still shipped with Alice; restore it with `alice skills reset <name> --restore`.",
        "",
