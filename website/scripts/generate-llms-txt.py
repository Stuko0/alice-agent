#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Alice docs site.
...
Both publish at:
  https://alice-agent.stuko.dev/docs/llms.txt
  https://alice-agent.stuko.dev/docs/llms-full.txt
...
SITE_BASE = "https://alice-agent.stuko.dev/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Alice as a Python Library", None),
        ("guides/use-mcp-with-alice", "Use MCP with Alice", None),
        ("guides/use-voice-mode-with-alice", "Use Voice Mode with Alice", None),
        ("guides/use-soul-with-alice", "Use SOUL.md with Alice", None),
        ("guides/build-a-alice-plugin", "Build a Alice Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Alice"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Alice Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Stuko. A terminal-native "
    ...
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/Stuko0/"
        "alice-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/Stuko0/alice-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Alice Agent — Full Documentation\n",
        (
            "This file is the entire Alice Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Alice, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://alice-agent.stuko.dev/docs\n",
        "Short index: https://alice-agent.stuko.dev/docs/llms.txt\n",
        "\n---\n\n",
    ]
