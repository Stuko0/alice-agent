...
Uses a file-based lock (~/.alice/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `alice update` reloads
# the module) fail with ModuleNotFoundError for alice_time et al.
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy alice_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from alice_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``alice native`` config for the ``cron`` platform.
    ...
    try:
        from alice_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _get_alice_home_webhook() -> Path:
    ...
    profile-scoped gateway, so resolving the active ALICE_HOME at call time
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within ALICE_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Alice-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against ALICE_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    # escape — scripts MUST reside within ALICE_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        agent = AIAgent(
            ...
            # ALICE_HOME. When a workdir is configured, also inject project
