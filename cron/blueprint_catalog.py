...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``alice://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``alice://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"alice://blueprint/{quote(blueprint.key)}{qs}"
