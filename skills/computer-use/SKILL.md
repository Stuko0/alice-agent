...
Alice drives [cua-driver](https://github.com/trycua/cua) under the hood
for the platform plumbing. The Alice-side `computer_use` tool exposed
in this skill is a higher-level Alice vocabulary; the raw cua-driver
...
|---|---|
| `cua-driver not installed` | Run `alice computer-use install`, or `alice native` and enable Computer Use |
| Captures consistently return empty / "no on-screen window" | On Linux: DISPLAY may not be set (X11) or you're on pure Wayland — ask the user to run `alice computer-use doctor`. On Windows: you may be in Session 0 (SSH session) instead of the interactive desktop — see the cua-driver `WINDOWS.md` deep-dive |
...
| Type text disappears into a terminal emulator | cua-driver detects terminals (Ghostty, iTerm2, Terminal.app, Windows Terminal, mintty, etc.) and routes through key-event synthesis — should "just work" on a recent cua-driver. If it doesn't, ask the user to run `alice computer-use doctor` |
...
| Anything else weird | **First action: ask the user to run `alice computer-use doctor`.** It runs the cua-driver `health_report` MCP tool and prints a structured per-check matrix. Their output tells you (and them) exactly what's wrong |
...
Alice intentionally keeps THIS skill focused on the Alice-side
...
When `cua-driver skills install` autodetects Alice (planned follow-up
