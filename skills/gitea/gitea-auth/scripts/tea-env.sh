#!/usr/bin/env bash
# Gitea environment detection helper for Alice Agent skills.
#
...
elif _alice_env="${ALICE_HOME:-$HOME/.alice}/.env"; [ -f "$_alice_env" ] && grep -q "^GITEA_TOKEN=" "$_alice_env" 2>/dev/null; then
