...
`$ALICE_HOME/config.yaml` (defaults to `~/.alice/config.yaml` when `ALICE_HOME` is unset)
...
Restart the Alice session for changes to take effect. The MCP connection is
