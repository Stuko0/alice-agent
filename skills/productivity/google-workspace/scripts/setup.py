#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Alice Agent.
...
CLIENT_SECRET_PATH = ALICE_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ALICE_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Alice profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Alice Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Alice Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'alice-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Alice home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_alice_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Alice")
