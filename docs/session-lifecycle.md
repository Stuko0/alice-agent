...
The canonical transcript store is SQLite via `SessionDB` (from `alice_state`). The
...
Counts consecutive restarts via a JSON file (`{ALICE_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `alice update`, `alice gateway restart`,
or `/restart`.
