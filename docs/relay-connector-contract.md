...
This document is the formal interface between the **Alice gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `alice gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`alice gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `alice` CLI.
