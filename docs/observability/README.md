# Alice Observer Hooks

Alice observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Alice' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Alice catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Alice only builds
