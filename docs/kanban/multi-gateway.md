...
Alice supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.alice/config.yaml`:
...
Or set the env var: `ALICE_KANBAN_DISPATCH_IN_GATEWAY=false`
