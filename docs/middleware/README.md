...
Alice middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Alice'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Alice calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Alice retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Alice guardrails,
...
- `telemetry_schema_version`: currently `alice.observer.v1`
- `middleware_schema_version`: currently `alice.middleware.v1`
...
Alice stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Alice runs
them as a nested chain in registration order. Middleware failures are fail-open:
Alice logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Alice applies middleware in this order:
...
For each tool call, Alice applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Alice pre-execution path against the effective arguments:
...
```bash
alice plugins enable <plugin-name>
```

For isolated local testing, use one `ALICE_HOME` for plugin enablement and the
agent run:

```bash
export ALICE_HOME=/tmp/alice-middleware-test
mkdir -p "$ALICE_HOME"
alice plugins enable <plugin-name>
alice chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run alice plugins enable <plugin-name>
uv run alice chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["alice_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/alice-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Alice expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Alice treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Alice preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Alice does not convert downstream
