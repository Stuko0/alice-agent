...
When running Alice inside Docker, the default `network_mode: host` gives the
...
The Alice [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ALICE_UID=$(id -u) ALICE_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=alice,alice-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://alice-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Alice trust model and vulnerability reporting
