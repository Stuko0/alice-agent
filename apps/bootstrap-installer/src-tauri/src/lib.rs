...
/// desktop app handing off via `Alice-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under ALICE_HOME/logs/ so install
    ...
    // Alice is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Alice installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Alice is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Alice" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Alice" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_alice_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::alice_home().join("alice-agent");
                if bootstrap::alice_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_alice_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "alice already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update::start_update,
            // Hand-off
            bootstrap::launch_alice_desktop,
            // Diagnostics
            paths::get_log_path,
            paths::get_alice_home,
            paths::open_log_dir,
        ])
        .run(tauri::generate_context!())
        .expect("error while running Alice Setup");
}
