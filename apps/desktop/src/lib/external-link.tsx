...
const SKIP_PROTO_RE = /^(?:file|data|mailto|javascript|blob|chrome|about|alice):/i
...
export function fetchLinkTitle(url: string): Promise<string> {
  ...
  const bridge = typeof window === 'undefined' ? undefined : window.aliceDesktop?.fetchLinkTitle
...
export function openExternalLink(href: string): void {
  if (href) {
    void window.aliceDesktop?.openExternal?.(href)?.catch(() => {})
  }
}
