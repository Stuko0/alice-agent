...
// bug that drives the REAL useGatewayBoot hook + REAL AliceGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshAliceConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { aliceDesktop?: unknown }).aliceDesktop = fakeDesktop()
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForAlice) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startAlice()'s remote branch awaits waitForAlice() for 45s before it
    ...
    ;(window as { aliceDesktop?: unknown }).aliceDesktop = desktop
    ...
    // After ~45s waitForAlice gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Alice backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
