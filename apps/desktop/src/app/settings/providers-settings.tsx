...
import { disconnectOAuthProvider, listOAuthProviders } from '@/alice'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.aliceDesktop?.terminal)
...
function ConnectedProviderRow({
  ...
  // Alice can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Alice can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Alice never deletes creds another tool owns behind a silent API call.
