...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Alice doesn't know how to remove it. */
...
export interface ModelOptionProvider {
   ...
   *  the `alice model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
export interface ToolProvider {
   ...
   *  CLI `alice native` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Alice); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/alice/update/check` — the backend's own update state.
