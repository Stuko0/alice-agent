  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://alice-agent.stuko.dev/docs' },
    ...
    expect(view.title).toBe('Failed to open alice-agent.stuko.dev')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://alice-agent.stuko.dev/docs' },
        result: { success: true, url: 'https://alice-agent.stuko.dev/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened alice-agent.stuko.dev')
  })
})
