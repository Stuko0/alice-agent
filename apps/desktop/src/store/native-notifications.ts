...
const STORAGE_KEY = 'alice:native-notifications'
...
// "Backgrounded" = the user isn't on Alice. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.aliceDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.aliceDesktop
