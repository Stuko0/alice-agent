...
import type { BackendUpdateCheckResponse } from '@/types/alice'
...
const UPDATE_TOAST_SNOOZE_KEY = 'alice:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'alice:backend-skew-toast-snooze-until'
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `alice update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'alice update',
        command: result.command ?? 'alice update'
      })
...
export async function applyBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    if (!started.ok) {
      ...
      const command = (started as { update_command?: string }).update_command || 'alice update'
