...
import type { OAuthProvider } from '@/types/alice'
...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('alice-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('alice-desktop-onboarded-v1')).toBe('1')
  })
