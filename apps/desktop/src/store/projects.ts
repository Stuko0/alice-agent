...
import type { ProjectInfo, ProjectsPayload } from '@/types/alice'
...
const PROJECT_SCOPE_KEY = 'alice.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Alice gateway is not connected')
  }
...
// repo, returning where Alice should start working. Git is the source of
