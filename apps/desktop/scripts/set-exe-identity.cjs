#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Alice icon + version metadata onto the
// built Alice.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Alice.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `alice desktop`, the installer's
...
// path (which rebuilds via `alice desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Alice.exe>
//
...
// Stamp the Alice icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Alice',
      FileDescription: 'Alice',
      CompanyName: 'Stuko',
      LegalCopyright: 'Copyright (c) 2026 Stuko'
    }
  })

  console.log('[set-exe-identity] done — Alice icon + identity stamped')
}
