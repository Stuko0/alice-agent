 ...
 *
 * Stamps the Alice icon + identity onto the packed Windows Alice.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `alice desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Alice')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Alice'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Alice.exe keeps the stock Electron icon`)
  }
}
