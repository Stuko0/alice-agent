 ...
 * product name (`Alice`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Alice'
 *
 * This is a hard failure with no obvious cause for the user — `alice desktop`
 ...
 * (the mac-unpacked Alice.app bundle) and Windows (win-unpacked), so we
