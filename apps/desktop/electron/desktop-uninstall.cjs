...
/**
 * Map an uninstall mode to the `python -m alice_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `alice_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'alice_cli.uninstall', '--mode', mode]
}
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import alice_cli`
 ...
 * modes + `pythonPath`=agentRoot so `import alice_cli` resolves from source
