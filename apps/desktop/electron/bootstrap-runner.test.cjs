...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'alice-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/alice-runner-test',
    sourceRepoRoot: null,
    aliceHome: '/tmp/alice-runner-test',
    logRoot: '/tmp/alice-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'alice-agent', 'scripts')
