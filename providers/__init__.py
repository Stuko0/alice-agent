...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with alice-agent)
2. User plugins: ``$ALICE_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$ALICE_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$ALICE_HOME/plugins/model-providers/`` if it exists."""
    try:
        from alice_constants import get_alice_home

        d = get_alice_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple ALICE_HOME profiles don't alias each other.
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$ALICE_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with alice-agent.
    ...
    # 2. User plugins — under $ALICE_HOME/plugins/model-providers/<name>/.
