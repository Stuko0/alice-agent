...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `alice native` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    # Coding posture (base Alice — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in alice_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Alice nativeets (CLI + messaging platforms)
    #
    ...
    # the `alice send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "alice-cron": {
        # Mirrors alice-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `alice native` filters
    ...
    "alice-gateway": {
        ...
        "tools": [],
        "includes": ["alice-telegram", "alice-discord", "alice-whatsapp", "alice-slack", "alice-signal", "alice-bluebubbles", "alice-homeassistant", "alice-email", "alice-sms", "alice-mattermost", "alice-matrix", "alice-dingtalk", "alice-feishu", "alice-wecom", "alice-wecom-callback", "alice-weixin", "alice-qqbot", "alice-webhook", "alice-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    ...
    Bundle nesting is one level deep in practice (only ``alice-gateway``
