...
row taxonomy is inferred from the atlas shape so Alice can render both legacy
...
# rows (1536x1872), while older Alice/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Alice' activity state names. They are not always identical to the
...
# Legacy Alice/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Alice activity names -> accepted row-name aliases in descending
