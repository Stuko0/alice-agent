"""Unified removal contract for every credential source Alice reads from.

Alice seeds its credential pool from many places:
    ...
    manual        — user ran `alice auth add`
...
unify here is **removal**:
    ...
    ``alice auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.alice/.env  → clear it
    ...
    """
    from alice_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Alice.",
            f"  The pool entry is now suppressed — Alice will ignore "
            f"{env_var} until you run `alice auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Alice stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `alice auth add anthropic` to re-enable if needed.",
    ])
...
def _remove_alice_pkce(provider: str, removed) -> RemovalResult:
    ...
    result = RemovalResult()
    oauth_file = get_alice_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Alice Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `alice auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `alice auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``alice auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``alice auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `alice model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Alice auth store is not enough — _seed_from_singletons() would
    ...
    via ``alice auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `alice auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `alice auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Alice stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `alice auth add copilot` to re-enable if needed.",
    ])
