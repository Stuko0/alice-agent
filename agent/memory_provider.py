...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `alice backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - alice_home (str): The active ALICE_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.alice``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "alice").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'alice memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], alice_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'alice memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``alice_home`` is the active ALICE_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE ALICE_HOME.

        ``alice backup`` only walks ALICE_HOME, so any provider state kept
        ...
        of the archive, and ``alice import`` restores them to their original
