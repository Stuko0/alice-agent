...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Alice starts and finds ``~/.openclaw/``.

    Points users at ``alice claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``alice claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Alice, run "
        "`alice claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `alice claw cleanup` (renames it to ~/.openclaw.pre-migration — "
