...
upstream prefix cache warm.  See ``alice-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Alice never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ALICE_HOME persona while keeping
    ...
    # Pointer to the alice-agent skill + docs for user questions about Alice itself.
    stable_parts.append(ALICE_AGENT_HELP_GUIDANCE)
    ...
    # ALICE_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Alice, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Alice profile the agent is running
    # under so it doesn't conflate ~/.alice/skills/ (default profile) with
    # ~/.alice/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Alice profile: default. Other profiles (if any) live "
            "under ~/.alice/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Alice profile: {active_profile}. This session reads "
            f"and writes ~/.alice/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.alice/skills/, ~/.alice/plugins/, "
            f"~/.alice/cron/, ~/.alice/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Alice never rebuilds or reinjects parts of it
