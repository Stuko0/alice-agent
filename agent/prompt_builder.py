...
from alice_constants import get_alice_home, get_skills_dir, is_wsl
...
def _find_alice_md(cwd: Path) -> Optional[Path]:
    ...
    # could pick up a .alice.md planted in /tmp, /home, etc.
...
ALICE_AGENT_HELP_GUIDANCE = (
    "You run on Alice Agent (by Stuko). When the user needs help with "
    "Alice itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://alice-agent.stuko.dev/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `alice-agent` skill with skill_view(name='alice-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "every assignee in a real profile (`alice profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `alice kanban <verb>` for board operations. Use "
...
#
# The alice-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to alice-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `alice computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Alice "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Alice terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the Alice WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Alice itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Alice runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Alice itself is running. The host OS, home, and cwd "
                f"of the Alice process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Alice "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Alice
...
def _skills_prompt_snapshot_path() -> Path:
    return get_alice_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.alice/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Alice Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `alice-agent` skill "
            "first. It has the actual commands (e.g. `alice config set …`, `alice native`, "
            "`alice setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Nous Portal integration removed — no subscription prompt section.

    Kept as a no-op returning an empty string so existing callers
    (``agent/system_prompt.py``, ``run_agent.py``) keep resolving.
    """
    return ""
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from ALICE_HOME and return its content, or None.
    ...
    soul_path = get_alice_home() / "SOUL.md"
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .alice.md / ALICE.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ALICE_HOME is independent and always included when present.
    ...
    # SOUL.md from ALICE_HOME only — skip when already loaded as identity
    if not skip_soul:
