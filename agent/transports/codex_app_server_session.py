...
Owns one Codex thread per Alice session. Drives `turn/start`, consumes
...
# Alice' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ALICE_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Alice/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Alice session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ALICE_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ALICE_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Alice' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Alice' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own alice-tools callback we
            # auto-accept — the user already approved Alice' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Alice approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Alice returns 'once', 'session', 'always', or 'deny'.
