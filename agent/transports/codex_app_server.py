...
projection into Alice' display, approval bridging, transcript projection into
...
"codex_app_server"`. Alice' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("ALICE_KANBAN_TASK"):
            kanban_db = spawn_env.get("ALICE_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ALICE_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ALICE_HOME", os.path.expanduser("~/.alice")),
                        "kanban",
                    ),
                )
            )
