...
as ``kind: backend``) or ``~/.alice/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``alice native``
...
def _videos_cache_dir() -> Path:
    """Return ``$ALICE_HOME/cache/videos/``, creating parents as needed."""
    from alice_constants import get_alice_home

    path = get_alice_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ALICE_HOME/cache/videos/``.
