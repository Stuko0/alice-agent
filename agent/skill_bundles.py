...
Storage
-------
Bundles live in ``~/.alice/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ALICE_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``alice bundles``)
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `alice bundles` CLI subcommand.
# ---------------------------------------------------------------------------
