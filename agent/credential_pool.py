...
def _write_through_provider_state_to_global_root(
    ...
    ``alice_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``alice model`` / ``alice auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Alice process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``alice`` invocation that defaults to the active provider
        (e.g. setup wizard, ``alice auth status``) would land on
        ...
        try:
            with _auth_store_lock():
                ...
                # alice_cli.auth._save_xai_oauth_tokens (#43589); the pool
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("alice_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Alice
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("alice_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Alice process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Alice profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `alice model` / `alice auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `alice model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``alice auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `alice auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `alice auth remove <provider> <N>` is stable across all source types.
    try:
        from alice_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Alice PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `alice setup` ("Claude
        ...
        # Claude Code CLI, alice_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.alice/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `alice setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"alice_pkce", "claude_code"}
            ]
        ...
        for source_name, creds in (
            ("alice_pkce", read_alice_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `alice auth add nous --label <name>`).  Fall back to the
        ...
        # Alice auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from alice_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.alice/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `alice auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `alice auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Alice auth store.  Without this gate the removal is instantly
        ...
        # Alice owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``alice model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``alice auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.alice/.env over os.environ — the user's config file is the
    # authoritative source for Alice credentials. Stale env vars from parent
    ...
    # Honour user suppression — `alice auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.alice/.env.
    ...
    try:
        from alice_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from alice_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.alice/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `alice auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Alice
