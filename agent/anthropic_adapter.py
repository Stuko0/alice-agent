"""Anthropic Messages API adapter for Alice Agent.

Translates between Alice's internal OpenAI-style message format and
...
# Alice effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to alice's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to alice's outer conversation
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Alice notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Alice historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``alice models``) never mutates ``~/.alice/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Alice)
      2. CLAUDE_CODE_OAUTH_TOKEN env var
      3. Claude Code credentials (~/.claude.json or ~/.claude/.credentials.json)
         — with automatic refresh if expired and a refresh token is available
      4. Anthropic credential_pool OAuth entry (~/.alice/auth.json)
    ...
    # 1. Alice-managed OAuth/setup token env var
    ...
    # 4. Alice credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration Alice configs.
...
# ── Alice-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.alice/.anthropic_oauth.json (our own file).
...
def run_alice_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print()
    print("Authorize Alice with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from alice_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_alice_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if _ALICE_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read Alice OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See alice-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See alice-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See alice-agent#13848 (Kimi) and
    alice-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Alice Agent", "Claude Code")
                text = text.replace("Alice Agent", "Claude Code")
                text = text.replace("alice-agent", "claude-code")
                text = text.replace("Stuko", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Alice-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Alice surfaces in its CLI. We
