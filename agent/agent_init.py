...
from alice_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: alice config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .alice.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / ALICE_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.alice/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from alice_cli.config import load_config as _load_pc_cfg
    ...
    if agent.verbose_logging:
        ...
        # on the fact that alice_logging.setup_logging() does not
        ...
        # handler level inside alice_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from alice_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from alice_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `alice model` to "
                        "select a provider, or run `alice setup` for first-time "
                        "configuration."
                    )
    ...
    try:
        from alice_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from alice_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "alice"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Alice' built-in
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from alice_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from alice_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Alice Agent.  Choose a model with at least "
