...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Alice also honours ALICE_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("ALICE_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_alice_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Alice Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Alice from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Alice' system prompt, but mapping
...
def _get_context_cache_path() -> Path:
    ...
    from alice_constants import get_alice_home
    return get_alice_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Alice grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from alice_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Alice sends to providers:
