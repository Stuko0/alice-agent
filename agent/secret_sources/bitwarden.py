...
Alice pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.alice/.env``.
...
* The access token is stored in ``~/.alice/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``alice`` invocations don't
  hammer the API.
* Failures NEVER block Alice startup.  Missing binary, no network,
...
# In-process cache so repeated load_alice_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `alice chat -q ...`
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    if home_path is None:
        home_path = Path(os.getenv("ALICE_HOME", Path.home() / ".alice"))
...
def install_bws(*, force: bool = False) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="alice-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
# ---------------------------------------------------------------------------
# Public entry point — called from alice_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_alice_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `alice secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `alice secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `alice secrets bitwarden setup` to install."
        )
        return result
