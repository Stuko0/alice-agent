...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Alice path.

    Three categories are blocked:

      * Internal Alice cache files under ``ALICE_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under ALICE_HOME and the global Alice
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.alice/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active ALICE_HOME (profile-aware) AND the global
    # Alice root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in alice_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Alice cache file "
    ...
    for hd in alice_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Alice credential store "
    ...
    for hd in alice_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Alice MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Alice MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Alice profiles are separate ALICE_HOME dirs under
...
#
# Reference: May 2026 incident where a alice-security profile session
# edited skills under both ``~/.alice/profiles/alice-security/skills/``
# AND ``~/.alice/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under ALICE_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from ALICE_HOME.
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Alice scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Alice entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Alice profile {info['target_profile']!r}, but the "
...
#
#   <ALICE_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.alice/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.alice/…`` segment and warns
# regardless of which Alice profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.alice/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.alice`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.alice/…`` and returns the
    index where the inner Alice-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .alice / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".alice":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Alice state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.alice``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.alice`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Alice resolver to
    succeed, so it works correctly even when called from contexts where
    ALICE_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
    ...
    mirror_root = Path(*parts[: inner_idx + 1])
    inner_path = (
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Alice process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real ALICE_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.alice/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.alice/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host ALICE_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Alice state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Alice "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real ALICE_HOME. Use the "
