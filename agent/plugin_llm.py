...
Plugins built on Alice Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Alice
...
:class:`~alice_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Alice' main config
...
handles every provider, fallback chain, and per-task override Alice
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`alice_cli.plugins.PluginContext`
