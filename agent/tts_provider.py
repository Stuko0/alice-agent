...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Alice with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.alice/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Alice
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``alice native``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``alice setup`` for
