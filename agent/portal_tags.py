...
Every Alice request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Alice Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=alice-agent",
        "client=alice-client-v<__version__>",
    ]

The version is sourced live from ``alice_cli.__version__`` so it auto-aligns
...
``alice_cli.__version__`` is the canonical source of truth.
"""
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=alice-agent", alice_client_tag()]
