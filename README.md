...
Use any model you want — OpenRouter, OpenAI, your own endpoint, and [many others](https://alice-agent.stuko.dev/en/docs/providers/). Switch with `alice model` — no code changes, no lock-in.
...
> **Heads up:** Native Windows runs Alice without WSL — CLI, gateway, TUI, and tools all work natively. If you'd rather use WSL2, the Linux/macOS one-liner above works there too. Found a bug? Please [file issues]([https://github.com/Stuko0/alice-agent/]).
...
The installer handles everything: uv, Python 3.11, Node.js, ripgrep, ffmpeg, **and a portable Git Bash** (MinGit, unpacked to `%LOCALAPPDATA%\alice\git` — no admin required, completely isolated from any system Git install). Alice uses this bundled Git Bash to run shell commands.
...
If your antivirus (Bitdefender, Windows Defender, etc.) quarantines `uv.exe` from the Alice `bin` folder (`%LOCALAPPDATA%\alice\bin\uv.exe`), this is a **false positive**. The file is Astral's `uv` — the Rust Python package manager Alice bundles to manage its Python environment. ML-based antivirus engines commonly flag unsigned Rust binaries that download and install packages.
...
# Run verification
$uv = "$env:LOCALAPPDATA\alice\bin\uv.exe"
...
**To whitelist Alice:**
- **Windows Defender:** Run PowerShell as Admin → `Add-MpPreference -ExclusionPath "$env:LOCALAPPDATA\alice\bin"`
- **Bitdefender:** Add an exception in the Bitdefender console (Protection > Antivirus > Settings > Manage Exceptions)
- Whitelist the **folder**, not the file hash — Alice updates `uv` and the hash changes every version
...
```bash
alice              # Interactive CLI — start a conversation
alice model        # Choose your LLM provider and model
alice native        # Configure which tools are enabled
alice config set   # Set individual config values
alice gateway      # Start the messaging gateway (Telegram, Discord, etc.)
alice setup        # Run the full setup wizard (configures everything at once)
alice claw migrate # Migrate from OpenClaw (if coming from OpenClaw)
alice update       # Update to the latest version
alice doctor       # Diagnose any issues
```
...
Alice has two entry points: start the terminal UI with `alice`, or run the gateway and talk to it from Telegram, Discord, Slack, WhatsApp, Signal, or Email. Once you're in a conversation, many slash commands are shared across both interfaces.

| Action                         | CLI                                           | Messaging platforms                                                              |
| ------------------------------ | --------------------------------------------- | -------------------------------------------------------------------------------- |
| Start chatting                 | `alice`                                      | Run `alice gateway setup` + `alice gateway start`, then send the bot a message |
...
All documentation lives at **[alice-agent.stuko.dev/en/docs](https://alice-agent.stuko.dev/en/docs/)**:
...
If you're coming from OpenClaw, Alice can automatically import your settings, memories, skills, and API keys.

**During first-time setup:** The setup wizard (`alice setup`) automatically detects `~/.openclaw` and offers to migrate before configuration begins.

**Anytime after install:**

```bash
alice claw migrate              # Interactive migration (full preset)
...
- **Skills** — user-created skills → `~/.alice/skills/openclaw-imports/`
...
See `alice claw migrate --help` for all options, or use the `openclaw-migration` skill for an interactive agent-guided migration with dry-run previews.

---

## Contributing

We welcome contributions! See the [Contributing Guide](https://alice-agent.stuko.dev/en/docs/developer-guide/contributing) for development setup, code style, and PR process.

Quick start for contributors — use the standard installer, then work from the
full git checkout it creates at `$ALICE_HOME/alice-agent` (usually
`~/.alice/alice-agent`). This matches the layout used by `alice update`, the
