"""ACP session manager — maps ACP sessions to Alice AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.alice/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Alice itself is running in WSL.

    Windows ACP clients can launch ``alice acp`` inside WSL while still sending
    ...
    """
    from alice_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Alice from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["alice-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Alice Agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Alice AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Alice runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.alice/state.db``) is lazily created.
        """
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Alice daemon's process cwd.
