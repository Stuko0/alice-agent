"""ACP auth helpers — detect and advertise Alice authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "alice-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Alice runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Alice can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Alice.
    ...
    not have Alice provider credentials configured yet, so Alice always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Alice using the currently configured "
