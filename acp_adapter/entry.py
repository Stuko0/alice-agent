"""CLI entry point for the alice-agent ACP adapter.

Loads environment variables from ``~/.alice/.env``, configures logging
...
try:
    ...
    # shadowing Alice's own modules — ``alice acp`` can be started from any
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="alice-acp",
        description="Run Alice Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Alice version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Alice provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.alice/node/ "
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``alice postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting alice-agent ACP adapter")
