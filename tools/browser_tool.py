...
from alice_constants import agent_browser_runnable, get_alice_home
from utils import env_int, is_truthy_value
from alice_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Alice secret
...
# plugins drop in under ``~/.alice/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.alice/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Alice silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Alice CLI where sys.stdout and sys.stderr both report
...
def _emergency_cleanup_all_sessions():
    ...
    crashed alice processes — this way every clean alice exit sweeps
    ...
    # Sweep orphans from other crashed alice processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live alice processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current alice PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live alice process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning alice process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced alice PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent alice instances won't reap each
         other's daemons.
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live alice process.
            continue
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Alice-managed
    ...
    # node_modules binary, which disappears on the next ``alice update`` and
    ...
    # Build an extended search PATH including Alice-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_alice_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_alice_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_alice_home() / "node")),
            ]
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this alice PID as the session owner (cross-process safe
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Alice
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/stuk0o/alice-agent:latest")
                else:
