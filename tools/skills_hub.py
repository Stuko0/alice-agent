#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Alice Skills Hub.
...
Used by alice_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _load_alice_index() -> Optional[dict]:
    ...
    try:
        ...
        logger.debug("Alice index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class LydiaIndexSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "alice-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "alice-index"),
            identifier=entry.get("identifier", ""),
