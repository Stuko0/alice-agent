    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Alice Agent"
"""
...
from alice_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__alice_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses ALICE_HOME so each profile gets its own OAuth tokens.
    Layout: ``ALICE_HOME/mcp-tokens/``
    """
    try:
        from alice_constants import get_alice_home
        base = Path(get_alice_home())
    except ImportError:
        base = Path(os.environ.get("ALICE_HOME", str(Path.home() / ".alice")))
...
class AliceTokenStorage:
    ...
    File layout::
        ...
        ALICE_HOME/mcp-tokens/<server_name>.json         -- tokens
        ALICE_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ALICE_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Alice records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Alice.</p></body></html>"
            ) if code else (
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `alice mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Alice Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`alice mcp login {server_name}` interactively first to complete "
