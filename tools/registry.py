"""Central registry for all alice-agent tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``alice native``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``alice native enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("alice_plugins."):
            return mod
        return None
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``alice native enable foo``)
