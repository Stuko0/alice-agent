...
provider** — turn on a backend by enabling a plugin (``alice plugins
enable video_gen/<name>``) and selecting it in ``alice native`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/alice-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `alice native` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `alice native` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `alice native` → "
...
# `alice native` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`alice native` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `alice native` → Video Generation."
        )
        return {"description": "\n".join(parts)}
