...
# via shell expansions like $HOME or $ALICE_HOME, or by the resolved absolute
# active profile home path such as /home/alice/.alice/config.yaml. The
# resolved-absolute form is folded into the ~/.alice/ patterns at detection
...
DANGEROUS_PATTERNS = [
    ...
    # that should always require user consent, just like `alice gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart alice-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart alice-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(alice|gateway|cli\.py)\b', "kill alice/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill alice` but not
    # `kill -9 $(pgrep -f alice)` because the substitution is opaque
    ...
    # the `alice gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.alice.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(alice|ai\.alice)\b', "stop/restart alice launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Alice file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.alice/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Alice-managed security file (~/.alice/config.yaml or
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.alice/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / ALICE_HOME even when those are set after this module is
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / ALICE_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.alice``); the matched
...
def _rewrite_resolved_alice_home_webhook(command: str) -> str:
    ...
    ``$ALICE_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from alice_constants import get_alice_home
        home = get_alice_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.alice")
