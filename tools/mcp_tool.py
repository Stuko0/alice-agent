...
transport, discovers their tools, and registers them into the alice-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.alice/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.alice/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            candidates = [
                ...
                # image (which the Alice Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Alice MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Alice gateways know how to render.
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Alice' existing approval
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'alice-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'alice-agent[all]'"
            )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Alice' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Alice is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `alice mcp login {server_name}` (or delete the tokens "
            f"file under ~/.alice/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local ALICE_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local ALICE_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_alice_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Alice config file.
    ...
    ``os.environ`` (which includes ``~/.alice/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Alice' MEDIA: tag convention and out to messaging
            ...
            # Alice' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Alice's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Alice registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``alice native`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Alice registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple alice
