...
`mcp_tool.py`, and `alice_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# AliceMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------
...
def _make_alice_provider_class() -> Optional[type]:
    ...
    class AliceMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`AliceTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, AliceTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import AliceTokenStorage
                        if isinstance(storage, AliceTokenStorage):
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import AliceTokenStorage
            if not isinstance(storage, AliceTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``alice mcp reauth``.
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`AliceMCPOAuthProvider` directly using the helpers
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `alice mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``alice mcp remove <name>`` and (indirectly) by
        ``alice mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
