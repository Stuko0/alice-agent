...
def iter_sync_files(container_base: str = "/root/.alice") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.alice to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync_back(self, lydia_home: Path | None = None) -> None:
        ...
        # against an uninitialized remote .alice/ directory.
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="alice-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.alice/skills/a.md`` →
        ``~/.alice/skills/a.md``, a new remote file at
        ``/root/.alice/skills/b.md`` maps to ``~/.alice/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
