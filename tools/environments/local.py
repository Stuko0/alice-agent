...
# Bedrock-specific bearer token, which is a Alice inference secret exactly
...
# project's dependencies into the Alice venv path instead of the project's own
# ``.venv`` — silently clobbering the Alice environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Alice venv stays reachable via PATH (its bin dir is first), so stripping
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Alice-managed secrets from a subprocess environment."""
    try:
...
def _resolve_alice_bin_dir() -> str | None:
    ...
    alice install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``alice``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``alice`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``alice`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("alice")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``alice`` executable (covers nix-store / venv wrappers).
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "alice" or base.startswith("alice."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "alice.exe" if _IS_WINDOWS else "alice"
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Alice trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``ALICE_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under ALICE_HOME.  Using
