...
Lets the LLM write a Python script that calls Alice native via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `alice_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `alice_tools.py` with file-based RPC stubs
...
# available on every platform Alice itself runs on.
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Alice-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Alice-profile soft guard."""',
...
"""Auto-generated Alice native RPC stubs."""
...
def _execute_remote(
    ...
    The script and the generated alice_tools.py module are shipped to
...
def execute_code(
    ...
    try:
        # Write the auto-generated alice_tools module.
        ...
        # Ensure the alice-agent root is importable in the sandbox so
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.alice/.env')).
...
def _load_config() -> dict:
    ...
    ``alice --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (alice-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``alice native`` (e.g. web is turned off),
    ...
    # temp-dir staging and alice-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.alice/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Alice native programmatically. "
        ...
        f"Available via `from alice_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into alice_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from alice_tools import {import_str}` "
