...
Backend is selected during ``alice native`` setup (web.backend in config.yaml).
When available, Alice can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Alice config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Alice' config/.env layer (``alice config set``, ``alice native``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.alice/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``alice native``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `alice native` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `alice native` to set one up."
                ),
            }
        else:
            logger.info(
