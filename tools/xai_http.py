...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``alice native`` repaint, tool-registration scans,
    ...
    2. ``~/.alice/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_alice_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.alice/.env`` first, then ``os.environ``.

    Wraps :func:`alice_cli.config.get_env_value` so tests can patch
...
def alice_xai_user_agent() -> str:
    ...
    try:
        ...
        __version__ = "unknown"
    return f"Alice-Agent/{__version__}"


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Alice config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Alice home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_alice_home() / "state"
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    Prefers Alice-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``alice_cli.config.get_env_value`` so keys
    stored in ``~/.alice/.env`` (the standard Alice location) are honored —
