...
`alice skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.alice.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.alice.blueprint`` is a mapping containing
    ...
    alice = meta.get("alice") if isinstance(meta, dict) else None
    blueprint = alice.get("blueprint") if isinstance(alice, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.alice.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.alice.blueprint`` block) they can hand
    to ``alice skills publish`` to share. ``body`` is the plain-language
