...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.alice",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``ALICE_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside ALICE_HOME so that a malicious
    ...
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to ALICE_HOME)",
            relative_path,
        )
        return False
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.alice/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="alice-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching alice_constants.get_alice_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_alice_dir()`` for backward compatibility with old directory layouts.
    """
    from alice_constants import get_alice_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_alice_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.alice``,
    SSH ``<remote_home>/.alice``, etc.) and whether translation is wanted.
