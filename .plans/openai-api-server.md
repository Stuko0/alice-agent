# OpenAI-Compatible API Server for Alice Agent
...
SSE streaming. By exposing this endpoint, alice-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  alice-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.alice/.env`
2. Run `alice gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with alice-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns alice-agent as a model) |
...
Authorization: Bearer alice-api-key-here
...
But alice-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "alice-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured alice-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of alice-agent's core prompt
...
Once implemented, alice-agent works as a drop-in backend for:
