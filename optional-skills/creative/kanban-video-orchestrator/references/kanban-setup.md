...
the actual `setup.sh` that creates the project workspace, configures Alice
...
2. **Create profiles** — `alice profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.alice/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `alice kanban create` assigned to the director
...
```bash
alice profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.alice/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.alice/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Alice
...
Each profile gets a `SOUL.md` at `~/.alice/profiles/<name>/SOUL.md` that
...
```bash
alice kanban create "Direct production of <video title>" \
    --assignee director \
...
the Alice `.env` (`${ALICE_HOME:-$HOME/.alice}/.env`) and macOS Keychain
(if on macOS):
...
check_key ELEVENLABS_API_KEY alice ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY alice OPENROUTER_API_KEY || exit 1
# ...
```
