...
Maps each role archetype to the Alice skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public alice-agent
...
## Alice skills relevant to video production

### Visual / rendering skills (`alice-agent/skills/creative/`)
...
### Design / pre-production skills (`alice-agent/skills/creative/`)
...
### Audio / media skills (`alice-agent/skills/creative/` + `skills/media/`)
...
These are **not** Alice skills but external CLIs / APIs that profiles invoke.
...
## Built-in Alice native for media review

These are native Alice native — not invoked via terminal but through their own
...
key is present in `${ALICE_HOME:-~/.alice}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Alice profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ALICE_HOME:-~/.alice}/.env` → environment variable.
...
When a new Alice-public video skill ships:
