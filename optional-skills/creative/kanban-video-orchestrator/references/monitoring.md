...
alice kanban watch --tenant <project-slug>

# Snapshot of the board
alice kanban list --tenant <project-slug>
alice kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
alice kanban stats --tenant <project-slug>

# Visual dashboard (browser)
alice dashboard

# Inspect a specific task (includes comments + events)
alice kanban show <task-id>

# Follow a single task's event stream
alice kanban tail <task-id>
```

Verify available subcommands with `alice kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `alice kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `alice kanban block <id>` then `alice kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `alice kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `alice kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `alice profile list` (profile exists?), `alice status` (gateway/dispatcher up?), and verify tenant. |
...
alice kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
alice kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(alice kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
alice kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
alice kanban block <task-id>
alice kanban archive <task-id>

# Diagnose what happened
alice kanban show <task-id>      # task body, comments, recent events
alice kanban tail <task-id>      # follow the live event stream
alice kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `alice kanban create` for the director
...
while true; do
    clear
    alice kanban list --tenant <slug>
    echo "---"
    alice kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `alice kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `alice profile list` and start
