...
Usage (via cron with --no-agent):
    ...
    alice cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ALICE_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Alice-Watcher/1.0"})
