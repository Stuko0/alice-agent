...
Usage (via cron with --no-agent):
    ...
    alice cron create alice-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ALICE_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name alice-issues --repo Stuko/alice-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Alice .env file
(``${ALICE_HOME:-~/.alice}/.env``) to avoid the 60 req/hr
