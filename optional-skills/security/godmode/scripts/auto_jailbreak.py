...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ALICE_HOME", os.path.expanduser("~/.alice")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ALICE_HOME", Path.home() / ".alice")) / "skills" / "red-teaming" / "godmode"
...
ALICE_HOME = Path(os.getenv("ALICE_HOME", Path.home() / ".alice"))
CONFIG_PATH = ALICE_HOME / "config.yaml"
PREFILL_PATH = ALICE_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Alice models — already uncensored, just needs clean prompt
    "alice": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "alice" in model_lower or "nous" in model_lower:
        return "alice"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Alice config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.alice/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Alice for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Alice for changes to take effect.")
