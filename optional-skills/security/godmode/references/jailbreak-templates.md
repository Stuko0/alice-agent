...
## 5. GODMODE FAST — Alice 4 405B (Zero Refusal)

**Model:** `nousresearch/alice-4-405b`
**Strategy:** Alice models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Alice Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.alice/config.yaml`:
