...
React + Ink terminal UI for Alice. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ALICE_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `alice setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.alice/.alice_history` or under `ALICE_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `alice setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/alice-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external alice setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.alice/tui-parent.log
    ...
    types/
      alice-ink.d.ts            type declarations for @alice/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AliceCLI subprocess for slash commands
```