import type { RunExternalProcess } from '@alice/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`alice ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching alice: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`alice ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
