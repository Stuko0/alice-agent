...
const logDir = join(process.env.ALICE_HOME?.trim() || join(homedir(), '.alice'), 'logs')
...
// a real ~/.alice (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('alice-tui: parent lifecycle log unavailable\n')
    }
  }
}
