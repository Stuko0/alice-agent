...
const ENV_KEYS = ['ALICE_AUTO_HEAPDUMP', 'ALICE_HEAPDUMP_DIR', 'ALICE_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'alice-heapdump-test-'))
    process.env.ALICE_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without ALICE_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without ALICE_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when ALICE_AUTO_HEAPDUMP=1', async () => {
    process.env.ALICE_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.ALICE_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of ALICE_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.ALICE_HEAPDUMP_DIR
    savedMax = process.env.ALICE_HEAPDUMP_MAX_BYTES
    delete process.env.ALICE_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'alice-heapdump-prune-'))
    process.env.ALICE_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.ALICE_HEAPDUMP_DIR
    } else {
      process.env.ALICE_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.ALICE_HEAPDUMP_MAX_BYTES
    } else {
      process.env.ALICE_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.ALICE_HEAPDUMP_MAX_BYTES = String(2 * 1024)
