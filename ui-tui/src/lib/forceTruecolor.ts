...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ALICE_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Alice on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ALICE_TUI_TRUECOLOR=1.
