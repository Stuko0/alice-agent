...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate ALICE_HOME for each test.

    The global hermetic fixture already redirects ALICE_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    ALICE_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "alice_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["alice_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_alice_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_alice_home(self, _isolate_env):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/etc/passwd")) is False

    def test_accepts_tmp_alice_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/alice-abc/x.log")) is True
...
class TestTrackForgetQuick:
    ...
    def test_quick_does_not_descend_into_protected_top_level_dirs(
        ...
        dg = _load_lib()
        protected_empty = (
            _isolate_env / "alice-agent" / "node_modules" / "pkg" / "empty"
        )
        ...
        def guarded_iterdir(path):
            if path == _isolate_env / "alice-agent":
                raise AssertionError("quick() descended into protected alice-agent/")
...
class TestBundledDiscovery:
    def _write_enabled_config(self, alice_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from alice_cli import plugins as pmod
