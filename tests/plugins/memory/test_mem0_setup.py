...
def _inject_fake_alice_cli(monkeypatch):
    """Inject fake alice_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("alice_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("alice_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_alice_cli = types.ModuleType("alice_cli")
    fake_alice_cli.config = fake_config_mod
    fake_alice_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "alice_cli", fake_alice_cli)
    monkeypatch.setitem(sys.modules, "alice_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "alice_cli.memory_setup", fake_setup_mod)
...
class TestPostSetup:
    ...
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["alice", "--mode", "platform", "--api-key", "sk-test"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_alice_home", lambda: tmp_path)
        _inject_fake_alice_cli(monkeypatch)
        config = {"memory": {}}
    ...
    def test_oss_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "alice", "--mode", "oss", "--oss-llm-key", "sk-oai",
        ])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_alice_home", lambda: tmp_path)
        _inject_fake_alice_cli(monkeypatch)
...
class TestDryRun:
    ...
    def test_dry_run_platform_no_files(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["alice", "--mode", "platform", "--api-key", "sk-test", "--dry-run"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_alice_home", lambda: tmp_path)
        _inject_fake_alice_cli(monkeypatch)
        config = {"memory": {}}
        post_setup(str(tmp_path), config)
    ...
    def test_dry_run_oss_no_files(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "alice", "--mode", "oss", "--oss-llm-key", "sk-oai", "--dry-run",
        ])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_alice_home", lambda: tmp_path)
        _inject_fake_alice_cli(monkeypatch)
        monkeypatch.setattr("plugins.memory.mem0._setup._install_provider_deps", lambda l, e, v: None)
