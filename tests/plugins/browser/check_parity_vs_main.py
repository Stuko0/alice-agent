...
Run from the PR worktree:
    ...
    cd ~/.alice/alice-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.alice/alice-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.alice/alice-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a alice-agent checkout"
)
...
# Isolated ALICE_HOME for the config write.
