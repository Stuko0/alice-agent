...
The ``alice update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m alice_cli.main gateway run`` — image name
    ``pythonw``, not ``alice.exe`` — so the ``taskkill /IM alice.exe`` guard
    ...
    # The alice.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM alice.exe' in text
