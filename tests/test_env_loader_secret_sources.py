"""Tests for the secret-source tracking in ``alice_cli.env_loader``.

These cover the small public surface that lets `alice model` / `alice setup`
...
from alice_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    ...
    runs exactly once per ALICE_HOME per process.
    """
    ...
    # cli.py, alice_cli/main.py, run_agent.py, trajectory_compressor.py,
