...
``alice_tools.py`` and ``script.py`` into a temp dir, and those files
...
# ``alice_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``alice_tools.py`` and ``script.py`` writes in
...
# let the child import alice_tools, a user script that printed non-ASCII
