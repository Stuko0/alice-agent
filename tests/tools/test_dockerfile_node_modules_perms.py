...
Older images made /opt/alice/ui-tui and /opt/alice/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ALICE_TUI_DIR=/opt/alice/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R alice:alice /opt/alice/ui-tui" not in text
    assert "chown -R alice:alice /opt/alice/node_modules" not in text
