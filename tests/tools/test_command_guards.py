...
def _clean_state():
    ...
    saved = {}
    for k in ("ALICE_INTERACTIVE", "ALICE_GATEWAY_SESSION", "ALICE_EXEC_ASK", "ALICE_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ALICE_INTERACTIVE"] = "1"
        session_key = os.getenv("ALICE_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ALICE_INTERACTIVE or ALICE_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
