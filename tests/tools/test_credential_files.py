...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        alice_home.mkdir()
        (alice_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(alice_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.alice/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        alice_home = tmp_path / ".alice"
        alice_home.mkdir()
        (alice_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        alice_home = tmp_path / ".alice"
        alice_home.mkdir()
        (alice_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        alice_home = tmp_path / ".alice"
        alice_home.mkdir()
        (alice_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.alice/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        alice_home = tmp_path / ".alice"
        (alice_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ALICE_HOME": str(alice_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.alice")

        assert mounts[0]["container_path"] == "/home/user/.alice/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.alice/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.alice/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ALICE_HOME."""
        monkeypatch.setenv("ALICE_HOME", str(tmp_path / ".alice"))
        (tmp_path / ".alice").mkdir()

        # Create a sensitive file one level above alice_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside alice_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ALICE_HOME must still be registered."""
        alice_home = tmp_path / ".alice"
        alice_home.mkdir()
        monkeypatch.setenv("ALICE_HOME", str(alice_home))
        (alice_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_alice_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ALICE_HOME must be allowed."""
        alice_home = tmp_path / ".alice"
        alice_home.mkdir()
        subdir = alice_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ALICE_HOME pointing outside must be rejected."""
        alice_home = tmp_path / ".alice"
        alice_home.mkdir()
        monkeypatch.setenv("ALICE_HOME", str(alice_home))

        # Create a sensitive file outside alice_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside alice_home pointing outside
        symlink = alice_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ALICE_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, alice_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ALICE_HOME."""
        alice_home = tmp_path / ".alice"
        alice_home.mkdir()
        monkeypatch.setenv("ALICE_HOME", str(alice_home))
        ...
        sensitive.write_text("{}")
        self._write_config(alice_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(alice_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ALICE_HOME via config must still mount."""
        alice_home = tmp_path / ".alice"
        alice_home.mkdir()
        monkeypatch.setenv("ALICE_HOME", str(alice_home))

        (alice_home / "oauth.json").write_text("{}")
        self._write_config(alice_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        alice_home = tmp_path / ".alice"
        alice_home.mkdir()
        (alice_home / "cache" / "documents").mkdir(parents=True)
        (alice_home / "cache" / "audio").mkdir(parents=True)
        (alice_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("ALICE_HOME", str(alice_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.alice/cache/documents" in paths
        assert "/root/.alice/cache/audio" in paths
        assert "/root/.alice/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.alice/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_alice_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        alice_home = tmp_path / ".alice"
        alice_home.mkdir()
        # Use legacy dir name with content — get_alice_dir prefers
        # populated old over new.
        legacy_doc = alice_home / "document_cache"
        legacy_img = alice_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(alice_home / "document_cache") in host_paths
        assert str(alice_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.alice/cache/documents" in container_paths
        assert "/root/.alice/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        alice_home = tmp_path / ".alice"
        img_dir = alice_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.alice/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.alice")
            == "/home/agent/.alice/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        alice_home = tmp_path / ".alice"
        (alice_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("ALICE_HOME", str(alice_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(alice_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        alice_home = tmp_path / ".alice"
        doc_dir = alice_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        alice_home = tmp_path / ".alice"
        ss_dir = alice_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.alice/cache/screenshots/session_abc/screen1.png"
