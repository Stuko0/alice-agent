"""Contract tests for the Docker image's immutable /opt/alice install tree."""
...
def test_dockerfile_makes_opt_alice_readonly_for_alice_user() -> None:
    ...
    assert "chown -R root:root /opt/alice" not in text
    assert "chmod -R a+rX /opt/alice" not in text
    assert "chmod -R a-w /opt/alice" not in text
...
def test_dockerfile_does_not_chown_install_trees_to_alice() -> None:
    ...
    forbidden_patterns = (
        ...
        r"chown\s+-R\s+alice:alice\s+/opt/alice/ui-tui",
        r"chown\s+-R\s+alice:alice\s+/opt/alice/gateway",
        r"chown\s+-R\s+alice:alice\s+/opt/alice/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/alice must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/alice/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $ALICE_HOME data volume (which a host install may also use).
    The stamp is created by root in the shim-wiring RUN block; the alice
    user can't modify it (go-w from the --chmod on the source COPY).
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/alice/.install_method" in text

    # The stamp must be in the RUN block that wires the exec shim.
    shim_block = re.search(
        r"RUN mkdir -p /opt/alice/bin && \\\n"
        r"(?:.*\\\n)+?"
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    assert '"$ALICE_HOME/lazy-packages"' in stage2, (
