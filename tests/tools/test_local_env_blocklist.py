...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_lydia_bin_injection(self):
        ...
        TestLydiaBinDirOnPath) so a real ``alice`` on the test runner's PATH
...
class TestLydiaBinDirOnPath:
    ...
    def test_resolves_via_which(self, monkeypatch):
        ...
        assert local_mod._resolve_lydia_bin_dir() == "/opt/alice/bin"
    ...
    def test_prepend_adds_missing_dir_at_front(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._LYDIA_BIN_DIR = "/opt/alice/bin"
