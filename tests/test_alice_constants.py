"""Tests for alice_constants module."""
...
class TestGetAliceHome:
    ...
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When ALICE_HOME is unset on Windows, use %LOCALAPPDATA%\\alice."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ALICE_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(alice_constants.sys, "platform", "win32")
        monkeypatch.setattr(alice_constants, "_profile_fallback_warned", False)
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Alice-managed npm/node wrappers."""
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(alice_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(alice_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".alice"
