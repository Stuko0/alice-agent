...
def alice_home(tmp_path, monkeypatch):
    ...
    # /etc/alice on the dev/CI box can't influence the test.
    monkeypatch.setenv("ALICE_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(alice_home, monkeypatch):
    from alice_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(alice_home, monkeypatch):
    from alice_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(alice_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_alice_dotenv(alice_home=str(home))
