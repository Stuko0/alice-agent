...
alice_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `alice config`/`doctor`
...
def _load_cli_config(home):
    ...
    this only matters for tests that swap ALICE_HOME.)
    """
    import cli
