"""Tests for IRC gateway configuration via `alice setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Alice install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Alice install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#alice")
            monkeypatch.setenv("IRC_NICKNAME", "alice-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `alice setup gateway` experience with IRC present."""
