"""Tests for issue #26670 — concurrent alice.exe detection and improved
quarantine retry / reboot-deferred fallback during `alice update` on Windows.
...
def _make_proc(pid: int, exe: str, name: str = "alice.exe"):
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "alice.exe").write_bytes(b"")
    (scripts_dir / "alice-gateway.exe").write_bytes(b"")
...
def test_detect_concurrent_finds_other_alice_process(_winp, tmp_path):
    ...
    procs = [
        _make_proc(other_pid, str(shim), "alice.exe"),
...
def test_detect_concurrent_matches_case_insensitively(_winp, tmp_path):
    ...
    # Simulate the desktop spawning alice.EXE (uppercase ext) from same path
    upper = str(shim).replace("alice.exe", "ALICE.EXE")
...
def test_detect_concurrent_excludes_parent_chain(_winp, tmp_path):
    ...
    Simulates the real Windows topology: alice.exe launcher (PID L) spawns
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "alice.exe"),
    ]
...
def test_detect_concurrent_still_finds_unrelated_other_alice(_winp, tmp_path):
    ...
    sibling_pid = me + 200  # an UNRELATED alice.exe (must still be reported)

    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(launcher_pid, str(shim), "alice.exe"),
        _make_proc(sibling_pid, str(shim), "alice.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
        parent_chain=[launcher_pid],
        proc_iter_rows=rows,
...
def test_detect_concurrent_parent_chain_walks_deep(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(parent_pid, str(shim), "alice.exe"),
        _make_proc(grandparent_pid, str(shim), "alice.exe"),
        _make_proc(greatgrandparent_pid, str(shim), "alice.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
...
def test_detect_concurrent_parent_walk_handles_stub_without_process(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "alice.exe"),
        _make_proc(other_pid, str(shim), "alice.exe"),
    ]
...
def test_format_message_mentions_pids_and_remediation(tmp_path):
    matches = [(1234, "alice.exe"), (5678, "alice.exe")]
    msg = cli_main._format_concurrent_instances_message(matches, tmp_path)

    assert "1234" in msg
    assert "5678" in msg
    assert "alice.exe" in msg
    assert "Alice Desktop" in msg
    assert "--force" in msg
    # Mentions the file that would have been overwritten
    assert str(tmp_path / "alice.exe") in msg
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    assert orig == shim
    assert quarantine.name.startswith("alice.exe.old.")
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "Alice Desktop" in captured or "gateway" in captured.lower()
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "alice_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "alice_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_resume_windows_gateways_after_update_respawns_unmapped_by_cmdline(
    ...
    scheduled_argv = ["pythonw.exe", "-m", "alice_cli.main", "gateway", "run"]
    token = {
        "resume_needed": True,
        "profiles": {},
        "unmapped_pids": [7560],
...
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another alice.exe is running, the update bails out before
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    detect = MagicMock(return_value=[(9, "alice.exe")])
