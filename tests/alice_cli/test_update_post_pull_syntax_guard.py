"""Tests for the post-pull syntax guard in ``alice update``.

When a bad commit lands on ``main`` with a syntax error in a critical file
(e.g. orphan merge-conflict markers in ``alice_cli/config.py``), the CLI
becomes unbootable — every ``alice`` invocation imports those files at
...
markers in ``alice_cli/config.py``; users who ran ``alice update`` in
the 7-minute window before #28458 landed could not run any ``alice``
command afterward.
"""
...
# release; if a future ``alice update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
...
def test_production_tree_passes_syntax_guard():
    ...
    assert ok is True, (
        ...
        f"alice update would brick users. Error: {error}"
    )
