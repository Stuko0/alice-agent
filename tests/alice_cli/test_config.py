"""Tests for alice_cli configuration management."""
...
class TestGetLydiaHome:
    ...
    def test_env_override(self):
        with patch.dict(os.environ, {"ALICE_HOME": "/custom/path"}):
            home = get_alice_home()
...
class TestEnsureLydiaHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from alice_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from alice_cli.default_soul import _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``alice logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from alice_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"ALICE_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="alice_cli.config"):
                config = load_config()
            ...
            # stands out at alice startup before logging is configured)
            captured = capsys.readouterr()
            assert "alice config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"ALICE_HOME": str(tmp_path)}):
            ...
            assert "alice config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"ALICE_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ALICE_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ALICE_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"ALICE_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ALICE_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ALICE_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from alice_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `alice setup` surface it (previously only Honcho was listed, leaving
...
class TestEnvWriteDenylist:
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Alice
    process on next startup via the dotenv → ``os.environ`` chain in
    ``alice_cli/env_loader.py``.
