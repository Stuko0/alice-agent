...
- Bare `alice setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `alice setup --quick` runs the narrower "fill in missing items" flow.
- `alice setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ...
        ("alice_cli.setup.save_config", {}),
        ("alice_cli.setup.get_env_value", {"return_value": None}),
        ("alice_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("alice_cli.setup._print_setup_summary", {}),
        ("alice_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("alice_cli.setup.load_config", {"return_value": {}}),
        ("alice_cli.setup.save_config", {}),
        ("alice_cli.auth.get_active_provider", {"return_value": None}),
        ("alice_cli.setup.get_env_value", {"return_value": None}),
        ("alice_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `alice setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="alice_cli.setup.prompt_choice",
                quick="alice_cli.setup._run_quick_setup",
                model="alice_cli.setup.setup_model_provider",
                terminal="alice_cli.setup.setup_terminal_backend",
                agent="alice_cli.setup.setup_agent_settings",
                gateway="alice_cli.setup.setup_gateway",
                tools="alice_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`alice setup --reconfigure` behaves the same as bare `alice setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("alice_cli.setup.prompt_choice", {"return_value": 0}),
                first="alice_cli.setup._run_first_time_quick_setup",
            )
            from alice_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "alice_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["alice", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["alice", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
