"""Tests for alice_cli/goals.py — persistent cross-turn goals."""
...
def alice_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves ALICE_HOME.
...
class TestGoalManager:
    ...
    def test_continuation_prompt_shape(self, alice_home):
        ...
        mgr.set("port goal command to alice")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to alice" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from alice_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_consecutive_parse_failures_persists_across_goalmanager_reloads(
        ...
        from alice_cli import goals
        from alice_cli.goals import GoalManager, load_goal
...
class TestGoalContractSerialization:
    def test_roundtrip_with_contract(self):
        from alice_cli.goals import GoalState, GoalContract
