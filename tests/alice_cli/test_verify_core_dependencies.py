...
base deps) during ``alice update``, leaving the venv in a broken state
...
def temp_pyproject(tmp_path, monkeypatch):
    """Point alice_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    def test_no_action_when_all_deps_present(self, temp_pyproject, fake_venv_python):
        ...
        with patch("alice_cli.main._resolve_install_target_python", return_value=py), \
             patch("alice_cli.main.subprocess.run") as mock_run, \
             patch("alice_cli.main._run_install_with_heartbeat") as mock_install:
    ...
    def test_triggers_reinstall_when_dep_missing(self, temp_pyproject, fake_venv_python):
        ...
        with patch("alice_cli.main._resolve_install_target_python", return_value=py), \
             patch("alice_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("alice_cli.main._run_install_with_heartbeat") as mock_install:
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("alice_cli.main._resolve_install_target_python") as mock_resolve, \
             patch("alice_cli.main._run_install_with_heartbeat") as mock_install:
            from alice_cli.main import _verify_core_dependencies_installed
    ...
    def test_repair_reinstall_quarantines_running_shim_on_windows(
        ...
        quarantine the running ``alice.exe`` on Windows before installing.
        ...
        is left missing and ``alice`` drops off PATH. Previously this path
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `alice update`.
        ...
        with patch("alice_cli.main._is_windows", return_value=True):
            from alice_cli.main import _resolve_install_target_python
