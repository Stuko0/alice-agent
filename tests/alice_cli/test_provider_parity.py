...
the SAME provider universe as ``alice model`` (the CLI/TUI picker).
...
def test_every_alice_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `alice model` universe."""
    ...
    assert not missing, (
        "providers shown in `alice model` but not configurable in the desktop "
