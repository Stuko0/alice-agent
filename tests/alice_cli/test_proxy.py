"""Tests for the `alice proxy` subcommand and its upstream adapters."""
...
from alice_cli.proxy.adapters import ADAPTERS, get_adapter
from alice_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
...
def test_registry_no_longer_lists_nous():
    # The Nous Portal integration was removed; the stub module stays importable
    # but must NOT be a registered proxy upstream anymore.
...
def test_get_adapter_raises_for_removed_nous_provider():
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("alice_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("alice_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from alice_cli.proxy.server import create_app  # noqa: E402
...
def test_server_forwards_chat_completions():
    async def run():
        ...
        try:
            ...
            assert "Alice-4-70B" in req["body"]
        finally:
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("ALICE_HOME", str(tmp_path))
    from alice_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from alice_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from alice_cli.proxy.cli import cmd_proxy_start
...
def test_cmd_proxy_start_refuses_removed_nous_provider(capsys, tmp_path, monkeypatch):
    # The Nous Portal upstream was removed: starting a nous proxy is an
    # unknown-provider error, not an auth prompt.
    monkeypatch.setenv("ALICE_HOME", str(tmp_path))
    from alice_cli.proxy.cli import cmd_proxy_start
    ...
    err = capsys.readouterr().err
    assert "nous" in err
    assert "alice auth add nous" not in err
