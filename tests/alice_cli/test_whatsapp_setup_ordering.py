...
Before the fix, ``alice whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``alice gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ALICE_HOME", str(alice))
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from alice_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("alice_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``alice whatsapp`` with an existing paired session and
