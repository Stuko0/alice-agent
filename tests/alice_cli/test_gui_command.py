"""Tests for ``alice gui`` desktop launcher wiring."""
...
def _make_packaged_executable(root: Path, monkeypatch, platform: str = "darwin") -> Path:
    ...
    desktop_dir = root / "apps" / "desktop"
    if platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Alice.app" / "Contents" / "MacOS" / "Alice"
    elif platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Alice.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "alice"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("alice_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("alice_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("alice_cli.main._desktop_build_needed", return_value=True), \
...
def test_gui_forwards_desktop_environment_overrides(tmp_path, monkeypatch):
    ...
    with patch("alice_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("alice_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("alice_cli.main._desktop_build_needed", return_value=True), \
         patch("alice_cli.main._write_desktop_build_stamp"), \
         patch("alice_cli.main._desktop_macos_relaunchable_fixup"), \
...
def test_gui_linux_falls_back_to_no_sandbox_when_userns_is_restricted(tmp_path, monkeypatch):
    ...
    with patch("alice_cli.main._desktop_linux_sandbox_fixup", return_value=False), \
...
def test_desktop_build_stamp_skips_build_when_up_to_date(tmp_path, monkeypatch):
    ...
    with patch("alice_cli.main._desktop_build_needed", return_value=False), \
         patch("alice_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("alice_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         patch("alice_cli.main._desktop_macos_relaunchable_fixup"), \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_force_build_overrides_stamp(tmp_path, monkeypatch):
    ...
    with patch("alice_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("alice_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("alice_cli.main._desktop_build_needed", return_value=False), \
...
def test_compute_desktop_content_hash_stable(tmp_path, monkeypatch):
    ...
    (root / "package.json").write_text('{"name":"alice"}', encoding="utf-8")
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    packaged_exe = root / "apps" / "desktop" / "release" / "linux-unpacked" / "alice"
    ...
    with patch("alice_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("alice_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("alice_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("alice_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("alice_cli.main._write_desktop_build_stamp"), \
         patch("alice_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("alice_cli.main._electron_dist_ok", return_value=False), \
...
def test_gui_redownloads_electron_via_mirror_then_repacks(tmp_path, monkeypatch, capsys):
    ...
    with patch("alice_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("alice_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("alice_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("alice_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("alice_cli.main._electron_dist_ok", return_value=False), \
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    Alice.app/Contents/MacOS/Alice in place. Re-downloading Electron can't
...
def test_redownload_electron_dist_missing_installer(tmp_path, monkeypatch):
    ...
    with patch("alice_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("alice_cli.main.subprocess.run") as mock_run:
...
def test_redownload_electron_dist_runs_installer_with_mirror(tmp_path, monkeypatch):
    ...
    with patch("alice_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("alice_cli.main.subprocess.run", side_effect=fake_run):
...
def test_redownload_electron_dist_returns_false_when_download_fails(tmp_path, monkeypatch):
    ...
    with patch("alice_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("alice_cli.main.subprocess.run",
...
def test_stop_desktop_build_lock_terminates_only_release_procs(tmp_path, monkeypatch):
    ...
    locker_exe = release / "Alice.exe"
    locker_exe.write_text("", encoding="utf-8")
    other_exe = tmp_path / "elsewhere" / "Alice.exe"
...
def test_desktop_launch_options_survives_config_error():
    with patch("alice_cli.config.load_config", side_effect=RuntimeError("boom")):
        flags, gpu = cli_main._desktop_launch_options()
    assert flags == []
    assert gpu == "auto"
