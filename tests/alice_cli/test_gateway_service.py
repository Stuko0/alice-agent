...
import alice_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "alice gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`alice gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `alice gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `alice gateway restart`)."""
        unit_path = tmp_path / "alice-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``ALICE_HOME`` into the unit's
        ...
        # Realistic generated unit referencing a pytest tmpdir ALICE_HOME
        polluted_unit = (
            "[Service]\n"
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E ALICE_HOME like
        ``/tmp/alice-e2e-41264`` carries none of the pytest markers but
        poisons the unit identically (seen live 2026-06-11 — an E2E probe ran
        ``alice gateway restart`` with a /tmp ALICE_HOME exported; the
        ...
        must refuse ANY temp-dir ALICE_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "alice-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="ALICE_HOME=/tmp/alice-e2e-41264"\n'
            "WorkingDirectory=/tmp/alice-e2e-41264\n"
        )
        monkeypatch.setattr(
            gateway_cli,
            "generate_systemd_unit",
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="ALICE_HOME=/var/tmp/alice-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="ALICE_HOME={tmp_path}/alice-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None

    def test_detects_tmp_home_in_launchd_plist(self):
        plist = (
            "<dict>\n  <key>ALICE_HOME</key>\n"
            "  <string>/tmp/alice-e2e-99999</string>\n</dict>\n"
        )
        assert (
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="ALICE_HOME=/tmpfs-data/.alice"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "alice gateway install" in out
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.alice.gateway.plist"
        ...
        # ALICE_HOME resolves under the (pytest tmp) test ALICE_HOME.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist>--replace\n<key>ALICE_HOME</key>"
                "<string>/Users/alice/.alice</string></plist>"
            ),
        )
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test ALICE_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>ALICE_HOME</key>"
                "<string>/Users/alice/.alice</string></plist>"
            ),
        )
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup alice gateway run" in out
    ...
    def test_launchd_unsupported_marker_write_and_clear(self, tmp_path, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_alice_home", lambda: tmp_path)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: alice gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup alice gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.alice.gateway"])
            ),
        )
...
class TestSystemUnitLydiaHome:
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'ALICE_HOME=/home/alice/.alice' in unit
        assert '/root/.alice' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: ALICE_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
    ...
    def test_system_unit_preserves_custom_lydia_home(self, monkeypatch):
        ...
        assert 'ALICE_HOME=/opt/alice-shared' in unit
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "alice gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.alice/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".alice" / "profiles" / "mybot"
    ...
    def test_named_profile_under_target_user_root_returns_flag(self, tmp_path):
        ...
        target_root = tmp_path / "home" / "alice" / ".alice"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile ALICE_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "alice"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.alice/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".alice" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".alice" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_alice_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_profile = root_home / ".alice" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("ALICE_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_alice_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
        ...
        assert f'ALICE_HOME={target_home / ".alice" / "profiles" / "mybot"}' in unit
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".alice" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.alice.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".alice" / "alice-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".alice" / "alice-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/alice", str(tmp_path / "alice"))
        assert result == "/opt/alice"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".alice" / "alice-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".alice" / "alice-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("ALICE_HOME", str(root_home / ".alice"))
        monkeypatch.setattr(gateway_cli, "get_alice_home", lambda: root_home / ".alice")
        ...
        # WorkingDirectory is anchored at the target user's ALICE_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.alice" in unit
        assert "WorkingDirectory=/home/alice/.alice/alice-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "alice-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "alice-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'alice gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'alice gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'alice gateway start' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyLydiaUnitDetection:
    ...
    renamed to ``alice-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``alice.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``alice-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Alice Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m alice_cli.main gateway run --replace\n"
    )
    ...
    def test_ignores_profile_unit_lydia_gateway_coder(self, tmp_path, monkeypatch):
        ...
        for base in (user_dir, system_dir):
            (base / "alice-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "alice-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "alice-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
    ...
    def test_ignores_unrelated_lydia_service(self, tmp_path, monkeypatch):
        ...
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "alice.service").write_text(
            "[Unit]\nDescription=Some Other Alice\n[Service]\n"
            "ExecStart=/opt/other-alice/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m alice_cli.main gateway run
          - python path/to/alice_cli/main.py gateway run
          - alice gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m alice_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/alice/alice_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/alice gateway run --replace",
            "ExecStart=/venv/bin/python /opt/alice/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"alice.service" if i == 0 else f"alice.service"  # same name
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "alice.service" in out
        assert "alice gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "alice.service"
...
class TestRemoveLegacyLydiaUnits:
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop alice.service" in c for c in cmds_joined)
        assert any("--user disable alice.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo alice gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop alice.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable alice.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "alice.service"
        system_legacy = system_dir / "alice.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (alice-gateway-coder.service)
        ...
        profile_unit = user_dir / "alice-gateway-coder.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `alice gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import alice_cli.main as cli_main
        ...
        # public, just confirm that `alice gateway --help` shows it.
        import subprocess
        import sys
        ...
        result = subprocess.run(
            [sys.executable, "-m", "alice_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "alice_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "alice-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "alice-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "alice-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``alice gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "alice-gateway")
        ...
        assert "sudo systemctl start alice-gateway" in out
        assert "sudo alice gateway uninstall --system" in out
        assert "alice gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart alice-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop alice-gateway" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``alice gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (ALICE_HOME), never the source checkout / worktree, so a relocated or
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # ALICE_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".alice"
