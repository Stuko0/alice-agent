"""Tests for xAI Grok OAuth — tokens stored in Alice auth store (~/.alice/auth.json)."""
...
def test_xai_oauth_authorize_url_includes_referrer_alice_agent():
    """Attribution: xAI's OAuth server can identify Alice-originated logins
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "alice-agent"
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "alice auth add xai-oauth" in body
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("alice_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    monkeypatch.setattr("alice_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    monkeypatch.setattr("alice_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``alice model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``alice model``
    ...
    assert "alice model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from alice_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Alice that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("alice_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `alice model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `alice auth list` reflects truth and
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`alice auth remove xai-oauth <N>` for the seeded entry suppresses
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``alice auth remove xai-oauth 1`` for a
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Alice invocation."
    )
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("alice_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("alice_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Alice process refreshed the singleton
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``alice model`` -> xAI Grok OAuth (or another process
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``alice auth add xai-oauth``) own their own refresh-token lifecycle
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`alice auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``alice auth status``, ``alice setup``, and the
    ``alice`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
