"""Tests for alice backup and import commands."""
...
def _make_alice_tree(root: Path) -> None:
    ...
    (root / "alice_state.db").write_bytes(b"fake-state")
    ...
    # alice-agent repo (should be EXCLUDED)
    (root / "alice-agent").mkdir(exist_ok=True)
    (root / "alice-agent" / "run_agent.py").write_text("# big file\n")
    (root / "alice-agent" / ".git").mkdir()
    (root / "alice-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_alice_agent(self):
        from alice_cli.backup import _should_exclude
        assert _should_exclude(Path("alice-agent/run_agent.py"))
        assert _should_exclude(Path("alice-agent/.git/HEAD"))
    ...
    def test_includes_nested_alice_agent_in_skills(self):
        """skills/autonomous-ai-agents/alice-agent/ must NOT be excluded —
        only the root-level alice-agent/ repo is skipped."""
        from alice_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/alice-agent/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/alice-agent/sub/item.txt"))
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under ALICE_HOME must be skipped —
...
class TestBackup:
    ...
    def test_excludes_alice_agent(self, tmp_path, monkeypatch):
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "alice-agent" in n]
            assert agent_files == [], f"alice-agent files leaked into backup: {agent_files}"
    ...
    def test_includes_nested_alice_agent_in_skills(self, tmp_path, monkeypatch):
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root alice-agent must be excluded
            root_agent = [n for n in names if n.startswith("alice-agent/")]
            assert root_agent == [], f"root alice-agent leaked: {root_agent}"
            # Nested skill alice-agent must be included
            assert "skills/autonomous-ai-agents/alice-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/alice-agent/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        ...
        zips = list(tmp_path.glob("alice-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Alice backup."""
        from alice_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only alice_state.db (old wrong name) is rejected."""
        from alice_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["alice_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_strips_alice_prefix(self, tmp_path, monkeypatch):
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".alice/config.yaml": "model: test\n",
            ".alice/skills/a/SKILL.md": "# A\n",
        })
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        from alice_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".alice"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("ALICE_HOME", str(dst_home))
        ...
        # alice-agent should NOT be present
        assert not (dst_home / "alice-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from alice_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without alice markers fails validation."""
        import io
        from alice_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_alice(self):
        ...
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".alice/config.yaml", "test")
            zf.writestr(".alice/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".alice/"
...
class TestBackupEdgeCases:
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("alice-backup-*.zip"))
        assert len(zips) == 1
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert "alice -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from alice_cli.backup import _safe_copy_db
...
# ---------------------------------------------------------------------------
# Pre-update backup (alice update safety net)
# ---------------------------------------------------------------------------
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_in_quick_state_files(self):
        from alice_cli.backup import _QUICK_STATE_FILES
...
class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``alice update``
    ...
    def test_backup_contents_match_full_backup(self, alice_home):
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # alice-agent repo excluded
        assert not any(n.startswith("alice-agent/") for n in names)
...
class TestRunPreUpdateBackup:
    ...
    def alice_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for alice_cli.config + alice_constants so they pick up ALICE_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("alice_cli.config") or mod == "alice_constants":
    ...
    def test_config_enabled_creates_backup(self, alice_home, capsys):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("alice_cli.config"):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (alice claw migrate safety net)
# ---------------------------------------------------------------------------
...
class TestRestoreCronJobsIfEmptied:
    """`alice update` config migration can leave cron/jobs.json valid-but-empty,
...
# location, NOT under ALICE_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------
...
class TestMemoryProviderExternalPaths:
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under ALICE_HOME,
