...
When ``ALICE_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.alice/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.alice            (has its own auth.json fixture)
    * Profile     -> tmp_path/.alice/profiles/coder   (active, ALICE_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Alice is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from alice_cli.auth import _load_auth_store, _load_provider_state
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ALICE_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.alice as the "real home" — which is NOT equal
    # to the ALICE_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    lydia_home = tmp_path / "classic"
