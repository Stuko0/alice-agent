"""Tests for the top-level `./alice` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./alice` should use `alice_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "alice"
    called = []

    fake_main_module = types.ModuleType("alice_cli.main")

    def fake_main():
        called.append("alice_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "alice_cli.main", fake_main_module)
