...
from alice_cli.auth import resolve_provider
...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from alice_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # alice model, not via upgrade prompts.
...
class TestGmiModelCatalog:
    ...
    def test_provider_model_ids_falls_back_to_static_models(self, monkeypatch):
        ...
        monkeypatch.setattr("alice_cli.models.fetch_api_models", lambda api_key, base_url: None)
...
class TestGmiDoctor:
    def test_provider_env_hints_include_gmi(self):
        from alice_cli.doctor import _PROVIDER_ENV_HINTS
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a AliceAgent User-Agent
    ...
    def test_gmi_profile_declares_alice_user_agent(self):
        ...
        assert ua.startswith("AliceAgent/"), (
            f"expected GMI profile User-Agent to start with 'AliceAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("alice_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "alice_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["alice", "chat", "--provider", "gmi"])

        from alice_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("alice_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("alice_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("alice_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        config = yaml.safe_load((get_alice_home() / "config.yaml").read_text()) or {}
