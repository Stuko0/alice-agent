...
prefix (e.g. ``mission-control.tilos.com/alice/*`` -> local Caddy ->
...
honour it too:
  ...
  1. The gate's ``Location:`` redirect to /login (in
     ``_unauth_response``) needs to be ``/alice/login`` so the browser
  ...
  3. ``_redirect_uri`` (the OAuth callback URL handed to the IDP) must
     ...
     ``/alice/auth/callback`` and the user gets 404.
  4. Cookies must use ``Path=/alice`` when behind a prefix so they
...
from alice_cli.dashboard_auth import clear_providers, register_provider
from tests.alice_cli.conftest_dashboard_auth import StubAuthProvider
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        r = gated_app_proxied.get(
            "/sessions",
            headers={"x-forwarded-prefix": "/alice"},
        ...
        assert r.headers["location"].startswith("/alice/auth/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        # include the prefix.
        assert body["login_url"].startswith("/alice/login"), (
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(
        ...
        ``https://mission-control.tilos.com/alice/auth/callback`` — the
        ...
        assert parsed.path == "/alice/auth/callback", (
...
class TestPublicUrlOverride:
    ...
    def patch_config(self, monkeypatch):
        """Replace ``alice_cli.config.load_config`` with a stub
        ...
        def _set(public_url) -> None:
            ...
            monkeypatch.setattr(
                "alice_cli.config.load_config", lambda: cfg
            )
    ...
    def test_public_url_with_path_prefix_baked_in(
        ...
        (``https://example.com/alice``), the OAuth callback URL is
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        from alice_cli.dashboard_auth import prefix as prefix_mod
    ...
    def test_scheme_less_public_url_warning_is_deduplicated(
        ...
        scheme_warnings = [
            ...
            for r in caplog.records
            if r.levelno == logging.WARNING
            and "alice.domain.com" in r.getMessage()
        ]
    ...
    def test_valid_public_url_emits_no_warning(
        ...
        assert result == "https://alice.domain.com"
        assert not [
...
class TestCookiePathRespectsPrefix:
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from alice_cli.dashboard_auth.cookies import set_pkce_cookie
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        __Secure- prefix AND Path=/alice, so the next request under
        ...
        (``/alice/auth/login``, ``/alice/auth/callback``). A cookie
        set with ``Path=/alice`` would therefore NOT be sent back on
        the second request through TestClient even though it WOULD be
        sent by a real browser hitting ``/alice/auth/callback``. To
        ...
        # automatically send a Path=/alice cookie to a /auth/callback
        # request path.
        r2 = gated_app_proxied.get(
            f"/auth/callback?code=stub_code&state={state}",
            headers={
                "x-forwarded-prefix": "/alice",
                "cookie": pkce_kv,
            },
            follow_redirects=False,
        )
        ...
        assert "Path=/alice" in at_cookies[0]
