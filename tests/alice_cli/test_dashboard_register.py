"""Tests for ``alice dashboard register``.
...
import alice_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "alice_cli.auth.resolve_nous_access_token", side_effect=err
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://stuko.dev",
        ...
        with patch(
            "alice_cli.auth.resolve_nous_access_token", return_value=account_token
        ), patch("alice_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "alice_cli.config.get_env_value", side_effect=fake_get_env
        ), patch(
            "alice_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_custom_redirect_uri_is_forwarded(self, capsys):
        ...
        assert (
            ...
            == "https://alice.example.com/auth/callback"
        )
...
class TestPublicUrlPersistence:
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        response = {
            ...
            "name": "dreamy_tesla",
            "kind": "SELF_HOSTED",
            "custom_redirect_uri": "https://alice.example.com/auth/callback",
            "created_at": "2026-06-04T12:00:00.000Z",
        }
        saved: dict = {}
        ...
        with patch(
            ...
            "alice_cli.config.get_env_value", return_value=None
        ), patch(
            "alice_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
