"""Tests for the Kanban DB layer (alice_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated ALICE_HOME with an empty kanban DB."""
    home = tmp_path / ".alice"
    home.mkdir()
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Alice profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="alice-task", assignee="daily")
...
def test_dispatch_worktree_task_persists_materialized_workspace_and_branch(kanban_home, tmp_path, monkeypatch):
    ...
    import alice_cli.profiles as profiles
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Alice-managed scratch dir should be cleaned up"
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Alice' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Alice' DB/metadata/logs on task completion.
    """
...
# spawned with `alice -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's ALICE_HOME.
# ---------------------------------------------------------------------------
...
class TestSharedBoardPaths:
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.alice/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.alice/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.alice/kanban.db.
        default_home = tmp_path / ".alice"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # ALICE_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `alice -p coder`).
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # ALICE_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile ALICE_HOME and reads.
        monkeypatch.setenv("ALICE_HOME", str(profile_home))
        with kb.connect() as conn:
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see alice_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("alice_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="alice_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'alice kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# $PATH that doesn't include the venv's bin/, so a bare `["alice", ...]`
...
def test_resolve_alice_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    `alice` package). Regression for #23198: the original PR shipped
    `python -m alice` which fails with `No module named alice` on every
    invocation.
    """
    import shutil
    import sys
...
def test_resolve_alice_argv_module_actually_runs():
    ...
    sufficient — if `alice_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m alice_cli.main --version`
    ...
    assert "Alice Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="alice_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from alice_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("alice_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from alice_cli.kanban_db import connect, write_txn
    import alice_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from alice_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from alice_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("alice_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("alice_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("alice_cli.kanban_db.os.name", "nt")
    with patch("alice_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("alice_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("alice_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "alice_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("alice_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "alice_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("alice_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("alice_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("alice_cli.kanban_db._record_worker_exit"):
            with patch("alice_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
