"""Tests for --ignore-user-config and --ignore-rules flags on `alice chat`.
...
files. In Alice the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.alice/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → AliceCLI → AIAgent call chain.
"""
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``AliceCLI.ignore_rules``
    ...
    def test_env_var_enables_ignore_rules(self, monkeypatch):
        ...
        # Build only enough of AliceCLI to reach the ignore_rules assignment.
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``alice_cli/main.py`` must set
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real alice parser accepts both flags."""
