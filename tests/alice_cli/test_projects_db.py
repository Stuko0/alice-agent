"""Tests for the per-profile Projects store (alice_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Alice Agent", folders=["/tmp/alice"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "alice-agent"
    assert proj.name == "Alice Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/alice"
    assert [f.path for f in proj.folders] == ["/tmp/alice"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "alice-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Alice Agent")
    pdb.create_project(conn, name="Alice Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["alice-agent", "alice-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ALICE_HOME.
