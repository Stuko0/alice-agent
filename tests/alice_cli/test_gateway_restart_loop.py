...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "alice gateway stop",
        "alice gateway start",
        "alice  gateway  restart",         # double spaces
    ...
    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.alice.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.alice.gateway.plist",
        "launchctl stop ai.alice.gateway",
        "systemctl restart alice-gateway",
        "systemctl stop alice-gateway.service",
        "systemctl start alice-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill alice gateway process",
        "pkill -f alice.*gateway",
    ])
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_alice_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade alice then run alice gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.alice.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart alice-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-alice systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart alice-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart alice-gateway"]
