...
``alice_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``alice --help``, ``alice version``,
``alice logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``alice --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from alice_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["alice", "-w", "--tui"], None),
        (["alice", "version"], "version"),
        (["alice", "--tui", "chat"], "chat"),
        (["alice", "-w", "logs"], "logs"),
        (["alice", "chat", "hello world"], "chat"),
        (["alice", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["alice", "-m", "gpt5", "chat"], "chat"),
        (["alice", "--model", "gpt5", "chat", "hi"], "chat"),
        (["alice", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["alice", "-z", "hello world"], None),
        (["alice", "-z", "hello", "chat"], "chat"),
        (["alice", "--model=gpt5", "chat"], "chat"),     # inline form
        (["alice", "--", "chat"], "chat"),               # -- terminator
        (["alice", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["alice", "some-plugin-cmd"], "some-plugin-cmd"),
        (["alice", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["alice"],                          # bare → chat
        ["alice", "--help"],                # top-level help
        ["alice", "-h"],
        ["alice", "version"],               # known built-in
        ["alice", "logs"],
        ["alice", "gateway", "run"],
        ["alice", "--tui"],
        ["alice", "-w", "--tui"],
        ["alice", "chat", "hi"],
        ["alice", "help"],                  # accepted built-in-ish
        ["alice", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["alice", "meet", "join"],          # potential google_meet plugin
        ["alice", "honcho", "status"],      # potential memory plugin
        ["alice", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``alice help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"alice_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
