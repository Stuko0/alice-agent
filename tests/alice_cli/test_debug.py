"""Tests for ``alice debug`` CLI command and debug utilities."""
...
def lydia_home(tmp_path, monkeypatch):
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO alice_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from alice_cli.debug import _upload_paste_rs
        ...
        with patch("alice_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "alice_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from alice_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from alice_cli.debug import upload_to_pastebin

        with patch("alice_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("alice_cli.debug._upload_paste_rs",
             ...
             patch("alice_cli.debug._upload_dpaste_com",
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, lydia_home, capsys):
        ...
        with patch("alice_cli.dump.run_dump"), \
             patch("alice_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("alice_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, lydia_home, capsys):
        ...
        with patch("alice_cli.dump.run_dump"), \
             patch(
                 "alice_cli.debug._sweep_expired_pastes",
    ...
    def test_share_uploads_five_pastes(self, lydia_home, capsys):
        ...
        with patch("alice_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- alice dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
    ...
    def test_share_keeps_report_and_full_log_on_same_snapshot(self, lydia_home, capsys):
        ...
        with patch("alice_cli.dump.run_dump"), \
             patch("alice_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("alice_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        ...
        with patch("alice_cli.dump.run_dump"), \
             patch("alice_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("alice_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from alice_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from alice_cli.debug import delete_paste
        ...
        with patch("alice_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.alice/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``alice debug``
    invocation.
    """
    ...
    def test_does_not_spawn_subprocess(self, lydia_home):
        ...
        import ast
        import inspect
        from alice_cli.debug import _schedule_auto_delete
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, lydia_home):
        ...
        with patch("alice_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, lydia_home):
        ...
        with patch("alice_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, lydia_home):
        ...
        with patch("alice_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
    ...
    def test_run_debug_survives_sweep_failure(self, lydia_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: alice debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from alice_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("alice_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("alice_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, lydia_home, capsys):
        ...
        with patch("alice_cli.dump.run_dump"), \
             ...
             patch("alice_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, lydia_home, capsys):
        ...
        with patch("alice_cli.dump.run_dump"), \
             patch("alice_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("alice_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
...
class TestBuildDebugShare:
    ...
    Backs both ``alice debug share`` (CLI) and ``POST /api/ops/debug-share``
    ...
    def test_returns_structured_urls(self, lydia_home):
        ...
        with patch("alice_cli.dump.run_dump"), patch(
            "alice_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("alice_cli.debug._schedule_auto_delete"):
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, lydia_home):
        ...
        from alice_cli.debug import build_nous_bundle
        ...
        assert envelope["format"] == "alice-debug-share/1"
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, lydia_home, capsys):
        ...
        with patch("alice_cli.dump.run_dump"), patch(
            "alice_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, lydia_home, capsys):
        ...
        with patch("alice_cli.dump.run_dump"), patch(
            "alice_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, lydia_home):
        ...
        with patch("alice_cli.dump.run_dump"), patch(
            "alice_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("alice_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    ``AliceCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("alice_cli.debug.run_debug_share", _fake_run):
