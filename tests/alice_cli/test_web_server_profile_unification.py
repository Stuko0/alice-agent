...
profile switcher can target any profile's ALICE_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via ALICE_HOME.
"""
import pytest
import yaml
...
class TestProfileScopedMcp:
    ...
    def test_mcp_probe_runs_inside_profile_scope(
        ...
        import alice_cli.mcp_config as mcp_config
        from alice_constants import get_alice_home
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same ALICE_HOME the rest
        ...
        monkeypatch.setattr(
            "alice_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid():
            seen_homes.append(str(get_alice_home()))
            return None
...
class TestProfileScopedChatPty:
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("ALICE_HOME") != str(isolated_profiles["worker_beta"])
