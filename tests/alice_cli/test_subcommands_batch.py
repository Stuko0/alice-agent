...
# ── deprecated `alice login` fails gracefully, not with argparse error ────
#
# `alice login` is a removed command; its handler (`login_command` in
# `alice_cli/auth.py`) prints a deprecation notice pointing at `alice auth` /
# `alice model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `alice --help`.
