"""Tests for `alice curator status` output.
...
def test_status_uses_last_activity_not_only_last_used(monkeypatch, capsys):
    ...
    import alice_cli.curator as curator_cli
...
def curator_status_env(tmp_path, monkeypatch):
    """Isolated ALICE_HOME with real agent-created skills on disk."""
    home = tmp_path / ".alice"
    skills = home / "skills"
    ...
    from alice_cli import curator as curator_cli
