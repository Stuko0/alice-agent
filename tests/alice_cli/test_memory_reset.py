"""Tests for the `alice memory reset` CLI command.
...
- Profile-scoped reset (uses ALICE_HOME)
"""
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `alice memory reset` is run.
    """
...
class TestMemoryReset:
    """Tests for `alice memory reset` subcommand."""
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ALICE_HOME."""
