"""Tests for alice_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateDeepLink:
    ...
    def test_name_url_encoded(self):
        link = generate_deep_link(
            ...
            suggested_name="Alice & Friends",
        )
        assert "Alice+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        with patch(
            "alice_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="Alice Agent")
...
class TestPollForToken:
    ...
    def test_immediate_success(self):
        ...
        with patch(
            "alice_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("alice_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("alice_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("alice_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("alice_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("alice_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "alice_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("alice_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("alice_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from alice_cli.setup import _setup_telegram_auto

        assert callable(_setup_telegram_auto)

    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        from alice_cli import setup

        seen = {}
        profile_home = tmp_path / ".alice" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_alice_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "alice_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
