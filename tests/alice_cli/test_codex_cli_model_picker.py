...
Covers:
 - Normal path (tokens already in Alice auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Alice
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `alice auth openai-codex`. The old
...
@pytest.fixture()
def alice_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Alice auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(alice_auth_only_env):
    """openai-codex appears when tokens are already in Alice auth store."""
...
def test_codex_picker_uses_live_codex_catalog(alice_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Alice
    auth store."""
    alice_home = tmp_path / ".alice"
    alice_home.mkdir()
    ...
    (alice_home / "auth.json").write_text(
