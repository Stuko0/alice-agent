...
class TestUnifiedDashboardRouting:
    ...
    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        ...
        machine root /opt/data — NOT drop ALICE_HOME.

        Dropping it makes the child fall back to $HOME/.alice
        (= /opt/data/.alice), an empty auto-seeded home, so the dashboard
        ...
        """
        monkeypatch.setenv("ALICE_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "alice_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
    ...
    def test_dashboard_starts_mcp_discovery_for_ws_backend(self, main_mod, monkeypatch):
        ...
        calls = []
        monkeypatch.setattr(
            "alice_cli.mcp_startup.start_background_mcp_discovery",
