    ...
    with patch.dict(
        "sys.modules",
        {
            "alice_constants": MagicMock(
                get_alice_home=MagicMock(return_value="/tmp/lydia_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'alice-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'alice-release' patched"
    }
