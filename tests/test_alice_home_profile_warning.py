...
profile is active, get_alice_home() should:
  1. STILL return ~/.alice (raising would brick 30+ module-level callers)
...
def fresh_constants(monkeypatch, tmp_path):
    """Import alice_constants fresh and reset the one-shot warn flag."""
    import importlib
...
class TestGetAliceHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file → silent, returns ~/.alice."""
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        assert err.count("ALICE_HOME fallback") == 1
        assert "'coder'" in err
        assert "#18594" in err

        # 3. One-shot: second and third calls don't re-warn
        fresh_constants.get_alice_home()
        fresh_constants.get_alice_home()
    ...
    def test_alice_home_set_suppresses_warning(
        ...
        (tmp_path / ".alice" / "active_profile").write_text("coder\n")
