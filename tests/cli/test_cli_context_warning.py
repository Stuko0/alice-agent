...
def _isolate(tmp_path, monkeypatch):
    """Isolate ALICE_HOME so tests don't touch real config."""
    home = tmp_path / ".alice"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal AliceCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = AliceCLI.__new__(AliceCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["alice-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Alice' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Alice' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Alice' minimum context length."""
