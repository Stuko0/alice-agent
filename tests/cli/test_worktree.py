...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("alice/alice-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned alice/* and pr-* branches."""

    def test_prunes_orphaned_alice_branch(self, git_repo):
        """alice/alice-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "alice/alice-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "alice/alice-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "alice/alice-deadbeef" in result.stdout
        ...
        assert "alice/alice-deadbeef" in orphaned
        ...
        assert "alice/alice-deadbeef" not in result.stdout
