...
def session_db(tmp_path):
    ...
    os.environ["ALICE_HOME"] = str(tmp_path / ".alice")
    os.makedirs(tmp_path / ".alice", exist_ok=True)
    from alice_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".alice" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal AliceCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        AliceCLI._handle_branch_command(cli_instance, "/branch refactor approach")
