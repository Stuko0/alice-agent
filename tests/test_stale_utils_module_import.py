...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.alice/alice-agent/utils.py)
...
2. ``alice update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``alice_cli/main.py`` (the ``alice update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``alice_constants`` after the
