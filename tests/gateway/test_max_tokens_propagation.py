...
def isolated_home(tmp_path, monkeypatch):
    """Isolated ALICE_HOME with a writable config.yaml and a clean module cache.

    These tests deliberately re-import ``alice_cli`` / ``gateway`` so each
    ...
    _saved = {
        ...
        if k.startswith(("alice_cli", "gateway"))
    }
    ...
    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("alice_cli", "gateway")):
                del sys.modules[mod]
...
def test_lift_helper_accepts_alias_and_rejects_garbage(isolated_home):
    ...
    write_cfg("model:\n  provider: openrouter\n")
    for mod in list(sys.modules.keys()):
        if mod.startswith("alice_cli"):
            del sys.modules[mod]
    rp = importlib.import_module("alice_cli.runtime_provider")
