...
``alice update``, serving pre-update bridge.js behavior forever (e.g.
...
class TestDepRefreshStamp:
    ...
    async def test_reinstalls_when_package_json_changed(self, tmp_path):
        bridge_dir = _setup_bridge_dir(tmp_path)
        _fresh_node_modules(bridge_dir)
        # Simulate `alice update` bumping the Baileys pin
        ...
        from plugins.platforms.whatsapp.adapter import _file_content_hash
        stamp = (bridge_dir / "node_modules" / ".alice-pkg-hash").read_text().strip()
