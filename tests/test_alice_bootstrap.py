"""Tests for alice_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Alice entry point
(alice, alice-agent, alice-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Alice entry point imports alice_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported alice_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("alice_bootstrap", None)
    import alice_bootstrap  # noqa: WPS433
    return alice_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Alice CLI
...
class TestEntryPointsImportBootstrap:
    """Every Alice entry point must import alice_bootstrap as its
    ...
    # Entry points that invoke Alice as a process.  Each one must
    # import alice_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "alice_cli/main.py",   # alice CLI (console_script)
        "run_agent.py",          # alice-agent (console_script)
        "acp_adapter/entry.py",  # alice-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import alice_bootstrap' and that
        ...
        half-finished ``alice update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``alice_bootstrap`` as a top-level module) leaves alice
        ...
        """
        # Resolve relative to the alice-agent repo root.  Tests live
        # at tests/test_alice_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # alice start even when alice_bootstrap hasn't been
        ...
        assert first_import_name == "alice_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'alice_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import alice_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Alice's own top-level modules — covering both
    ...
    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/alice", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Alice root.  The guard must relocate Alice to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/alice"])
        assert result[0] == "/opt/alice"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Alice root.
        assert result.index("/opt/alice") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/alice", "/opt/alice", ""])
        assert result.count("/opt/alice") == 1
