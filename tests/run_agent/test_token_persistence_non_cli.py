...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    alice_state = ModuleType("alice_state")
    alice_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "alice_state", alice_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Alice"}, "task-id"))
