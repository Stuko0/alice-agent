"""Tests for ACP Registry metadata shipped with Alice."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "alice-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Alice Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/Stuko0/alice-agent"
    assert data["website"].startswith("https://alice-agent.stuko.dev/")
    assert data["authors"] == ["Stuko"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"alice-agent[acp]=={data['version']}"
    assert uvx["args"] == ["alice-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"alice-agent[acp]=={_pyproject_version()}"
    )
