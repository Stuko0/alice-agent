"""Shared fixtures for the alice-agent test suite.
...
2. **Isolated ALICE_HOME.** ALICE_HOME points to a per-test tempdir so
   code reading ``~/.alice/*`` via ``get_alice_home()`` can't see the
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ALICE_HOME to per-test tempdirs so code that
    reads ``~/.alice/*`` can't touch the real one, and pins TZ/LANG so
    ...
    #    ``~/.alice/*`` via ``get_alice_home()`` now gets the tempdir.
    #
    ...
    #    ~/.alice/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import alice_cli.plugins as _plugins_mod
...
@pytest.fixture()
def mock_config():
    """Return a minimal alice config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``alice-gateway`` process
...
# correlated with a ``tests/alice_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> alice-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart alice-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting alice/python patterns are also blocked.
    """
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch alice-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m alice_cli.main".
                if (
                    "alice" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live alice-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting alice/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `alice update` (or the
        # equivalent `python -m alice_cli.main update`).  These commands
        ...
        if "update" in low and (
            # alice update / alice update --gateway / setsid bash -c ... alice update
            ("alice" in low and "update" in low.split())
            or
            # python -m alice_cli.main update --gateway
            ("alice_cli" in low and "update" in low.split())
            or
            # venv/bin/alice update  (absolute path variant used in tests)
            (".venv/bin/alice" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`alice update` against the real checkout, fetching "
