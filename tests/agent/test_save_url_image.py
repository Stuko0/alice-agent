...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ALICE_HOME under tmp_path."""
    monkeypatch.setenv("ALICE_HOME", str(tmp_path / ".alice"))
    (tmp_path / ".alice").mkdir()

    # Force the constants/image cache helpers to re-read ALICE_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("alice_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_alice_home_cache(self, http_server):
        base, _ = http_server
