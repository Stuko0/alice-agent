...
engine plugins (e.g. alice-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``alice plugins`` picker."""
    from alice_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "alice_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "alice_cli.plugins.get_plugin_context_engine",
