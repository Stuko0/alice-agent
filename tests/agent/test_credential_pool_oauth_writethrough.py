...
Companion to ``tests/alice_cli/test_xai_oauth_writethrough.py``. That file
...
from alice_cli import auth as A
...
def profile_and_root(tmp_path, monkeypatch):
    ...
    only refuses the *real* user's ``$HOME/.alice/auth.json``; a tmp_path
