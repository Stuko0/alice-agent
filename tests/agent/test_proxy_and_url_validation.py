...
def test_proxy_env_rejects_malformed_port(monkeypatch, key):
    ...
    # Windows os.environ is case-insensitive: setting ``all_proxy`` stores
    # ``ALL_PROXY``, so the error names the key in canonical upper case.
    # Match case-insensitively so the contract holds on every platform.
