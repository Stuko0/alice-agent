...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_lydia_env(self):
        ...
        # even ~/.alice/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.alice/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Alice cache files must remain blocked."""
