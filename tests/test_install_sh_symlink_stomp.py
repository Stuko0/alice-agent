...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/alice\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/alice"')
    cat_idx = block.find('cat > "$command_link_dir/alice" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/alice before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    Layout mirrors a real install:
        ...
        tmp/
          venv/bin/alice        <- pip entry point (the one we must preserve)
    ...
    requires that, after the run:
      ...
      * ``local_bin/alice`` is a regular file (not a symlink) holding the shim
    """
    venv_bin = tmp_path / "venv" / "bin"
    venv_bin.mkdir(parents=True)
    pip_entry = venv_bin / "alice"
    ...
    shim_path = command_link_dir / "alice"
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/alice was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/alice must be replaced with a regular file, not "
