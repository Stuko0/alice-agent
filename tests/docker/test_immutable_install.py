...
def test_install_tree_not_writable_by_alice(
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/alice as the alice user
        "touch /opt/alice/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"alice user can write to /opt/alice (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/alice/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"alice user can write to /opt/alice/.venv: {r.stdout}"
    )
...
def test_install_method_stamp_is_code_scoped(
    ...
    /opt/alice/.install_method (code-scoped), NOT in $ALICE_HOME."""
    start_container(built_image, container_name)

    # Code-scoped stamp must exist and say "docker"
    r = docker_exec_sh(
        container_name,
        "cat /opt/alice/.install_method",
        timeout=10,
    )
    assert r.returncode == 0, (
        f"/opt/alice/.install_method not found: {r.stderr}"
    )
    assert r.stdout.strip() == "docker", (
        f"expected 'docker' stamp, got: {r.stdout.strip()!r}"
    )

    # $ALICE_HOME must NOT have a 'docker' stamp
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() != "docker", (
        f"$ALICE_HOME/.install_method is stamped 'docker' - stage2 must "
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $ALICE_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $ALICE_HOME was not healed on boot: "
        f"{r.stdout}"
    )
