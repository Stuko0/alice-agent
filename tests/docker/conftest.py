...
Override the image with ``ALICE_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ALICE_TEST_IMAGE", "alice-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ALICE_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ALICE_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged alice user
# ---------------------------------------------------------------------------
#
# Background: every Alice runtime path inside the container drops to UID
# 10000 (the ``alice`` user) via ``s6-setuidgid alice``. ``docker exec``
...
# fails (PermissionError swallowed) for alice, so a test that ran as root
...
# helpers below run every probe under ``-u alice`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: alice).
