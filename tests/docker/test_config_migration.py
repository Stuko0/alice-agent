...
Build the real image and verify: a config.yaml present in $ALICE_HOME
is migrated by docker_config_migrate.py on boot, running as the alice
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $ALICE_HOME must be migrated on boot by
    docker_config_migrate.py, running as the alice user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $ALICE_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/alice/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by alice (migration ran as alice)
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() == "alice", (
        f"config.yaml not owned by alice (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
...
def test_config_migration_opt_out_env_var_respected(
    ...
    """ALICE_SKIP_CONFIG_MIGRATION=1 must skip the migration."""
    start_container(
        built_image, container_name, "ALICE_SKIP_CONFIG_MIGRATION=1",
    )
