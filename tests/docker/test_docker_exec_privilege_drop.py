...
The shim (docker/alice-exec-shim.sh, installed at /opt/alice/bin/alice)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> alice login` would write /opt/data/auth.json as
...
failed: Alice is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> alice …`` (defaulting to root) gets dropped to the
   alice user before the real binary runs.
2. ``docker exec --user alice <c> alice …`` (already non-root) short-
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-alice-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown alice:alice`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to alice:alice,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$ALICE_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (alice_cli.container_boot), which s6 runs
    *strictly after* ``01-alice-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_lydia_uid(sleep_container: str) -> None:
    ...
    pure-alice "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `alice` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `alice`
    do anything that writes to disk, then check the file's owner.

    Use `alice config set` which writes config.yaml under ALICE_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "alice", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by alice, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "alice:alice", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected alice:alice. "
        "The shim did not drop privileges before invoking alice."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user alice already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "alice", sleep_container,
         "alice", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user alice failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up alice:alice — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_strict_truthiness(
    ...
    assert r.stdout.strip() == "alice:alice", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected alice:alice"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/alice/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid alice alice <args>` which resolves `alice`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> alice login`` (root) wrote
    ...
    "Alice is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `alice config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is alice-readable.

    Specifically: pretend the operator ran `alice login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the alice UID. We use `alice auth list` since that touches the
    ...
    # `alice auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `alice config set` first to
    # provoke a write into ALICE_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "alice", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # ALICE_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "alice", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the alice user "
