"""Runtime smoke test for Docker $ALICE_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the alice user after container boot.
...
registered later runs its log service as the dropped alice user and
...
def test_logs_gateways_seeded_and_alice_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by alice after boot."""
    ...
    assert "logs=alice" in r.stdout, (
        f"logs/ not owned by alice: {r.stdout}"
    )
    assert "gateways=alice" in r.stdout, (
        f"logs/gateways/ not owned by alice: {r.stdout}"
    )
