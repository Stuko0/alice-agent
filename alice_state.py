#!/usr/bin/env python3
"""
SQLite State Store for Alice Agent.
...
DEFAULT_DB_PATH = get_alice_home() / "state.db"
...
# alice_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``alice_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see alice_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple alice processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``alice_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `alice update` to rebuild the venv with a "
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Alice surface and jump
    ...
    def delete_session_if_empty(
        ...
        and ``alice sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Alice, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Alice session.

        A Alice session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Alice session is already bound to any Telegram DM topic.
