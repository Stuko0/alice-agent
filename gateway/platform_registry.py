...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``alice setup`` display).
    ...
    # built-ins).  Used by ``alice gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `alice`
        # invocation -- including plain `alice chat`, which never touches any
        ...
        # `alice setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``alice setup``/``gateway status``, channel
