...
There are two ways to add a platform to the Alice gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.alice/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Alice code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `alice gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `alice_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Alice core.
...
And add it to the `alice-gateway` composite:

```python
"alice-gateway": {
    "includes": [..., "alice-your-platform"]
}
```
...
## 12. Status Display (`alice_cli/status.py`)
...
## 13. Gateway Setup Wizard (`alice_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ alice_cli/ toolsets.py \
  --include="*.py" -l | sort -u
