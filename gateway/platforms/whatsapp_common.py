...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "🌹 *Alice Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to ALICE_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/alice), this function
    mirrors the bridge source to a writable ALICE_HOME location and returns that
