...
# ``sessions_dir / f"{session_id}.json"`` in alice_state, request-dump
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `alice native` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `alice native` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_alice_home()}/cron/output/)"
    )
    ...
    def _save(self) -> None:
        ...
        data = {
            "_README": (
                ...
                "~/.alice/state.db and are shown by `alice sessions list` and "
    ...
    def get_or_create_session(
        ...
        with self._lock:
            ...
            if session_key in self._entries and not force_new:
                ...
                # (alice_state.find_latest_gateway_session_for_peer) selects
