...
The PID file lives at ``{ALICE_HOME}/gateway.pid``.  ALICE_HOME defaults to
``~/.alice`` but can be overridden via the environment variable.  This means
separate ALICE_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting ALICE_HOME."""
    home = get_alice_home()
...
def _get_lock_dir() -> Path:
    ...
    return state_home / "alice" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Alice gateway lifecycle subcommand from a command line.
    ...
    test also matched ``alice_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\alice-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Alice's
    ...
    for token in tokens:
        ...
        if basename in ("alice-gateway", "alice-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "alice_cli.main" in joined
        or "alice_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("alice", "alice.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `alice gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Alice-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Alice gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a ALICE_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.alice`` or ``$ALICE_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``alice_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``ALICE_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    # a non-matching explicit ALICE_HOME= on the argv. ALICE_HOME is usually
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `alice status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``ALICE_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    ALICE_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ALICE_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``alice.service`` + ``alice-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different ALICE_HOME. When two profile gateway
    # services share the same default ~/.alice (ALICE_HOME not set
    ...
    # field is absent in markers written by older Alice versions; treat
    ...
    # ``alice gateway stop`` on Windows would be misclassified as an
