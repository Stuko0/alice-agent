"""Windows UTF-8 bootstrap for Alice entry points.
...
should be imported at the very top of every Alice entry point
(``alice``, ``alice-agent``, ``alice-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Alice modules.

    Alice ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Alice source root
    ...
    """
    root = src_root or os.environ.get("ALICE_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``ALICE_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("ALICE_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import alice_bootstrap``
# (or ``from alice_bootstrap import apply_windows_utf8_bootstrap``) at
