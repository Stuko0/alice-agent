...
Pure logic: walks a Alice config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `alice doctor` and a future `alice migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Alice config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Alice config and return retirement issues.
