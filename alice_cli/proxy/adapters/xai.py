...
# xAI's public API is OpenAI-compatible for the endpoints Alice commonly
# uses. The Responses endpoint is included because Alice' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Alice-managed OAuth credentials."""

    auth_hint = "alice auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`alice auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`alice auth reset xai-oauth` or re-authenticate with "
                    "`alice auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `alice auth add xai-oauth --type oauth`."
            )
