"""``alice logs`` — view and filter Alice log files.
...
Usage examples::
    ...
    alice logs                    # last 50 lines of agent.log
    alice logs -f                 # follow agent.log in real time
    alice logs errors             # last 50 lines of errors.log
    alice logs gateway -n 100    # last 100 lines of gateway.log
    alice logs gui -f            # follow gui.log (dashboard/pty/ws)
    alice logs desktop -f        # follow desktop.log (Electron app boot/backend)
    alice logs --level WARNING    # only WARNING+ lines
    alice logs --session abc123   # filter by session ID substring
    alice logs --component tools  # only tool-related lines
    alice logs --since 1h         # lines from the last hour
    alice logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_alice_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Alice runs — try 'alice chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_alice_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_alice_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_alice_home()}/logs/")
        return

    print(f"Log files in {display_alice_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'alice chat' to generate logs)")
