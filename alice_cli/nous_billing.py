...
billing screens drive. Companion to ``alice_cli/nous_account.py`` (which owns
...
# alice_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Nous Portal — run `alice portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
