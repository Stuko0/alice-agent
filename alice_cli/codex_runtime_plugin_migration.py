"""Migrate Alice' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Alice' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Alice mcp_servers.<n>.command/args/env  → codex stdio transport
  Alice mcp_servers.<n>.url/headers       → codex streamable_http transport
  Alice mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Alice mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Alice-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Alice' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Alice config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Alice' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ALICE_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Alice)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Alice' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Alice' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Alice
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="alice-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ALICE_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed alice-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.alice`` symlink that happens to live under ``/private/var/folders``
...
def _build_alice_tools_mcp_entry() -> dict:
    ...
    so a alice installed under /opt/, /usr/local/, or a venv all work.
    ALICE_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ALICE_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_alice_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ALICE_HOME its launcher sets
    ...
    # — that would override the launcher's ALICE_HOME and pin the subprocess
...
def migrate(
    ...
    *,
    ...
    # Inject Alice' own tool surface as an MCP server so the spawned
