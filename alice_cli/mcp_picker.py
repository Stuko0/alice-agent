"""MCP picker — interactive `alice mcp picker` (also the default `alice mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``alice mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `alice plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from alice_cli.cli_output import prompt_yes_no
from alice_cli.curses_ui import curses_single_select
from alice_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Alice session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from alice_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `alice mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: alice mcp install <name>    Picker: alice mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Alice is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Alice — run `alice update` "
...
def show_catalog() -> None:
    """`alice mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`alice mcp picker` (and default `alice mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`alice mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `alice mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
