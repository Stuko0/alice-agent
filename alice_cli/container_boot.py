...
``$ALICE_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-alice-setup
(the stage2 hook) has chowned the volume and seeded $ALICE_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$ALICE_HOME``,
    not under ``profiles/``). The dispatcher in ``alice_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``alice gateway start`` (no ``-p``) targets. Without it,
    bare ``alice gateway start`` inside the container would land on
    ...
    ``$ALICE_HOME/gateway_state.json`` (sibling to the profile root,
    ...
    # ``alice gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `alice profile create` (config.yaml
            # is not — that comes later via `alice setup`). Use it as the
            ...
            # via ``alice -p default-named gateway start`` if they
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the alice args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/alice/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/alice/docker/main-wrapper.sh
    ...
    ``alice`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``alice <subcommand>``, so an
    explicit leading ``alice`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `alice <subcommand>`; peel an explicit alice.
    if args and Path(args[0]).name == "alice":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``alice dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted ALICE_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `alice -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit alice
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the alice user) won't hit EACCES. See
...
def main() -> int:
    ...
    # bind-mounted ALICE_HOME, both race to flock() the same s6-log lock
