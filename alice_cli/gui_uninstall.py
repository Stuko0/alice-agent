"""
Alice Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``alice desktop`` / ``alice gui``)
     Built inside the agent checkout under ``$ALICE_HOME/alice-agent/``:
       ...
       - ``$ALICE_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Alice.app`` or ``~/Applications/Alice.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Alice`` (NSIS per-user)
...
the app name ("Alice"), separate from ``$ALICE_HOME``:
  - macOS:   ``~/Library/Application Support/Alice``
  - Windows: ``%APPDATA%\\Alice``
  - Linux:   ``$XDG_CONFIG_HOME/Alice`` (default ``~/.config/Alice``)
...
``alice uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(alice_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return alice_home / "alice-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Alice"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Alice"
...
def source_built_gui_artifacts(alice_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``alice desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``alice-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(alice_home)
    ...
    return [
        ...
        alice_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Alice" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Alice.app"),
            home / "Applications" / "Alice.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Alice).
            local_base / "Programs" / "Alice",
            # Older / alternate layout some builds used.
            local_base / "alice-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Alice")
    else:
        ...
        paths += [
            data_base / "applications" / "alice.desktop",
            data_base / "applications" / "Alice.desktop",
        ]
    return paths


def agent_is_installed(alice_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ALICE_HOME.
    ...
    if (agent_root / "alice_cli").is_dir():
        return True
...
def gui_is_installed(alice_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(alice_home):
        if p.exists():
            return True
...
def gui_install_summary(alice_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = alice_home if alice_home is not None else get_alice_home()
...
def uninstall_gui(alice_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``alice-agent/alice_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ALICE_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove alice' or "
            "'sudo dnf remove alice'). AppImage builds are a single file you "
