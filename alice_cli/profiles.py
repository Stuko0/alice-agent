"""
Profile management for multiple isolated Alice instances.

Each profile is a fully independent ALICE_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.alice/profiles/<name>/`` by default.

The "default" profile is ``~/.alice`` itself — backward compatible,
zero migration needed.

Usage::

    alice profile create coder          # fresh profile + bundled skills
    alice profile create coder --clone  # also copy config, .env, SOUL.md, skills
    alice profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    alice -p coder chat                 # or via flag
    alice profile use coder             # set as sticky default
    alice profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See alice_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `alice profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `alice
...
# `alice skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Alice infrastructure directories that only the default profile
         (``~/.alice``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.alice) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "alice-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "alice", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the alice root, NOT to the current ALICE_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ALICE_HOME points outside
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Alice installation itself or a common system binary.  "
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, alice subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Alice. If we
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``alice -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``alice profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``alice -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``alice profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``ALICE_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Alice config
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ALICE_HOME.
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``alice update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.alice)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.alice/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `alice -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Alice profile.\n"
    ...
    if not soul_path.exists():
        try:
            from alice_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `alice update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`alice profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Alice (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `alice profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `alice -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ALICE_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``alice profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ALICE_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.alice).\n"
            "To remove everything, use: alice uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ALICE_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from alice_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ALICE_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ALICE_HOME")
    try:
        os.environ["ALICE_HOME"] = str(profile_dir)
        from alice_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ALICE_HOME"] = old_home
        elif "ALICE_HOME" in os.environ:
            del os.environ["ALICE_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.alice/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: alice profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ALICE_HOME.

    Returns ``"default"`` if ALICE_HOME is not set or points to ``~/.alice``.
    Returns the profile name if ALICE_HOME points into ``~/.alice/profiles/<name>``.
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.alice itself — its parent is ~/ and its
        # directory name is ".alice", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: alice profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.alice itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.alice). "
            "Specify a different name: alice profile import <archive> --name <name>"
        )
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ALICE_HOME path string.

    Called early in the CLI entry point, before any alice modules
    are imported, to set the ALICE_HOME environment variable.
    """
