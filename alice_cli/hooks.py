"""alice hooks — inspect and manage shell-script hooks.

Usage::

    alice hooks list
    alice hooks test <event> [--for-tool X] [--payload-file F]
    alice hooks revoke <command>
    alice hooks doctor

Consent records live under ``~/.alice/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.alice/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``alice hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: alice hooks {list|test|revoke|doctor}")
        print("Run 'alice hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.alice/config.yaml.")
        print("See `alice hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `alice hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `alice hooks test` and `alice hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Alice wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `alice hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `alice hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `alice hooks doctor`.")
