"""Shell completion script generation for alice CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Alice Agent bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef alice
# Alice Agent zsh completion
# Add to ~/.zshrc:
...
_alice_profiles() {{
    ...
    if [[ -d "$HOME/.alice/profiles" ]]; then
        profiles+=($HOME/.alice/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'alice command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
