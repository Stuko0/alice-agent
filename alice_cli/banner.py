...
Pure display functions with no AliceCLI state dependency.
"""
...
def _fetch_pypi_latest(package: str = "alice-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    # and `alice update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/alice/update/check`
    ...
    if embedded_rev:
        ...
        # $ALICE_HOME/alice-agent/ may be a stale copy from --clone-all;
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Alice git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$ALICE_HOME/alice-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``alice_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from alice_cli.build_info import get_build_sha
...
def format_banner_version_label() -> str:
    ...
    base = f"Alice Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(
    ...
    try:
        from alice_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            ...
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built alice; we know an update
    ...
    # Pip-install warning — `pip install alice-agent` is not the supported
