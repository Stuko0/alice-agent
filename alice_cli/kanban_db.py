...
``<root>`` is the **shared Alice root** (the parent of any active
...
``alice -p <profile>`` joins the same board as the dispatcher that
...
the profile's Alice home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``alice kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.alice``. In Docker / custom
deployments where ``ALICE_HOME`` points outside ``~/.alice`` (e.g.
...
coding workloads.  See ``docs/alice-kanban-v1-spec.pdf`` for the full
...
from alice_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active ALICE_HOME so dispatcher- and
...
# enough that kebab-case names like ``atm10-server`` or ``alice-agent``
...
def kanban_home() -> Path:
    """Return the shared Alice root that anchors the kanban board.
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``alice kanban boards switch <slug>``
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``alice kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``alice kanban log`` unambiguous even when multiple
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (alice_cli/projects_db). When set,
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``alice gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Alice treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    # holder (e.g. an external `alice kanban list` mid-integrity-probe) block
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from alice_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See alice_state._WAL_INCOMPAT_MARKERS for detection logic.
                from alice_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``alice kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from alice_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``alice --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        try:
            from alice_cli import projects_db as _pdb
    ...
    # `alice --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``alice kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `alice kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``alice kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``alice kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``alice kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``alice kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Alice' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Alice-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``alice kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Alice checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Alice treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Alice creates/reuses that linked worktree. With
      no ``workspace_path``, Alice anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Alice uses
      ``wt/<task-id>``.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Alice
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``alice kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``alice kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Alice profile.
    ...
    if not rows:
        return False
    try:
        from alice_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from alice_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Alice profile.
        # `_default_spawn` invokes ``alice -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Alice
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `alice kanban tail` — without
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from alice_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _resolve_lydia_argv() -> list[str]:
    ...
    2. ``shutil.which("alice")`` — the console-script shim, normalized to
       ...
       ``.\\alice.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m alice_cli.main`` — fallback for setups where
       Alice is launched from a venv and the ``alice`` shim is not on
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``alice -p <profile> chat -q ...`` subprocess.
    ...
    # Inject ALICE_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `alice -p <name>` the
    # _apply_profile_override() runs *before* alice_constants is imported.
    # If ALICE_HOME is absent from the child's env, get_alice_home() falls
    # back to Path.home() / ".alice" (the DEFAULT profile root), ignoring the
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("ALICE_HOME"))
    if worker_toolsets:
    ...
    # `alice kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`alice` executable not found on PATH. "
            "Install Alice Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``alice kanban daemon`` is systemd-friendly.
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Alice root exists

    Reads profile paths directly so this module has no import dependency on
    ``alice_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from alice_constants import get_default_lydia_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``alice kanban assignees`` for the terminal.
