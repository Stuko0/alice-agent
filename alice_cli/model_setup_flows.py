"""Per-provider model-selection wizard flows for ``alice setup`` / ``alice model``.

Extracted from ``alice_cli/main.py`` as part of the god-file decomposition
campaign (``~/.alice/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from alice_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``alice_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from alice_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``alice_cli.main`` at import time -> no import cycle.
"""
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from alice_cli.main import _prompt_api_key
    from alice_constants import OPENROUTER_BASE_URL
    ...
    # in-flow (K/R/C) instead of having to edit ~/.alice/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `alice setup` from scratch. OpenRouter
    ...
    from alice_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    """
    from alice_cli.auth import _save_model_choice, deactivate_provider
    from alice_cli.config import load_config, save_config
    ...
    if not presets:
        print("No MoA presets configured. Run `alice moa configure <name>` first.")
        return
...
def _model_flow_nous(config, current_model="", args=None):
    """Nous Portal provider setup — deprecated/removed."""
    print("Nous Portal integration has been removed.")
    print("Please select another provider, such as OpenRouter or bring your own API key.")
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `alice auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from alice_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "reauth":
            ...
            print()
            try:
                # Forward CLI flags from ``alice model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``alice model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``alice auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from alice_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from alice_cli.auth import (
    ...
    from alice_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from alice_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from alice_cli.auth import _save_model_choice, deactivate_provider
    from alice_cli.config import get_env_value, load_config, save_config
    ...
    from alice_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `alice model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from alice_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    ...
    from alice_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Alice will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Alice will install it now (the preflight below "
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from alice_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from alice_cli.auth import _save_model_choice, deactivate_provider
    from alice_cli.config import load_config, save_config
    from alice_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from alice_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from alice_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from alice_cli.config import save_env_value, load_config, save_config
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from alice_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from alice_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Alice turns to `copilot --acp`.")
    print("  Alice currently starts its own ACP subprocess for each request.")
    print("  Alice uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ALICE_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from alice_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from alice_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from alice_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so alice knows where to find it
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``alice_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from alice_cli.main import _prompt_provider_choice
    from alice_cli.auth import ZAI_ENDPOINTS
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Alice typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Alice, enable billing on your "
    ...
    if provider_id == "lmstudio":
        ...
        from alice_cli.models import fetch_lmstudio_models
        ...
        from alice_cli.models import fetch_ollama_cloud_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from alice_cli.main import _run_anthropic_oauth_flow
    from alice_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'alice model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
