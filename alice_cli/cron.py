...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(alice\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*alice)"
    r"|(systemctl\s+(-\S+\s+)*(restart|stop|start)\s+.*alice)"
    r"|(p?kill\s+.*alice.*gateway)"
)
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: alice gateway install", Colors.DIM))
    print(color("                    sudo alice gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  alice cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'alice cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: alice gateway restart")
        ...
        print("    alice gateway install    # Install as a user service")
        print("    sudo alice gateway install --system  # Linux servers: boot-time system service")
        print("    alice gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `alice gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: alice cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
