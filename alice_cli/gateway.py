"""
Gateway subcommand for alice CLI.

Handles: alice gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_alice_home is imported lazily at call sites to avoid ImportError
# when alice_constants is cached from a pre-update version during `alice update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "alice CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``alice gateway status`` from falsely counting the ``alice`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``alice gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_alice_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. ALICE_HOME
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``alice update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Alice profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Alice profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from alice_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "alice_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m alice_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose ALICE_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``alice update`` users
    # with no running gateway until they re-invoke ``alice gateway``
    ...
    from alice_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from alice_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / ALICE_HOME) the windowless
        # base interpreter needs to import alice_cli.  Empty on POSIX, where
...
def _sync_alice_home_from_systemd_unit(system: bool) -> None:
    ...
    :func:`get_alice_home` falls back to ``/root/.alice`` — the wrong
    profile. The unit file pins ``ALICE_HOME`` for the actual gateway
    ...
    unit_home = env.get("ALICE_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ALICE_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ALICE_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}alice gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}alice gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from alice_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: alice gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `alice gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``alice gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from alice_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (alice_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ALICE_HOME-scoped).
...
from alice_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Alice Docker image — that one runs s6-overlay as
...
_SERVICE_BASE = "alice-gateway"
SERVICE_DESCRIPTION = "Alice Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ALICE_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ALICE_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ALICE_HOME paths
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ALICE_HOME.

    Default ``~/.alice`` returns ``alice-gateway`` (backward compatible).
    Profile ``~/.alice/profiles/coder`` returns ``alice-gateway-coder``.
    Any other ALICE_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Alice installs that predate the
# alice-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (alice-gateway-*.service) and unrelated third-party
# "alice" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("alice.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "alice_cli.main gateway",
    "alice_cli/main.py gateway",
    "gateway/run.py",
    " alice gateway ",
    "/alice gateway ",
)
...
def _find_legacy_lydia_units() -> list[tuple[str, Path, bool]]:
    ...
    different service name (e.g. ``alice.service`` before the rename to
    ``alice-gateway.service``). When both a legacy unit and the current
    ``alice-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``alice-gateway-coder.service`` and unrelated third-party
      ``alice-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``alice.service`` running an unrelated
...
def print_legacy_unit_warning() -> None:
    """Warn about legacy Alice gateway unit files if any are installed.
    ...
    print_info("  These run alongside the current alice-gateway service and")
...
def remove_legacy_lydia_units(
    ...
    print()
    print("Legacy Alice gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: alice gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo alice gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    alice gateway uninstall")
    print_info("    sudo alice gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: alice gateway install"
            )
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Alice often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.alice`` → ``ai.alice.gateway.plist`` (backward compatible).
    Profile ``~/.alice/profiles/coder`` → ``ai.alice.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.alice.gateway-{suffix}" if suffix else "ai.alice.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.alice/alice-agent  -> /home/alice/.alice/alice-agent
      /opt/alice                 -> /opt/alice  (kept as-is)
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m alice_cli.main``, so module
    ...
    dir, or a clone that ``alice update`` later relocates/removes — the path
    ...
    ``ALICE_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ALICE_HOME
...
User={username}
Group={group_name}
ExecStart={python_path} -m alice_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir ALICE_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved ALICE_HOME
    ...
    exported a throwaway ``ALICE_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``alice gateway restart``
    with ``ALICE_HOME=/tmp/alice-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="ALICE_HOME=..."`` lines and launchd
    ``<key>ALICE_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'ALICE_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>ALICE_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp ALICE_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: ALICE_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported ALICE_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # ALICE_HOME into the unit (manual E2E homes like /tmp/alice-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Alice install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo alice gateway uninstall --system")
    print_info("         alice gateway install")
    print_info("         alice gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (alice.service from pre-rename installs)
    # before installing the new alice-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}alice gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}alice gateway status{scope_flag}             # Check status"
    )
    print(
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `alice gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `alice gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}alice gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}alice gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}alice gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}alice gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.alice.gateway-{suffix}" if suffix else "ai.alice.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``alice gateway start/stop/status`` call).
...
# (the documented `nohup alice gateway run` workaround). See #23387.
...
def _launchd_unsupported_marker_path() -> Path:
    return get_alice_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m alice_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active ALICE_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "alice_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup alice gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup alice gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Alice install"
    )
    return True
...
def launchd_stop():
    ...
    # `alice gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # Alice PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Alice PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Alice install")
    else:
        print("⚠ Service definition is stale relative to the current Alice install")
        print("  Run: alice gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: alice gateway stop")
            else:
    ...
    if deep:
        log_file = get_alice_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``alice gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``alice gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``alice gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other ALICE_HOME roots.
    """
    ...
    print("  Use 'alice gateway restart' to replace it,")
    print("  or 'alice gateway stop' first.")
    print("  Or use 'alice gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Alice gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'alice' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/alice/docker/entrypoint.sh before the Alice command."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # /restart) rather than through `alice gateway restart` which already
    ...
    # `alice gateway start/restart` — leaving the gateway vulnerable to
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. alice) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Alice delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Alice connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: alice pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Alice will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``alice setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run alice under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.alice/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_alice_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from alice_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'alice pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: alice pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'alice gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Alice will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Alice will store the returned account_id/token in ~/.alice/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `alice gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_alice_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `alice pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `alice pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "AliceAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``alice_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from alice_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.alice/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.alice/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: alice gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: alice gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: alice gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo alice gateway install --system"
                        )
                    print_info("  Or run in foreground:  alice gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: alice gateway run")
                print_info(
                    "  For persistence:   tmux new -s alice 'alice gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: alice gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup alice gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'alice gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`alice_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from alice_cli.service_manager import (
    ...
    if profile is None:
        ...
        # ALICE_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``alice gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``alice gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    # `docker logs` and ${ALICE_HOME}/logs/gateways/<profile>/current,
    ...
    #
    # Prefer `sleep infinity` (matches the static main-alice service's
    # pattern in docker/s6-rc.d/main-alice/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: alice gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: alice gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s alice 'alice gateway run'"
                )
                print()
            ...
            print()
            print(
                "  alice gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s alice 'alice gateway run'         # persistent via tmux"
            )
            print(
                "  nohup alice gateway run > ~/.alice/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from alice_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  alice profile create <name>     # creates the s6 service slot")
                print("  alice -p <name> gateway start   # bring it up via s6")
                print("  alice status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: alice gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: alice gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: alice gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  alice profile delete <name>     # tears down the s6 service slot")
                print("  alice -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `alice -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: alice gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_LYDIA_GATEWAY") == "1":
            print_error(
                ...
                "Use `alice gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_LYDIA_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `alice gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: alice gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  alice gateway install")
                else:
                    print("To install as a service:")
                    print("  alice gateway install")
                    print("  sudo alice gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  alice gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup alice gateway run > ~/.alice/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s alice 'alice gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  alice gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  alice gateway install  # Install as user service")
                    print(
                        "  sudo alice gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Alice gateway unit files from
        # pre-rename installs (e.g. alice.service). Profile units and
        # unrelated third-party services are never touched.
