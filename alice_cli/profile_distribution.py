"""Profile distributions — shareable, packaged Alice profiles via git.

A distribution is a Alice profile published as a git repository (or
...
* ``alice profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``alice skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``alice profile``, not a parallel tree):

    alice profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "alice-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            alice_requires=str(data.get("alice_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.alice_requires:
            out["alice_requires"] = self.alice_requires
        if self.author:
...
def check_alice_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Alice {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``alice profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Alice profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Alice distribution."
        )

    # Version check up-front so we fail fast
    check_alice_requires(manifest.alice_requires, alice_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.alice).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="alice_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `alice profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`alice profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="alice_dist_update_") as tmp:
        plan = plan_install(
