"""
Configuration management for Alice Agent.

Config files are stored in ~/.alice/ for easy access:
- ~/.alice/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.alice/.env         - API keys and secrets

This module provides:
- alice config          - Show current configuration
- alice config edit     - Open config in editor
- alice config set      - Set a specific value
- alice config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``alice config set`` (which rewrites
    ...
    ``config.yaml`` in place: alice never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.alice/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``alice logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  alice config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Alice makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Alice itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``alice update``, the TUI build.
...
#   on every plugin install / ``alice update``.
...
from alice_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_alice_home() / ".managed"
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade alice-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``alice_cli/`` — i.e. the git checkout for source
    installs, ``/opt/alice`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$ALICE_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Alice was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.
    ...
    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.alice``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ALICE_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.alice:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ALICE_HOME``, a
    ...
    ``docker`` and ``alice update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``stuk0o/alice-agent`` image bakes a ``docker``
        stamp into ``/opt/alice`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ALICE_HOME.
    try:
    ...
    #    host install whose shared $ALICE_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``alice update``.
    try:
        method = (
            (get_alice_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``alice_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ALICE_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/alice`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Alice lives in a ``uv tool`` layout.

    ``uv tool install alice-agent`` places the install at
    ``.../uv/tools/alice-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``alice-agent`` happens to be uv-tool-installed on the machine while
    the *active* Alice is a regular pip/venv install, causing
    ``alice update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/alice-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade alice-agent"
    if method == "docker":
        return "docker pull stuk0o/alice-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade alice-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade alice-agent"
        return "pip install --upgrade alice-agent"
    return "alice update"
...
# Long-form text for ``alice update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# alice_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Alice, it doesn't update
...
✗ ``alice update`` doesn't apply inside the Docker container.

Alice Agent runs as a published image (stuk0o/alice-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull stuk0o/alice-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate alice-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm stuk0o/alice-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/stuk0o/alice-agent/tags
  • Your config and session history live under ``$ALICE_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``alice update`` inside Docker.
...
def format_managed_message(action: str = "modify this Alice installation") -> str:
    ...
    return (
        f"Cannot {action}: this Alice installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Alice."
    )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    container_mode_file = get_alice_home() / ".container-mode"
...
# Re-export from alice_constants — canonical definition lives there.
from alice_constants import get_alice_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_alice_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_alice_home() / ".env"
...
def _is_container() -> bool:
    ...
    When Alice runs in a container with volume-mounted config files, forcing
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ALICE_HOME, upgrading legacy empty templates.
...
def _ensure_alice_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"ALICE_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Alice-level retry loop that wraps the whole call.  Lower
        ...
        # app, ACP) in a code workspace, Alice adds a coding operating brief
    ...
    "terminal": {
        ...
        #             ALICE_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ALICE_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Alice auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Alice sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Alice
        ...
        # bundled Alice image, which drops to the `alice` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Alice sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Alice should operate in it.
            "user_id": "",
            "session_key": "",
        ...
        # User-facing browser — the real browser Alice opens when it needs
        ...
        # inspect visually).  Set automatically by ``alice setup`` via the
    ...
    # Opt in via ``alice chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.alice/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: alice sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .alice.md, or .cursorrules before Alice applies
    ...
    # single read_file page exceeds these limits, Alice truncates the
    ...
    # See tools/kanban_tools.py and alice_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``alice dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``alice kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``alice kanban decompose`` and the kanban
        ...
        # ``alice profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `alice model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `alice` (and `alice chat`) launches by default:
        ...
        # e.g. `model · 68% · ~/projects/alice`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `alice pets`. Disabled until
        ...
        "pet": {
            ...
            # get_alice_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Alice appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.alice/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.alice/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.alice/cache/delegation/
    ...
    # response. If not, Alice feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Alice auto-pauses the goal and
    ...
    # cron daemon.
    #
    # See `alice curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `alice curator run --consolidate` overrides
        ...
        # normally restored on every `alice update`, so pruning them only
        ...
        # skipped), snapshot ~/.alice/skills/ into
        # ~/.alice/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `alice curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for alice-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "✦ *Alice Agent*" header.
    ...
    # replace Alice' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.alice/shell-hooks-allowlist.json.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `alice doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``alice_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Alice to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $ALICE_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
    ...
    # todos to ready, and fires `alice -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `alice` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `alice kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     alice-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Alice native (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.alice/logs/.
    ...
    # update model picker lists without shipping a alice-agent release.
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.alice/.env,
        ...
        # behavior: files must live under the Alice cache, under
        ...
        # cache (~/.alice/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.alice/state.db.
    ...
    "sessions": {
        ...
        # default of ``alice sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.alice/sessions/session_{sid}.json`` on every turn
    ...
    # ``alice update`` behaviour.
    "updates": {
        # Run a full ``alice backup``-style zip of ALICE_HOME before every
        # ``alice update``.  Backups land in ``<ALICE_HOME>/backups/`` and
        # can be restored with ``alice import <path>``.  Off by default:
        # zipping a large ALICE_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``alice update
        ...
        "backup_keep": 5,
        # What `alice update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<ALICE_HOME>/lsp/bin/`` on first use.
    ...
    # `alice native`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.alice/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.alice/.env (or your shell) and never in config.yaml.
            ...
            # ~/.alice/bin/ on first use.  When False you must install
    ...
    "computer_use": {
        ...
        # by default upstream. Alice disables it for our users unless they
        # explicitly opt in here. When false (default), Alice sets
    ...
    # Alice Desktop (Electron app) launch options. These only affect
    # `alice desktop`; they do not touch the CLI/gateway.
    "desktop": {
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'alice model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Alice auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Alice, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @alice:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #alice)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: alice-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'alice-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Alice API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Alice API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Alice API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.alice.config`` entries, then checks
    ...
    try:
        ...
        # should never break `alice update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Alice versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``alice_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'alice setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Alice won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'alice doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ALICE_HOME", "~/.alice")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "alice update / alice -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ALICE_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_alice_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `alice plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `alice curator status` has no
    ...
    if current_ver < 23:
        try:
            curator_dir = get_alice_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `alice config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `alice config set`"
                    )
    ...
    # behavior set it to true explicitly via `alice config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from alice_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: alice config set <key> <value>")
    ...
    # `alice update`, but only the version bump is persisted.
    ...
    # metadata.alice.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``alice config set``
    ...
    backends) — while display paths (``alice status``/``dump``) read ``name``
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.alice/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.alice/config.yaml.
    ...
    (which change ``ALICE_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/alice/config.yaml invalidates the
...
#   openai-codex (OAuth — alice auth) — OpenAI Codex
#   nous         (OAuth — alice auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.alice/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Alice writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.alice/.env.
    ...
    interactive menu render (`alice native`, `alice setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `alice native`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Alice env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.alice/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.alice/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.alice/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.alice/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Alice — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Alice vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.alice/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.alice/.env`` over ``os.environ``.

    Used for Alice-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ✦ Alice Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  alice config edit     # Edit config file", Colors.DIM))
    print(color("  alice config set <key> <value>", Colors.DIM))
    print(color("  alice setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `alice config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `alice config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: alice config set <key> <value>")
            print()
            print("Examples:")
            print("  alice config set model anthropic/claude-sonnet-4")
            print("  alice config set terminal.backend docker")
            print("  alice config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'alice config migrate' to add them")
        ...
        print("  alice config           Show current configuration")
        print("  alice config edit      Open config in editor")
        print("  alice config set <key> <value>   Set a config value")
        print("  alice config check     Check for missing/outdated config")
        print("  alice config migrate   Update config with new options")
        print("  alice config path      Show config file path")
        print("  alice config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``alice config`` UI so users
