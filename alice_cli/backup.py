"""
Backup and import commands for alice CLI.

`alice backup` creates a zip archive of the entire ~/.alice/ directory
(excluding the alice-agent repo and transient files).

`alice import` restores from a backup zip, overlaying onto the current
ALICE_HOME root.
"""
...
# ``alice-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/alice-agent/``
...
# ALICE_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "alice-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under ALICE_HOME) —
...
# File names that ``alice import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE ALICE_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside ALICE_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to alice root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``alice-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/alice-agent/`` — must be preserved.
        if part == "alice-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ALICE_HOME.
...
def run_backup(args) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"alice-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"alice-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_alice_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(lydia_root, followlinks=False):
        ...
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "alice-agent" and not is_root)
        ]
    ...
    # outside ALICE_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_alice_home()}."
        )
    ...
    print(f"\nRestore with: alice import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Alice backup.
    ...
    # Look for telltale files that a alice home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Alice backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.alice/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a alice dir name
        if prefix in {".alice", "alice"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_alice_home()}")
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Alice configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under ALICE_HOME.
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_alice_home()}."
            )
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # alice_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: alice profile list  (after installing alice)")
        ...
        if restored_profiles:
            ...
            for pname in gw_profiles:
                print(f"  alice -p {pname} gateway install")

        print("Done. Your Alice configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and alice backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ALICE_HOME).
...
# — `alice update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $ALICE_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside ALICE_HOME and the entry is silently
...
# Relative path of the cron job database inside ALICE_HOME. Kept in sync with
...
def run_quick_backup(args) -> None:
    """CLI entry point for alice backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_alice_home()}/state-snapshots/")
...
def create_pre_update_backup(
    ...
    but writes to ``<ALICE_HOME>/backups/pre-update-<timestamp>.zip`` and
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `alice claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    ``alice claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``alice import <archive>``.  Writes to
    ``<ALICE_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
