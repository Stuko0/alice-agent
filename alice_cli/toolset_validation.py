...
the tool registry or launching Alice (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``alice-cli`` to the non-existent ``alice``. ``resolve_toolset('alice')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``alice-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``alice`` instead of ``alice-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `alice native` to reconfigure."
        )
    return warnings
