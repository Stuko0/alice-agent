"""
Interactive setup wizard for Alice Agent.
...
from alice_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from alice_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("✦ Alice Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Alice using environment variables or config commands:")
    print_info("  alice config set model.provider custom")
    print_info("  alice config set model.base_url http://localhost:8080/v1")
    print_info("  alice config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'alice setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from alice_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, alice_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'alice setup' to configure"))
    ...
    # Video generation — opt-in via `alice native` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from alice_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'alice setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'alice setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'alice setup terminal'"))
    ...
    # Spotify (OAuth via alice auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'alice setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('alice setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('alice setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('alice setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('alice setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('alice setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('alice config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('alice config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('alice config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('alice', Colors.GREEN)}              Start chatting")
    print(f"   {color('alice gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('alice doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `alice native` and `alice setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``alice model``)
    ...
    provider added to ``alice model`` is automatically available here.
    ...
    # Delegate to the shared alice model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from alice_cli.main import select_provider_and_model
    try:
    ...
    # on demand via `alice auth add`, `alice setup` vision, and
    # `alice setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``alice model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from alice_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.alice/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Alice auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via alice setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'alice setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Alice runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # Image and resource limits use defaults; tune via `alice setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `alice setup agent` later to customize.")
...
def _setup_telegram_auto_result():
    ...
    try:
        from alice_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Alice delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Alice to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from alice_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from alice_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Alice from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'alice setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     alice config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_alice_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: alice gateway install --system")
                print_info("  Or run in foreground:  alice gateway")
        else:
            from alice_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   alice gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   alice gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `alice setup tools` and `alice native` use the same flow:
...
def setup_browser(config: dict):
    ...
    Alice will use this browser to open URLs when it wants to show the
    user a rendered page (dev server preview, browser_navigate result, etc).
    """
    from alice_cli.browser_scanner import scan_browsers

    print_header("Browser")
    print_info("Pick which browser Alice opens rendered web pages in.")
    print_info("Alice uses a headless browser for automation — this is the")
    ...
    if not browsers:
        ...
        print_info("Alice will use your system default browser.")
        print_info("You can configure a browser later with: alice setup browser")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``alice_cli.auth`` — lists every supported
    ...
    """
    try:
        from alice_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from alice_cli.config
    ...
    if section_key == "model":
        ...
        from alice_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_alice.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_alice migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_alice", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Alice.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Alice to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Alice at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Alice at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Alice at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Alice at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Alice equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Alice config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Alice.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Alice's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(alice_home: Path) -> bool:
    ...
    print_info("Alice can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: alice claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: alice claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Alice configs are
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Alice (use alice claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      alice setup           — full or quick (auto-detected)
      alice setup model     — just model/provider
      alice setup tts       — just text-to-speech
      alice setup terminal  — just terminal backend
      alice setup gateway   — just messaging platforms
      alice setup tools     — just tool configuration
      alice setup agent     — just agent settings
    """
    from alice_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ✦ Alice Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ✦ Alice Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Alice Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Alice configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'alice setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Alice?",
            [
    ...
    # Tune later with `alice setup agent`.
    if not is_existing:
    ...
    # Section 6: Browser — pick which browser Alice opens for rendered pages
...
def _run_first_time_quick_setup(config: dict, alice_home, is_existing: bool):
    ...
    settings and tools; the user can customize later via ``alice setup <section>``
    or switch providers with ``alice model``.
    """
    from alice_cli.config import load_config
    ...
    if selected_id == "_api_key":
        ...
        try:
            from alice_cli.model_setup_flows import _model_flow_copilot_acp
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'alice setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    alice setup")
    if gateway_choice != 0:
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``alice native`` (which
    ...
    try:
        ...
        from alice_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``alice setup agent`` /
    ``alice config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, alice_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `alice update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        alice native")
        print_info("  Seed skills:         alice skills opt-in --sync")
        print_info("  Add MCP servers:     alice mcp add")
...
def _blank_slate_walkthrough(config: dict, alice_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`alice update` runs from re-injecting them. Opt back in any")
            print_info("time with `alice skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `alice native`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `alice plugins list` / `alice plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `alice plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `alice mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `alice mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   alice native")
    print_info("  Seed skills:         alice skills opt-in --sync")
    print_info("  Add MCP servers:     alice mcp add")
    print_info("  Tune agent settings: alice setup agent")
    print()
...
def _run_quick_setup(config: dict, alice_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'alice setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Alice to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'alice setup gateway'.")
