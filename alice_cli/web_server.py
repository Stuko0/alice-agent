"""
Alice Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m alice_cli.main web          # Start on http://127.0.0.1:9119
    python -m alice_cli.main web --port 8080
"""
...
from alice_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from alice_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
try:
    ...
    # running `alice dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``alice gateway run`` — but the
    desktop app spawns a ``alice dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same ALICE_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import alice_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from alice_cli.gateway import _get_restart_drain_timeout
...
app = FastAPI(title="Alice Agent", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from alice_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
#
# This list is defined in ``alice_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from alice_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Alice-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``alice-0day``
...
async def _dashboard_auth_gate(request: Request, call_next):
    from alice_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from alice_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Alice (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``alice model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Alice provider slug — fine.
    ...
    1. Vendor-name → Alice-provider mapping: when the provider string is
       not a known Alice provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from alice_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from alice_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from alice_cli.models import _AGGREGATOR_PROVIDERS
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``alice update``.
    ...
    a container — e.g. the alice-webui image sharing the Alice source tree),
    the dashboard's ``alice update`` button is the correct update path and
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``alice:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.alice-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``alice_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from alice_cli import web_git as _web_git  # noqa: E402
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_alice_home() at call time
    ...
    try:
        ...
        # Windows install the first import of alice_cli.gateway blocks the
        ...
        try:
            from alice_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the alice home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_alice_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from alice_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from alice_cli.nous_subscription import get_nous_subscription_features
...
# streamed to a per-action log file under ``~/.alice/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_alice_home() / "logs"
...
def _spawn_alice_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "alice_cli.main", *subcommand]
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["alice", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``alice gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``alice gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``alice gateway restart`` in the background."""
    try:
...
async def update_alice():
    ...
    if _dashboard_local_update_managed_externally():
        message = (
            "Alice updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("alice-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "alice-update",
...
async def check_alice_update(force: bool = False):
    ...
    ``POST /api/alice/update`` actually runs ``alice update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Alice version string
    ...
    try:
        ...
        if force:
            try:
                (get_alice_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.alice/config.yaml`` under ``tts.``.
    """
...
def get_profiles_sessions(
    ...
    from alice_state import SessionDB
    from alice_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Alice surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Alice supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_alice_home() / provider.name / "config.json"
...
# in alice_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `alice model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `alice model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from alice_cli.auth import get_provider_auth_state
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from alice_cli.moa_config import normalize_moa_config
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.alice/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (alice_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from alice_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``alice model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from alice_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``alice model`` universe). This is what lets the desktop Keys tab
    ...
    try:
        from alice_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `alice model` provider — would be
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `alice model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Alice from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Alice to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Alice from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Alice to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Alice in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Alice through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Alice via Home Assistant.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Alice to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Alice inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Alice through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Alice to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Alice to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in alice gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in alice gateway setup",
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Alice Agent").strip() or "Alice Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Alice appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's ALICE_HOME.
...
# the canonical ``alice auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.alice/.anthropic_oauth.json`` — Alice-managed PKCE flow (what
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Alice even
    when they also have a separate Alice-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Alice never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "alice auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "alice auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "alice auth add minimax-oauth",
    ...
    # ── API-key providers ────────────────────────────────────────────
    {
        ...
        "flow": "pkce",
        "cli_command": "alice auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from alice_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Alice, so the disconnect
    ...
    user sees exactly what executes, and Alice then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Alice — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``alice model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `alice model` universe (zero-edit for new plugins).
    try:
        from alice_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"alice auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `alice model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Alice-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            ...
            try:
                from alice_cli.auth import clear_provider_auth
        ...
        try:
            from alice_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.alice/.anthropic_oauth.json AND credential pool
...
# Guarded so alice web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Alice file AND credential pool.
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "alice-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
# the tokens exactly like `alice auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `alice auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from alice_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from alice_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `alice sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `alice sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_alice_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from alice_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_alice_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from alice_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ALICE_HOME)."""
    from alice_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process ALICE_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (alice_cli.mcp_config), so
# servers managed here show up under `alice mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from alice_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from alice_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from alice_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from alice_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # ALICE_HOME override (see mcp_tool._wrap_with_home_override), so
...
#
# Wraps the same JSON store the CLI uses (alice_cli.webhook); the webhook
...
async def list_webhooks():
    import alice_cli.webhook as wh
...
# spawn the real `alice gateway <verb>` so behaviour matches the CLI exactly.
...
async def list_credential_pool():
    ...
    from alice_cli.auth import read_credential_pool
...
# `alice memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `alice memory setup` to configure a new one.",
            )
...
def _dashboard_backup_dir() -> Path:
    return get_alice_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"alice-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `alice import`. The spawned action runs with
...
async def list_hooks():
    ...
    try:
        from alice_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    cp_dir = get_alice_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``alice`` subprocess, and
...
# Human-readable labels for each hub source id (matches `alice skills search`
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from alice_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`alice -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the ALICE_HOME
...
def _profile_setup_command(name: str) -> str:
    ...
    return "alice setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ALICE_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local ALICE_HOME override (same mechanism as
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's ALICE_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``alice profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from ALICE_HOME).
    """
    from alice_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``alice profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the ALICE_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``alice profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from alice_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    retargeted to the *current* ``get_alice_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a ALICE_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    which is all endpoints that resolve ``get_alice_home()`` at call time
...
async def get_toolsets(profile: Optional[str] = None):
    from alice_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``alice tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``alice tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``alice tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.alice/.env`` via ``save_env_value`` —
    the same store ``alice tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``alice tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``alice -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same ALICE_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Alice,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``alice computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``alice --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from alice_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from alice_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from alice_cli.dashboard_auth.audit import AuditEvent, audit_log
        from alice_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``alice --tui`` would run.  Tests monkeypatch this
    ...
    ``ALICE_HOME`` at the profile dir in the child env. Every spawned
    ...
    """
    from alice_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from alice_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="alice-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Alice inside WSL2 to use the dashboard's /chat "
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`alice_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from alice_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/alice``)
    ...
    # under ``/alice`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Alice backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Alice Teal",         "description": "Classic dark teal — the canonical Alice look"},
    {"name": "default-large", "label": "Alice Teal (Large)", "description": "Alice Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.alice/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.alice/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_alice_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.alice/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    from alice_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_alice_home() / "plugins", "user"),
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_alice_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"alice auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from alice_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from alice_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from alice_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from alice_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.alice/plugins/``) ship with the CWD and are
...
from alice_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    try:
        from alice_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the alice-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `alice dashboard register` (Nous Portal) or "
