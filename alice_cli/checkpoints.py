"""`alice checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.alice/checkpoints/``.  Actions:

    alice checkpoints               # same as `status`
    alice checkpoints status        # total size, project count, breakdown
    alice checkpoints list          # per-project checkpoint counts + workdir
    alice checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    alice checkpoints clear [-f]    # nuke the entire base (asks first)
    alice checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    alice checkpoints
    alice checkpoints prune --retention-days 3 --max-size-mb 200
    alice checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: alice checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``alice checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `alice checkpoints` → status
