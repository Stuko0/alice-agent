...
Used by ``alice profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``alice_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Alice Agent kanban board.
...
Rules:
  ...
  - Never write "Alice Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_alice_home())
        else:
