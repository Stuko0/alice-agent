...
the existing module-level functions in alice_cli.gateway and
alice_cli.gateway_windows directly. This protocol is a thin facade
...
``alice gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``alice gateway start/stop/restart``).
    """
    ...
    # `alice gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged alice
    ...
    detection always fails. Since every Alice runtime call inside the
    container drops to alice via ``s6-setuidgid``, that silent failure
...
# in ``alice_cli.gateway`` (systemd/launchd) and ``alice_cli.gateway_windows``
...
# active (set via the ``alice -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in alice_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from alice_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from alice_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from alice_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from alice_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in alice_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from alice_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from alice_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from alice_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from alice_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``alice_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `alice profile create`
# runs inside the container (Phase 4). Static services (main-alice, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's ALICE_HOME blindly;
    derive the shared profile root from the current ALICE_HOME and map the
...
# UID/GID of the in-image ``alice`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the alice user.
    ...
    (the ``alice`` user — UID 10000 — running every Alice runtime
    ...
    is dispatched as the alice user.
    ...
    with alice ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           alice:alice, 0755 (parent must already exist)
    ``svc_dir/event/``                     alice:alice, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 alice:alice, 0755
    ``svc_dir/supervise/event/``           alice:alice, 03730
    ``svc_dir/supervise/control``          alice:alice, 0660    (FIFO)
    ...
    slot's supervise/ was alice-owned.
    ...
    # when the parent slot's supervise/ is alice-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``alice -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`alice profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-alice, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources ALICE_HOME (and any extra env) via with-contenv —
             so e.g. ``-e ALICE_HOME=/data/alice`` is honored at run
          ...
          4. Drops to the alice user and exec's
             ``alice -p <profile> gateway run`` (or just ``alice
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``alice gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root ALICE_HOME profile" (the implicit profile that exists at
        the top of $ALICE_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``alice_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$ALICE_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``ALICE_HOME``,
        ...
        allocator in ``alice_cli.profiles._allocate_gateway_port``).
        ...
        # the Open WebUI helper) grabs the per-ALICE_HOME PID lock first;
        ...
        if profile == "default":
            gateway_cmd = "alice gateway run --replace"
        else:
            gateway_cmd = f"alice -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid alice {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${ALICE_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the ALICE_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e ALICE_HOME=/data/alice``
        gets its logs under /data/alice/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``alice logs`` reads and what
        ...
        return (
            ...
            f': "${{ALICE_HOME:=/opt/data}}"\n'
            f'log_dir="$ALICE_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as alice and crash-loops on
            ...
            f'chown alice:alice "$ALICE_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R alice:alice "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid alice s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``alice gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``alice -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with alice ownership
            ...
            # the alice user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is alice-owned (see ``_seed_supervise_skeleton``), and on
