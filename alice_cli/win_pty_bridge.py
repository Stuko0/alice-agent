"""Windows ConPTY bridge for the `alice dashboard` chat tab.

Drop-in counterpart to ``alice_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``alice_cli.web_server`` consumes: ``spawn``, ``read``,
