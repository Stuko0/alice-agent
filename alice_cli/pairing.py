...
Usage:
    alice pairing list              # Show all pending + approved users
    alice pairing approve <platform> <code>  # Approve a pairing code
    alice pairing revoke <platform> <user_id> # Revoke user access
    alice pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle alice pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: alice pairing {list|approve|revoke|clear-pending}")
        print("Run 'alice pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.alice/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'alice pairing list' to see pending codes.\n")
