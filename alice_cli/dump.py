"""
Dump command for alice CLI.

Outputs a compact, plain-text summary of the user's Alice setup
...
def _redact(value: str) -> str:
    ...
    ``alice dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from alice_cli.gateway import get_gateway_runtime_snapshot
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `alice auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `alice auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["alice-cli"])
