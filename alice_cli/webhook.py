"""alice webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.alice/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: alice gateway run
"""
...
def webhook_command(args):
    """Entry point for 'alice webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: alice webhook {subscribe|list|remove|test}")
        print("Run 'alice webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (alice gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: alice webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from alice webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (alice gateway run)")
