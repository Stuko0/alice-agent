...
on Japanese locales, etc.).  Alice's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``alice_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Alice is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ALICE_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ALICE_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Alice-managed Git install directories so
    ...
    # already-running shells don't see the change — which means alice
    ...
    # install without Alice-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Alice's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Alice's
    ``alice config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Alice (User env var in Windows Settings, or
    export in a PowerShell profile) and Alice picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Alice-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\alice\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``alice`` from right after install) retain their old PATH.

    Any subprocess Alice spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Alice-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "alice", "git", "cmd"),
        os.path.join(local_appdata, "alice", "git", "bin"),
        os.path.join(local_appdata, "alice", "git", "usr", "bin"),
        # Alice venv Scripts directory — host of the alice.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes alice, but harmless to include.
        os.path.join(local_appdata, "alice", "alice-agent", "venv", "Scripts"),
