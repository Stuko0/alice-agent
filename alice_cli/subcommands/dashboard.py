"""``alice dashboard`` / ``alice serve`` subcommand parsers.
...
``alice_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `alice dashboard` / `alice serve` cmdlines and SIGTERM them directly —
    # the same path `alice update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Alice web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Alice web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Alice Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Alice desktop app shells (<= 0.15.x) spawn the
    # backend as `alice dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Alice couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Alice Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Alice backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Alice backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # `alice dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.alice/.env.
    # Nested subparser so bare `alice dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "ALICE_DASHBOARD_OAUTH_CLIENT_ID into ~/.alice/.env, and prints "
            "how to engage the login gate. Requires being logged in (alice setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://alice.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via ALICE_DASHBOARD_PORTAL_URL. Mainly for "
