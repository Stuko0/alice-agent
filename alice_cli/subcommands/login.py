...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``alice login`` was removed in favor of ``alice auth`` / ``alice model``
    (the runtime handler in ``alice_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``alice login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``alice --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``alice login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``alice model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `alice auth` to manage credentials, "
            "`alice model` to select a provider, or `alice setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `alice model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: alice-cli)"
    )
