...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Alice Agent. Use 'alice debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    alice debug share              Upload debug report and print URL
    alice debug share --lines 500  Include more log lines
    alice debug share --expire 30  Keep paste for 30 days
    alice debug share --local      Print report locally (no upload)
    alice debug share --no-redact  Disable upload-time secret redaction
    alice debug share --nous       Upload to Nous-internal storage (private)
    alice debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'alice debug share'",
    )
