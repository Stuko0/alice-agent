"""
Alice Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.alice/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Alice PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing alice-agent or alice PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Alice Agent" comment and following line
                if '# Alice Agent' in line or '# alice-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('alice' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing alice
                if 'alice' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the alice wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "alice",
        Path("/usr/local/bin/alice"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains alice_cli reference)
                content = wrapper.read_text()
                if 'alice_cli' in content or 'alice-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(alice_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``alice`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Alice home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (alice_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``alice gateway run`` processes
    ...
    try:
        from alice_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from alice_cli.gateway import get_launchd_plist_path
...
#      like ``%LOCALAPPDATA%\alice\git\cmd``, ``%LOCALAPPDATA%\alice\git\bin``,
#      ``%LOCALAPPDATA%\alice\git\usr\bin``, ``%LOCALAPPDATA%\alice\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\alice\git\`` and Node to
#      ``%LOCALAPPDATA%\alice\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``alice dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\alice\gateway-service\`` and sometimes
...
def _alice_path_markers(alice_home: Path) -> list[str]:
    """Path-entry substrings that identify Alice-owned User-PATH entries."""
    root = str(alice_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare alice-agent install dir.
...
def remove_path_from_windows_registry(alice_home: Path) -> list[str]:
    """Strip Alice-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _alice_path_markers(alice_home)
            kept: list[str] = []
...
def _is_default_alice_home(alice_home: Path) -> bool:
    """Return True when ``alice_home`` points at the default (non-profile) root."""
    try:
        from alice_constants import get_default_alice_root
        return alice_home.resolve() == get_default_alice_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ALICE_HOME directory.

    We shell out to ``alice -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ALICE_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m alice_cli.main` so we don't depend on a `alice`
    #    wrapper that may be half-removed mid-uninstall.
    alice_invocation = [_sys.executable, "-m", "alice_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                alice_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ALICE_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``alice uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ALICE_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from alice_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(alice_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ✦ Alice Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Alice Chat GUI installation was found.")
        print(f"  Checked: {alice_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Alice Agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(alice_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Alice Agent at {alice_home / 'alice-agent'}")
        print(f"  • Your config, sessions, and secrets under {alice_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Alice Agent is still installed. Run 'alice' to use the CLI,")
    print("or 'alice uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.alice/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.alice/ for future reinstall
    """
    project_root = get_project_root()
    alice_home = get_alice_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ALICE_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_alice_home(alice_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.alice data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ✦ Alice Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {alice_home / 'config.yaml'}")
    print(f"  Secrets: {alice_home / '.env'}")
    print(f"  Data:    {alice_home / 'cron/'}, {alice_home / 'sessions/'}, {alice_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ALICE_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Alice data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Alice code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``alice`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ALICE_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in alice_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\alice\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(alice_home))))
        ...
        if removed_env:
            ...
            log_info("No Alice-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing alice command...")
    ...
    #     (only when they still point into this Alice home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Alice-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(alice_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Alice-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(alice_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ALICE_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from alice_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(alice_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.alice/, just remove the alice-agent subdir
            if alice_home in project_root.parents or project_root.parent == alice_home:
    ...
    #     under ALICE_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(alice_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(alice_home)
    ...
    # 5. Optionally remove ~/.alice/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ALICE_HOME dirs live under
        ...
        try:
            if alice_home.exists():
                ...
                log_success(f"Removed {alice_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {alice_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {alice_home}")
    ...
    print()
    print("Thank you for using Alice Agent! ✦")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m alice_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import alice_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``alice_constants`` + ``alice_cli.colors``
    (and lazily ``alice_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m alice_cli.uninstall")
    parser.add_argument(
        "--mode",
