#!/usr/bin/env python3
"""
Alice CLI - Main entry point.

Usage:
    alice                     # Interactive chat (default)
    alice chat                # Interactive chat
    alice gateway             # Run gateway in foreground
    alice gateway start       # Start gateway as service
    alice gateway stop        # Stop gateway service
    alice gateway status      # Show gateway status
    alice gateway install     # Install gateway service
    alice gateway uninstall   # Uninstall gateway service
    alice setup               # Interactive setup wizard
    alice logout              # Clear stored authentication
    alice status              # Show status of all components
    alice cron                # Manage cron jobs
    alice cron list           # List cron jobs
    alice cron status         # Check if cron scheduler is running
    alice doctor              # Check configuration and dependencies
    alice honcho setup                    # Configure Honcho AI memory integration
    alice honcho status                   # Show Honcho config and connection status
    alice honcho sessions                 # List directory → session name mappings
    alice honcho map <name>               # Map current directory to a session name
    alice honcho peer                     # Show peer names and dialectic settings
    alice honcho peer --user NAME         # Set user peer name
    alice honcho peer --ai NAME           # Set AI peer name
    alice honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    alice honcho mode                     # Show current memory mode
    alice honcho mode [hybrid|honcho|local]  # Set memory mode
    alice honcho tokens                   # Show token budget settings
    alice honcho tokens --context N       # Set session.context() token cap
    alice honcho tokens --dialectic N     # Set dialectic result char cap
    alice honcho identity                 # Show AI peer identity representation
    alice honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    alice honcho migrate                  # Step-by-step migration guide: OpenClaw native → Alice + Honcho
    alice version             Show version
    alice update              Update to latest version
    alice uninstall           Uninstall Alice Agent
    alice acp                 Run as an ACP server for editor integration
    alice sessions browse     Interactive session picker with search

    alice claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: alice_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``alice_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``alice update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``alice_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, alice crashes on import and the user can't run
# ``alice update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import alice_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'alice' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``alice native`` or
      ...
      4. No-op on Windows (the .exe name is already ``alice.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("alice")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"alice", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"alice")
        # Windows: the .exe name is already ``alice.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* alice_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ALICE_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".alice", "config.yaml")
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`alice --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from alice_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Alice Agent v{__version__} ({__release_date__})")
...
from alice_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from alice_cli.subcommands.cron import build_cron_parser
from alice_cli.subcommands.gateway import build_gateway_parser
from alice_cli.subcommands.profile import build_profile_parser
from alice_cli.subcommands.model import build_model_parser
from alice_cli.subcommands.setup import build_setup_parser
from alice_cli.subcommands.postinstall import build_postinstall_parser
from alice_cli.subcommands.whatsapp import build_whatsapp_parser
from alice_cli.subcommands.slack import build_slack_parser
from alice_cli.subcommands.login import build_login_parser
from alice_cli.subcommands.logout import build_logout_parser
from alice_cli.subcommands.auth import build_auth_parser
from alice_cli.subcommands.status import build_status_parser
from alice_cli.subcommands.webhook import build_webhook_parser
from alice_cli.subcommands.hooks import build_hooks_parser
from alice_cli.subcommands.doctor import build_doctor_parser
from alice_cli.subcommands.security import build_security_parser
from alice_cli.subcommands.dump import build_dump_parser
from alice_cli.subcommands.debug import build_debug_parser
from alice_cli.subcommands.backup import build_backup_parser
from alice_cli.subcommands.import_cmd import build_import_cmd_parser
from alice_cli.subcommands.config import build_config_parser
from alice_cli.subcommands.version import build_version_parser
from alice_cli.subcommands.update import build_update_parser
from alice_cli.subcommands.uninstall import build_uninstall_parser
from alice_cli.subcommands.dashboard import build_dashboard_parser
from alice_cli.subcommands.gui import build_gui_parser
from alice_cli.subcommands.logs import build_logs_parser
from alice_cli.subcommands.prompt_size import build_prompt_size_parser
from alice_cli.subcommands.memory import build_memory_parser
from alice_cli.subcommands.acp import build_acp_parser
from alice_cli.subcommands.tools import build_tools_parser
from alice_cli.subcommands.insights import build_insights_parser
from alice_cli.subcommands.skills import build_skills_parser
from alice_cli.subcommands.pairing import build_pairing_parser
...
from alice_cli.subcommands.helpcmd import build_help_parser

def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (alice native, alice setup, alice model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'alice {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any alice module import.
#
# Many modules cache ALICE_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ALICE_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.alice/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ALICE_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `alice mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Alice' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo alice -p <name>` against the invoking user's home.
        ...
        candidate = home / ".alice" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`alice chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors alice_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ALICE_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.alice/profiles/coder or
    # /opt/data/profiles/coder).  If ALICE_HOME points to the alice root
    # instead (e.g. systemd hardcodes ALICE_HOME=/root/.alice), we must
    ...
    # "the root ALICE_HOME profile". If the reserved default child read
    ...
    # 3. If we found a profile, resolve and set ALICE_HOME
    if profile_name is not None:
        try:
            ...
            # A bug in profiles.py must NEVER prevent alice from starting
            print(
...
try:
    ...
    _cfg_path = get_alice_home() / "config.yaml"
    if _cfg_path.exists():
...
# Initialize centralized file logging early — all `alice` subcommands
...
try:
    from alice_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from alice_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if alice_constants not importable yet
...
from alice_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to alice_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (alice_cli.main._model_flow_*) keep resolving unchanged.
from alice_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_alice_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _has_any_provider_configured() -> bool:
    ...
    from alice_cli.config import get_env_path, get_alice_home, load_config
    from alice_cli.auth import get_auth_status

    # Determine whether Alice itself has been explicitly configured (model
    ...
    auth_file = get_alice_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo alice {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo alice {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  alice --tui --resume {target}")
    if title:
        print(f'  alice --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @alice/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@alice" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/alice-ink/package.json",
    "packages/alice-ink/index.js",
    "packages/alice-ink/text-input.js",
)
...
def _find_bundled_tui(alice_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if alice_cli_dir is None:
        alice_cli_dir = Path(__file__).parent
    bundled = alice_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``alice update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        "Error: the TUI workspace is missing from this Alice checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `alice update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Alice checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `alice --tui`\n"
        "If the checkout is still inconsistent, run `alice update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from alice_cli.dep_ensure import ensure_dependency
    ...
    if tui_dev:
        # Keep the local @alice/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @alice/ink resolves through
        # packages/alice-ink/dist/entry-exports.js. If that dist bundle is
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from alice_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from alice_cli.config import apply_terminal_config_to_env
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `alice update` so
    ...
    if code == 42:
        from alice_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    ``alice kanban boards switch`` from another session can flip the file
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.alice/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Alice — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Alice still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'alice sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from alice_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from alice_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'alice doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Alice isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  alice setup")
        print()

        from alice_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'alice setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from alice_cli.banner import prefetch_update_check
...
def cmd_proxy(args):
    ...
    from alice_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Alice?")
        print()
    ...
    # has no creds.json.  Every subsequent `alice gateway` then paid a 30s
    ...
    session_dir = get_alice_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: alice gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `alice gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  alice gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '✦ Alice Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  alice gateway")
        ...
        print()
        print("  Or install as a service: alice gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'alice whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``alice whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``alice_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from alice_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from alice_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from alice_cli.config import stamp_install_method
    from alice_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("✦ Alice post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from alice_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``alice model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from alice_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    from alice_cli.providers import resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'alice model' for "
                "available providers, or run 'alice doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in alice_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.alice/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.alice/.env if the active provider is not 'custom'.
    ...
    """
    from alice_cli.config import get_env_value, save_env_value, load_config
...
#
# Alice uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `alice model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `alice model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`alice_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from alice_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Alice only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``alice model`` flow, then route aux tasks to them here.
    """
    from alice_cli.config import load_config
    from alice_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from alice_cli.auth import _prompt_model_selection
    from alice_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from alice_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from alice_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from alice_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Alice URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from alice_cli.curses_ui import curses_radiolist
...
# downstream call sites read `alice_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `alice` invocation — including
# fast paths like `alice --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from alice_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``alice setup`` / ``alice model``.
    ...
    recover from a malformed paste without editing ``~/.alice/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from alice_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from alice_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `alice setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from alice_constants import display_alice_home as _dhh_fn

            print(
                f"    Alice will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Alice CLI with a provider."""
    from alice_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from alice_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from alice_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from alice_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from alice_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from alice_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``alice slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: alice slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `alice slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from alice_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from alice_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from alice_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from alice_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from alice_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `alice security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from alice_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from alice_cli.dump import run_dump
...
def cmd_config(args):
    ...
    from alice_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Alice home directory to a zip file."""
    if getattr(args, "quick", False):
        from alice_cli.backup import run_quick_backup
...
def cmd_import(args):
    """Restore a Alice backup from a zip file."""
...
def _print_version_info(*, check_updates: bool = True) -> None:
    from alice_cli.banner import format_banner_version_label
    ...
    try:
        from alice_cli.banner import check_for_updates
        from alice_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Alice Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from alice_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from alice_cli.uninstall import run_gui_uninstall
    ...
    from alice_cli.uninstall import run_uninstall
...
# Critical files that every ``alice`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``alice update`` again to roll forward. The post-pull syntax
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``alice`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="alice-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``alice update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``alice_cli/web_dist/`` (per vite.config.ts
    outDir: "../alice_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "alice_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``alice`` launcher present but ``alice_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the alice venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — alice venv python3, no nix-shell overhead
    ...
    # a alice venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``alice update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``alice web``).
    ...
    # build path usable on a stock `py -m alice_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (alice web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``alice update`` can unconditionally call ``alice desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``alice desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ALICE_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ALICE_HOME."""
    from alice_constants import get_alice_home
    return get_alice_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``alice update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Alice.app/Contents/MacOS/Alice"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Alice.exe",
            release_dir / "win-ia32-unpacked" / "Alice.exe",
            release_dir / "win-arm64-unpacked" / "Alice.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "alice",
            release_dir / "linux-unpacked" / "Alice",
            release_dir / "linux-arm64-unpacked" / "alice",
            release_dir / "linux-arm64-unpacked" / "Alice",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Alice'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Alice.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Alice.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Alice.exe: Access is
    ...
    an unrelated "Alice" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Alice is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Alice.app/Contents/MacOS/Alice  ->  app bundle = .../Alice.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``alice desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Alice Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Alice Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from alice_logging import setup_logging as _setup_logging_gui
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  alice gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Alice.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Alice.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Alice.exe, close any")
                    print("  running Alice desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> alice desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Alice is
    ...
    if source_mode:
        print("→ Launching Alice Desktop from source build...")
    ...
    print(f"→ Launching packaged Alice Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``alice dashboard`` processes other than ourselves.

    ``alice dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``alice update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Alice Desktop Electron app to protect its own
    ...
    patterns = [
        "alice dashboard",
        "alice_cli.main dashboard",
        "alice_cli/main.py dashboard",
        # The headless backend (`alice serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "alice serve",
        "alice_cli.main serve",
        "alice_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "alice.*dashboard"`) keeps us consistent
            # with `alice_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `alice update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``alice update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``alice update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: alice curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``alice dashboard`` processes.

    Called at the end of ``alice update`` (default ``reason``) and also
    from ``alice dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    alice dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.alice-update-staging"
    backup = f"{dst}.alice-update-old"
...
def _update_via_zip(args):
    """Update Alice Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `alice update "
            f"--branch {branch}`, or update against main with `alice update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="alice-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"alice-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for alice-agent itself
        ...
        # GitHub ZIPs extract to alice-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"alice-agent-{branch}")
    ...
    from alice_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: alice skills list-modified  "
    ...
    try:
        from alice_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "alice-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Alice behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes alice completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Alice couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Alice couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Alice behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Alice couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Alice couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `alice update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_alice_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_alice_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``alice update``, every profile is now current.
    """
    homes = []
...
# Install-scoped breadcrumb dropped right before ``alice update`` mutates the
...
# survives and the next ``alice`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $ALICE_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``alice update`` left half-done.
    ...
    protocol stream (``alice acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if alice.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _alice_exe_shims(scripts_dir)
            if shims:
                ...
                try:
                    ...
                    for _anc in [_me] + list(_me.parents()):
                        ...
                        if _anc_norm in _shim_set:
                            print(
                                "✗ Alice is running from the binary that "
                                "needs to be replaced — the auto-recovery "
                                "cannot overwrite a running executable."
                            )
                            print(
                                "  Restart Alice from a different terminal, "
    ...
    try:
        ...
        print(
            "⚠ A previous `alice update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from alice_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``alice update`` is still progressing even if pip/uv itself is silent.
    """
...
def _alice_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_alice_instances(
    ...
    The Alice Desktop Electron app spawns ``alice.EXE`` as a backend child,
    so during ``alice update`` the user-invoked process and the desktop's
    ...
    shims (``alice.exe`` / ``alice-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``alice update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``alice.exe``) is a separate process from the Python
    ...
    for shim in _alice_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated alice.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``alice update``
    ...
    #      alice.exe sitting *under* a non-Alice parent (e.g. a Alice
...
def _format_concurrent_instances_message(
    ...
    lines = ["✗ Another alice.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Alice Desktop, exit any open `alice` REPLs, and")
    lines.append("  stop the gateway (`alice gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `alice update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_alice_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``alice.exe``.
    ...
    when ``alice update`` runs, ``alice.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``alice.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next alice invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Alice Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Alice Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _alice_exe_shims(scripts_dir):
        ...
        print(
            "    Close Alice Desktop, exit other `alice` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `alice update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_alice_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``alice.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``alice.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``alice`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_alice_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``alice.exe.old.*`` left by prior updates.

    Called early on every alice invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``alice update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `alice update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``alice.exe`` / ``alice-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_alice_exe`` for the rationale.
    """
    ...
    # hours later inside ``alice update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``alice.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``alice.exe`` shim is locked during ``alice update``, or when uv/distlib
    skips a launcher write. The symptom is ``alice-agent.exe`` and
    ``alice-acp.exe`` present but ``alice.exe`` missing, so ``alice`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `alice update --force` after "
            "closing other alice processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m alice_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``alice update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``alice.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``alice`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `alice update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `alice update --force` after closing other alice processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from alice_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ALICE_HOME/bin/uv``, but on Termux the official
    ...
    """
    from alice_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `alice update`, so we install root-only first
    ...
    # hung. The chatty npm-deprecation noise during `alice update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``alice update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.alice/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``alice update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``alice update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.alice/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``alice update --gateway``) the update is already
    ...
    try:
        ...
        # alice_cli.config.get_alice_home to simulate setup failure.
        from alice_cli.config import get_alice_home as _get_alice_home_webhook

        logs_dir = _get_alice_home_webhook() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.alice/logs/update.log`` only, never the terminal.

    During ``alice update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``alice update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from alice_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``alice update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from alice_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from alice_cli.config import recommended_update_command
        from alice_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``alice update`` without requiring a reinstall.  The
    ...
    ``alice`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/alice.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v alice'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/alice, code at /usr/local/lib/alice-agent).
    fhs_link = Path("/usr/local/bin/alice")
    ...
    path_comment = (
        "# Alice Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ALICE_HOME before running the update.
    ...
    ALICE_HOME directories.  The ``--backup`` flag on ``alice update``
    ...
    # ``alice_cli/config.py``). Fall back to false if the key is missing
    ...
    # Render path using display_alice_home so the user sees ~/.alice/...
    try:
        from alice_constants import get_alice_home, display_alice_home

        home = get_alice_home()
        try:
            display_path = f"{display_alice_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  alice import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    alice.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from alice_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Alice...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m alice_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: alice gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``alice gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from alice_cli import gateway_windows
        from alice_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``alice update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Alice Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Alice Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `alice update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Alice via pip (for PyPI installs)."""
    from alice_cli import __version__
    from alice_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "alice-agent"]
        ...
        cmd = [pipx, "upgrade", "alice-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "alice-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "alice-agent"]
    ...
    print("✓ Update complete! Restart alice to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("✦ Updating Alice Agent...")
    print()

    # On Windows, abort early if another alice.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_alice_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $ALICE_HOME, this is cheap
        ...
        # orphan merge-conflict markers in alice_cli/config.py bricked
        # every user who ran ``alice update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``alice
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``alice update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_alice_home added to alice_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``alice`` launch finishes the
        ...
        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `alice desktop` to retry)")
                tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
                if tail:
                    print(tail)
                from alice_constants import display_alice_home as _dhh
        ...
        # current — so copy it straight over ~/.alice/cache/model_catalog.json
        ...
        # modules in this process.  Reload alice_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_alice_home() added since the last release.
        try:
            import importlib
            import alice_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ALICE_HOME so
        # it is not affected by sync_skills()'s module-level ALICE_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ALICE_HOME env var points at the default or a named profile.
        try:
            from alice_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from alice_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'alice config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: alice config migrate")
            else:
                print()
                print("Skipped. Run 'alice config migrate' later to configure.")
        else:
        ...
        # check `alice curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``alice update`` gives users a
        ...
        try:
            if sys.platform in ("darwin", "win32", "linux") and shutil.which("cua-driver"):
                from alice_cli.tools_config import install_cua_driver
        ...
        # When running as ``alice update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_alice_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... alice-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... alice-gateway*)
            ...
            # Discover all alice-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. alice-gateway.service or alice-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `alice update` restart it automatically, allow\n"
                        ...
                        print(
                            ...
                            f"Check the gateway with: alice gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: alice gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: alice -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Alice gateway unit files are still installed.
        # When both alice.service (from a pre-rename install) and the
        # current alice-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `alice update` surfaces the issue until the user migrates.
        try:
            from alice_cli.gateway import (
                has_legacy_alice_units,
                _find_legacy_alice_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_alice_units():
                print()
                print("⚠ Legacy Alice gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_alice_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (alice.service) fight the current")
                print("  alice-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    alice gateway migrate-legacy")
                print()
        ...
        print("  alice model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``alice -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `alice profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_alice_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → alice -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.alice)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  alice profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     alice -p {name} chat")
                else:
        ...
        from alice_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from alice_constants import get_alice_home as _hh
        ...
        from alice_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `alice profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → alice -p {name}  ({wrapper})")
        print()
        ...
        from alice_cli.profiles import profile_exists, validate_alias_name
        ...
        from alice_cli.profiles import rename_profile
        ...
        from alice_cli.profiles import export_profile
        ...
        from alice_cli.profiles import import_profile
        ...
        import tempfile
        from alice_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  alice -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      alice -p {plan.manifest.name} chat")
        ...
        from alice_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `alice profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"alice -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from alice_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from alice_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _report_dashboard_status() -> int:
    """Print ``alice dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``alice dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No alice dashboard processes running.")
        return 0

    print(f"{len(pids)} alice dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``alice dashboard register`` for OAuth.
    ...
    try:
        from alice_cli.web_server import should_require_auth
    ...
    print("    [2] OAuth via Nous Portal (run `alice dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    try:
        from alice_cli.plugins import discover_plugins
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No alice dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `alice update`.
    ...
    # launches the dashboard (`worker dashboard` → ALICE_HOME points into
    ...
    try:
        from alice_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        reexec_argv = [
            sys.executable, "-m", "alice_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        # ALICE_HOME.  We must resolve the root explicitly instead of just
        # dropping ALICE_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ALICE_HOME=/opt/data`), so an unset
    ...
    # the same logs directory as every other Alice surface.
    try:
        from alice_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    try:
        from alice_cli.mcp_startup import start_background_mcp_discovery
    ...
    from alice_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from alice_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from alice_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from alice_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from alice_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Alice log files."""
    from alice_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from alice_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``alice -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``alice_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``alice -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``alice`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from alice_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from alice_cli.oneshot import run_oneshot
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `alice --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_alice_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_alice_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_alice_home()}/memories/\n")
    else:
        from alice_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Alice Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from alice_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from alice_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from alice_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from alice_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from alice_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from alice_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from alice_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from alice_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from alice_cli.claw import claw_command
...
def cmd_help(args):
    from alice_cli.help_command import help_command

    raise SystemExit(help_command(args))
...
def main():
    ...
    try:
        from alice_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``alice.exe.old.*`` quarantine files left by previous
    # ``alice update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_alice_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``alice update``
    ...
    # ``alice skills install update``) merely defers recovery one launch;
    # under-matching (missing ``alice -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in alice_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from alice_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from alice_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `alice model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    from alice_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from alice_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in alice_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in alice_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in alice_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in alice_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `alice whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in alice_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from alice_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in alice_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in alice_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in alice_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in alice_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in alice_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in alice_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from alice_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from alice_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from alice_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in alice_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in alice_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in alice_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in alice_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in alice_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in alice_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.alice/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo alice uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from alice_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in alice_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in alice_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in alice_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in alice_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from alice_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in alice_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``alice --help``, ``alice version``, ``alice logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from alice_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from alice_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Alice shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from alice_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from alice_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in alice_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in alice_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `alice computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `alice native` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `alice computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Alice app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                version = ""
                try:
                    from alice_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: alice computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: alice computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: alice computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: alice computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: alice computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: alice computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in alice_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_alice_home() / "sessions"
            ...
            # Launch alice --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in alice_cli/subcommands/insights.py)
    # =========================================================================
    ...
    build_help_parser(subparsers, cmd_help=cmd_help)
    ...
    # =========================================================================
    # update command  (parser built in alice_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in alice_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in alice_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in alice_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in alice_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Alice-Setup.exe success screen tells users to
    # run `alice desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in alice_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in alice_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in alice_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``alice -c Pokemon Agent Dev`` → ``alice -c 'Pokemon Agent Dev'``
    ...
    from alice_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'alice -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (alice hooks list, cron
