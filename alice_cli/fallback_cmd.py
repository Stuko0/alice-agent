"""
alice fallback — manage the fallback provider chain.
...
Subcommands:
  alice fallback [list]   Show the current fallback chain (default when no subcommand)
  alice fallback add      Pick provider + model via the same picker as `alice model`,
                           then append the selection to the chain
  alice fallback remove   Pick an entry to delete from the chain
  alice fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.alice/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from alice_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `alice model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  alice fallback add")
        print()
        return
    ...
    print("  Docs: https://alice-agent.stuko.dev/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `alice model`, then append the selection to the chain."""
    from alice_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `alice model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `alice fallback list` to view, or `alice fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``alice fallback [subcommand]``."""
