"""Slash command definitions and autocomplete for the Alice CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Alice config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Alice works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Alice is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.alice/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Alice Agent to the latest version", "Info"),
    CommandDef("version", "Show Alice Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`alice_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/alice`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Alice ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Alice
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``alice skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``alice skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``alice skills list`` and
...
# unrelated command landed. These claim their slots right after /alice,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /alice <command>).
...
# native slot, the alias spelling stays reachable via /alice reset).
...
# ``/alice <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/alice" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /alice credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /alice billing.
#   - moa: high-cost slash mode, available through /alice moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /alice debug on Slack.
_SLACK_VIA_ALICE_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/alice <verb>`` subcommand.
    ...
    can still reach them via ``/alice <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/alice`` is always reserved as the first entry so the
    legacy ``/alice <subcommand>`` form keeps working for anything that
    ...
    # Reserve /alice as the catch-all top-level command.
    entries.append(("alice", "Talk to Alice or run a subcommand", "[subcommand] [args]"))
    seen.add("alice")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_ALICE_ONLY:
            # Intentionally Slack-via-/alice only (see _SLACK_VIA_ALICE_ONLY).
            return
    ...
    # everything except /alice, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://alice-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /alice handler.

    Maps both canonical names and aliases so /alice bg do stuff works
    the same as /alice background do stuff.

    Plugin-registered slash commands are included so ``/alice <plugin-cmd>``
