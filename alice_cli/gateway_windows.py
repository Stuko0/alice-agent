...
_TASK_NAME_DEFAULT = "Alice_Gateway"
_TASK_DESCRIPTION = "Alice Agent Gateway - Messaging Platform Integration"
...
def _preserve_alice_home_path(path: str | Path) -> str:
    ...
    path lives under ALICE_HOME.
    """
    candidate = Path(path)
    try:
        from alice_cli.config import get_alice_home

        home = Path(get_alice_home())
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Alice profile."""
    from alice_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "alice_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    Default profile: ``Alice_Gateway``
    Named profile X: ``Alice_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during alice_cli boot.
    from alice_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\alice\\gateway-service\\<task_name>.cmd``
    (or ``<ALICE_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Alice installs stay self-contained).
    """
    _assert_windows()
    from alice_cli.config import get_alice_home

    script_dir = Path(get_alice_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``ALICE_HOME`` whenever it
    ...
    source checkout only if ``ALICE_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports ALICE_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m alice_cli.main [--profile X] gateway run``
    ...
    prog_args = [pythonw_path, "-m", "alice_cli.main"]
    if profile_arg:
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Alice builds and failed
...
def windowless_gateway_restart_spec(
    ...
    ``alice_cli`` package without the venv launcher's site config.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m alice_cli.main
    ...
    from alice_cli.config import get_alice_home
    from alice_cli.gateway import PROJECT_ROOT
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m alice_cli.main gateway run``
    ...
    log_dir = Path(get_alice_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from alice_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `alice -p <profile> gateway install` must be safe.
    ...
    from alice_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"alice {profile_arg} gateway start" if profile_arg else "alice gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: alice gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Alice gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: alice gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: alice gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from alice_cli.config import get_alice_home
        print(f"    type {Path(get_alice_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_alice_home())}\\logs\\gateway-stdio.log")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Alice gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: alice gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: alice gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: alice gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `alice gateway status`."
        )
