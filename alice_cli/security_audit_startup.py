...
3. Running inside a container with no persistent volume mount over the
   ALICE_HOME data dir (state is ephemeral — lost on container restart).
...
logger = logging.getLogger("alice.security_audit")
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Alice as an unprivileged user (or in a "
