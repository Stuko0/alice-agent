...
Profile-aware location: ``$ALICE_HOME/logs/dashboard-auth.log``.
...
from ``alice_constants`` or other alice_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ALICE_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``alice_constants.get_alice_home`` semantics: env var wins,
    else ``~/.alice``. A local copy avoids an import cycle with the
    middleware which lives below ``alice_cli``.
    """
    home = os.environ.get("ALICE_HOME") or str(Path.home() / ".alice")
