...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with alice-agent;
...
from alice_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Alice' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by alice_cli.kanban_db when a task
    ...
    # separate `alice -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `alice kanban dispatch`),
...
class PluginManifest:
    ...
    #              in ~/.alice/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``alice plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Alice profile name (e.g. ``"default"``).

        Derived from ``ALICE_HOME`` via
        :func:`alice_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.alice/profiles/<name>``, or ``"custom"`` when
        ``ALICE_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Alice core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``alice honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``alice <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`alice_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from alice_cli.dashboard_auth import (
    ...
    def register_slack_action_handler(
        ...
        Alice' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``alice model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: alice_cli.main imports plugins indirectly
        from alice_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.alice/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to alice_cli/. Two layouts are
        ...
        # 2. User plugins (~/.alice/plugins/)
        user_dir = get_alice_home() / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with alice and must
            ...
            # all ~20 of them added several seconds to every `alice`
            # invocation — including plain `alice chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Alice ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `alice plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``alice plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`alice plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `alice plugins list`.
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="alice-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``alice native`` TUI so plugin-provided toolsets appear
