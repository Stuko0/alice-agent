...
#
# Wired into the image as /etc/cont-init.d/01-alice-setup by the
...
# `id -u alice` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# alice UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u alice)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-alice UID).
...
and the baked /opt/alice install tree is intentionally root-owned and
...
The image remaps the alice user to that UID/GID at boot and chowns the data
...
# host's `docker` group (or root). The supervised alice user (UID 10000)
...
# alice process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes alice. Idempotent across container
...
#   - alice is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G alice 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] alice already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" alice 2>/dev/null; then
        echo "[stage2] Added alice to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group alice failed; docker backend may fail with EACCES"
    fi
    break
done
...
chown_lydia_tree() {
    ...
    chown -R alice:alice "$target" 2>/dev/null || \
...
# --- Seed directory structure as alice user ---
# Run as alice via s6-setuidgid so dirs end up owned correctly (matters
...
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/alice/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Alice spawns for the
...
# main-wrapper.sh propagates it into the supervised ``alice`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-alice, dashboard, etc.).
