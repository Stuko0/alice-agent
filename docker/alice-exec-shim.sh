#!/bin/sh
# shellcheck shell=sh
# /opt/alice/bin/alice — `docker exec` privilege-drop shim.
#
# Background
# ----------
...
# returns "Provider authentication failed: Alice is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> alice
...
# Root, no opt-out. Drop to the alice user.
#
...
# Reset HOME to the alice user's home before dropping privileges. Without
