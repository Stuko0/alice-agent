...
# shellcheck shell=sh
# /opt/alice/docker/main-wrapper.sh — wraps the container's CMD with
...
#
# Routing:
#   no args                       → exec `alice` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `alice <args>` (subcommand passthrough)
#
# Drop to alice via s6-setuidgid, but skip it when already non-root.
set -e

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid alice "$@"; exec "$@"; }
...
# an arbitrary non-root, non-alice UID, the bootstrap was skipped and the
# baked image dirs (owned by the alice build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u alice)" ]; then
    cat >&2 <<EOF
[alice] ERROR: container started with --user $cur_uid (an arbitrary, non-alice UID) — not supported.
...
# to the alice user's home before dropping privileges so libraries that
