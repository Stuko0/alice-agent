...
2. **Las herramientas referenciadas en el cuerpo de SKILL.md deben ser herramientas nativas de Alice o servidores MCP que la habilidad espere explícitamente.** Usa los nombres de herramientas en comillas invertidas: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, etc.
...
Alice usa un sistema de skins basado en datos — no se necesitan cambios de código para añadir una nueva skin.

**Opción A: Skin de usuario (archivo YAML)**

Crea `~/.alice/skins/<nombre>.yaml`:
...
Añade al dict `_BUILTIN_SKINS` en `alice_cli/skin_engine.py`. Usa el mismo esquema que arriba pero como dict de Python.
...
Alice se ejecuta en Linux, macOS y Windows nativo (además de WSL2). Al escribir código
...
Alice tiene acceso al terminal. La seguridad importa.
...
2. **Probar manualmente**: Ejecuta `alice` y ejercita la ruta de código que cambiaste
